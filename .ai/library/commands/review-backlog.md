...
4. **For items to PROMOTE:**
   ...
   - Rename the directory from `999.x-slug` to `{new_num}-slug`:
     ```bash
     NEW_NUM=$(workflow-tools query phase.add "${DESCRIPTION}" --raw)
     ```
...
6. **Commit changes:**
   ```bash
   workflow-tools query commit "docs: review backlog — promoted N, removed M" --files .planning/ROADMAP.md
   ```
