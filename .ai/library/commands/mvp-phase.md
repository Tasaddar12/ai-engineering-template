...
4. Delegates to `/workflow plan-phase <N>` which auto-detects MVP mode via the roadmap field

Phase 1 of the vertical-mvp-slice PRD shipped the planner-side machinery; this command is the user entry point for it.
</objective>

<execution_context>
@.ai/library/workflows/mvp-phase.md
...
The phase must already exist in ROADMAP.md (created via `/workflow new-project`, `/workflow add-phase`, or `/workflow insert-phase`). This command does not create new phases — it converts an existing phase to MVP mode.
</context>

<process>
Execute the mvp-phase workflow from @.ai/library/workflows/mvp-phase.md end-to-end.
