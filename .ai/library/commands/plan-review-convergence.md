---
name: workflow:plan-review-convergence
...
<objective>
Cross-AI plan convergence loop — an outer revision gate around workflow-review and planner.
...
**Flow:** Skill("workflow-plan-phase") → Agent→Skill("workflow-review") → check unresolved HIGH + actionable non-HIGH → Skill("workflow-plan-phase --reviews") → Agent→Skill("workflow-review") → ... → Converge or escalate

Replaces workflow-plan-phase's internal plan-checker with external AI reviewers (codex, gemini, etc.). Plan-phase runs **inline** (bare Skill at depth 0) so it can spawn planner/plan-checker at depth 1. Review runs inside an isolated Agent (workflow-review is a Bash leaf — no sub-agents needed). Orchestrator only does loop control.

**Orchestrator role:** Parse arguments, validate phase, run plan-phase inline (Skill at depth 0), spawn an Agent for workflow-review, check unresolved HIGH and actionable non-HIGH counts, stall detection, escalation gate.
</objective>

<execution_context>
@.ai/library/workflows/plan-review-convergence.md
