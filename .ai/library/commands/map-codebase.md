...
<execution_context>
@.ai/library/workflows/map-codebase.md
</execution_context>
...
- If it is `--fast`: strip the flag, then read and execute `.ai/library/workflows/scan.md` (passing remaining args including optional --focus). Load it on demand here — it is deliberately not in `<execution_context>`, so the common full-map path does not pay for it.
...
- Via /workflow:onboard for first-time brownfield setup - creates codebase map first
- After /workflow:new-project (greenfield codebases) - updates codebase map as code evolves
...
7. Offer next steps (typically: /workflow:onboard, /workflow:new-project, or /workflow:plan-phase)
</process>
