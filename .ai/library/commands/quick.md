---
name: workflow:quick
description: Execute a quick task with Workflow guarantees (atomic commits, state tracking) but skip optional agents
...
---
<objective>
Execute small, ad-hoc tasks with Workflow guarantees (atomic commits, STATE.md tracking).
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  workflow-tools query frontmatter.get .planning/quick/{dir}/SUMMARY.md status
  ```
...
---
Resume with: /workflow:quick resume {slug}
```
...
5. Load context via:
   ```bash
   workflow-tools query init.quick
   ```
...
- Status fields read via `workflow-tools query frontmatter.get` — never eval'd or shell-expanded
</security_notes>
