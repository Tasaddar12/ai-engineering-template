---
name: workflow:mempalace-recall
...
**DO NOT use `workflow-tools config get-value`** -- it hard-exits on missing keys.
...
MemPalace memory is disabled. To activate:
  ...
  node <runtime-home>/workflow-core/bin/workflow-tools.cjs config-set mempalace.enabled true
...
_MemPalace unavailable at recall time — falling back to Workflow native memory (`.planning/graphs/`, STATE). No palace recall this run._
```
