---
type: prompt
name: workflow:complete-milestone
...
- @.ai/library/workflows/complete-milestone.md (main workflow)
...
0. **Check for audit:**
   ...
   - If missing or stale: recommend `/workflow:audit-milestone` first
   - If audit status is `gaps_found`: recommend closing the gaps inline
     ...
     via `/workflow:phase --insert <N>` plus the standard
   ...
   ⚠ No milestone audit found. Run `/workflow:audit-milestone` first to verify
   ...
   a closure phase per gap with `/workflow:phase --insert <N>` and run the
   standard `/workflow:discuss-phase` → `/workflow:plan-phase` → `/workflow:execute-phase`
...
- **Fresh requirements:** Next milestone starts with `/workflow:new-milestone` which includes requirements definition
  </critical_rules>
