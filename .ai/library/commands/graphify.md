...
**CJS-only (graphify):** `graphify` subcommands are not registered on `workflow-tools query`. Use the `workflow_run` launcher shim (defined in each bash block below) or invoke the binary directly: `node <runtime-home>/workflow-core/bin/workflow-tools.cjs graphify …` where `<runtime-home>` is your runtime's config directory (e.g. `~/.claude`, `~/.hermes`, `~/.cursor`). See `docs/CLI-TOOLS.md` for details. Other tooling may still use `workflow-tools query` where a handler exists.
...
**DO NOT use the workflow-tools config get-value command** -- it hard-exits on missing keys.
...
Knowledge graph is disabled. To activate:
  ...
  node <runtime-home>/workflow-core/bin/workflow-tools.cjs config-set graphify.enabled true

Then run /workflow:graphify build to create the initial graph.
```
...
workflow_run graphify query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If the output contains `"error"` field, display the error message and **STOP**
- If no nodes found, display: `No graph matches for '<term>'. Try /workflow:graphify build to create or rebuild the graph.`
...
```text
Workflow > Building knowledge graph...
```
...
graphify update . \
  ...
  && workflow_run graphify build snapshot \
  && workflow_run graphify status
```
...
**MVP-mode rendering.** When a phase has `**Mode:** mvp` in ROADMAP.md (resolved via `workflow-tools query roadmap.get-phase --pick mode`), render its graph node with two distinct visual signals:
...
5. DO NOT use `workflow-tools config get-value` for the config gate -- it exits on missing keys.
