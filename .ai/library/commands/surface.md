...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `~/.claude/.workflow-surface.json`
(sibling to `~/.claude/.workflow-profile`) and re-stages the active skills directory in place.
Skill dirs live at `~/.claude/skills/workflow-*/`.
...
| `reset` | Delete `.workflow-surface.json`, return to install-time profile |
| *(none)* | Treat as `list` |
...
the engine module at `${runtimeConfigDir}/workflow-core/bin/lib/surface.cjs`. The registry is loaded via:
```js
const registry = require(runtimeConfigDir + '/workflow-core/bin/lib/capability-registry.cjs');
```
Display:
...
```
Base profile:   standard  (from .workflow-surface.json)
Install profile: standard  (from .workflow-profile)
```
...
1. Check if `.workflow-surface.json` exists.
...
`~/.claude/skills/workflow-*/` because the `claude global` layout has `destSubpath =
...
Surface state is stored at `${RUNTIME_CONFIG_DIR}/.workflow-surface.json`
(i.e. `~/.claude/.workflow-surface.json`).
...
- Missing `surface.cjs` → prompt: "Run `npm i -g @openworkflow/workflow-core` to reinstall Workflow."

<execution_context>
Surface state file: `~/.claude/.workflow-surface.json`
Install profile marker: `~/.claude/.workflow-profile`
Skill dirs: `~/.claude/skills/workflow-*/`
Engine module: `~/.claude/workflow-core/bin/lib/surface.cjs`
Cluster definitions: `~/.claude/workflow-core/bin/lib/clusters.cjs`
</execution_context>
