...
See also: `.ai/library/references/universal-anti-patterns.md` for the complete set of universal rules.
...
1. **Never** read agent definition files (`.ai/library/agents/*.md`) -- `subagent_type` auto-loads them
...
| POOR | 70%+ | Emergency mode. Checkpoint progress immediately. No new reads unless critical. | `warn`: emit warning + recommend `/workflow:pause-work`. `auto`: invoke pause-work before next wave. `off`: proceed anyway. |
...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever Workflow itself can save through `model_profile` tuning. This is a Claude Code harness concern, not a Workflow concern: Workflow does **not** manage MCP enablement. The toggle lives in `.claude/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
Before starting a long phase (especially `/workflow:execute-phase`, `/workflow:plan-phase`, or anything that fans out across many subagents), run this audit:
...
3. `confidence` is **derived, not judged**: it is the `confidence` value from the same calibration query, keyed to the sample count (`low` <3, `med` 3–5, `high` ≥6). **Do not rate your own certainty.** Self-rated confidence was measured in this project and found weak (`.ai/library/references/honest-verifier.md:25-29`); every signal here routes on measured history instead.
...
See @.ai/library/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
