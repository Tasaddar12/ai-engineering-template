...
The gate runs at `verify:pre` (before `/workflow:verify-work` begins UAT). A phase is
...
| No `.planning/phases` tree at all | pass — not a Workflow project layout |
...
```bash
echo "$PHASE_SCOPE" | node workflow-core/bin/lib/api-coverage.cjs --json
