...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @.ai/library/references/planner-antipatterns.md for vague-vs-specific comparison table.
...
Run `/clear` first for a fresh context window, then execute: `/workflow:execute-phase {phase}`
```

## Gap Closure Return (artifact-based — #3440)

Gap-closure completion has **no completion marker**: nothing consumes one. The return contract is the artifacts themselves — `verify-work.md` spawns you in `--gaps` mode, and your output is complete when the fix plans are on disk carrying `gap_closure: true` in their frontmatter; the spawning workflow detects the files (never a return string), and `/workflow:execute-phase {phase} --gaps-only` consumes them. (The former `GAP CLOSURE PLANS CREATED` completion marker was retired as vestigial — it had no dispatch branch anywhere.)
...
**Next step for the orchestrator:** `/workflow:execute-phase {phase} --gaps-only`
