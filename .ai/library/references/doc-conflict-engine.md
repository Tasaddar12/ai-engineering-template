...
Display:
```
Workflow > BLOCKED: {N} blockers must be resolved before {operation} can proceed.
```
...
Render the full report, then prompt for approval via the `approve-revise-abort` or `yes-no` pattern from `.ai/library/references/gate-prompts.md`. Respect text mode (see the workflow's own text-mode handling). If the user aborts, exit cleanly with a cancellation message.

**If the report is empty (no entries in any bucket):**

Proceed silently or display `Workflow > No conflicts detected.` Either is acceptable; workflows choose based on verbosity context.
