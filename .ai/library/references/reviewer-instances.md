...
Custom reviewer instances for `/workflow:review`: run one model-capable adapter (e.g. OpenCode)
as several independent reviewer identities in a single review pass. Loaded lazily by
`.ai/library/workflows/review.md` when `review.reviewer_instances` is configured. See
[ADR-1517](../SOURCES.md#source-b496b53eec510dbf) for the contract.
...
- `model`: opaque `provider/model` string, passed through verbatim. Workflow does not parse it.
...
workflow_run query review-lane invoke \
  --slug "$INSTANCE_CLI" \
...
`--as` is what makes the run write `{run_dir}/workflow-review-${INSTANCE_NAME}.md` instead of the lane's
own `{run_dir}/workflow-review-<slug>.md`.
...
Running 2+ instances changes how `/workflow:plan-review-convergence` counts HIGHs. Its **consensus gate**
