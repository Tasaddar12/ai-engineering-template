...
`@.ai/library/references/edge-probe.md`: `edge-probe` reaches the
...
- OWASP / prototype-pollution / path-traversal / injection → breadcrumb to `/workflow:secure-phase`
  and `eslint` (security plugins), not a minted prohibition.
- GDPR / data-retention / consent → breadcrumb to `/workflow:secure-phase`.
- Generic fairness/bias canon → breadcrumb to `/workflow:secure-phase`.

The breadcrumb reads like: *"prototype-pollution is canon — covered by /workflow:secure-phase +
...
- **verification** (only when `status` is `resolved`; `null` otherwise) — `test | judgment`
  ...
  - A **test**-tier prohibition is enforced + hard-gates via the deterministic
    ...
    **Machine-proven fail-first (#1279).** `failFirst` is now **machine-proven, not caller-attested**:
    ...
    - **`Workflow_PROHIB_SUBJECT`** — the node-test subject-injection convention: the producer spawns the
      negative test with `Workflow_PROHIB_SUBJECT=<violationFixture>` in the child env; the test reads that
    ...
    > **PROPOSED, renamable conventions (zero live consumers).** Both `Workflow_PROHIB_SUBJECT` and
...
- `check_violation_fixture` — path to a KNOWN-BAD subject the #1279 prover runs the check against to
  machine-prove fail-first (rides BOTH kinds; for `node-test` it is injected via `Workflow_PROHIB_SUBJECT`).
- `check_clean_fixture` — **optional** path to a KNOWN-CLEAN control subject (#1346). When present the
  ...
  caused by the subject's *content* (not merely by `Workflow_PROHIB_SUBJECT` being set). Absent → no control.
...
|----------------|---------------|------------------------------------------|----------------------------------------------|
| Workflow SPEC | a SPEC Requirement | a SPEC acceptance criterion (marked prohibition) that `plan-phase` lifts into `must_haves.prohibitions` | a `must_haves.prohibitions` item routed to judgment review |
...
verifier iterates over** (Workflow: a `must_haves.prohibitions` item with a test); a
