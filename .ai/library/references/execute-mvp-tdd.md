...
> Loaded by `execute-phase` workflow and `executor` agent when `TDD_MODE=true` for the phase (#4011 — the gate no longer requires MVP mode; MVP may imply TDD, but TDD never requires MVP). Defines the runtime gate that blocks behavior-adding tasks until a failing-test commit exists.
...
2. **The test was actually red — INTENTIONALLY (#3770).** A nonzero exit is not RED by itself: syntax errors, zero-test discovery, fixture crashes, parser errors, and unrelated assertions are INVALID_RED. The executor must persist the RED evidence record (command, exit code, failing test, expected result from `<behavior>`, actual result) and verify it:
   ```bash
   workflow_run check tdd-red-evidence <record.json> --raw
   ```
...
### TDD GATE TRIPPED — Plan {plan_id}, Task {task_id}
   ...
   3. Re-run /workflow execute-phase
   ```
...
The existing end-of-phase TDD review (in `.ai/library/workflows/execute-phase.md`'s `tdd_review_checkpoint` step) is normally **advisory** — it surfaces gate violations but does not block phase completion.
...
- The user is shown the same review table, but the verdict line reads:
  > "Phase blocked: {N} TDD plan(s) violate the RED→GREEN gate sequence under TDD. Resolve and re-run /workflow execute-phase, or override with `/workflow execute-phase {phase} --force-mvp-gate` to ship anyway."
...
- It does not enforce REFACTOR commits. REFACTOR remains optional (per `.ai/library/references/tdd.md`).
...
This gate is additive to `.ai/library/references/tdd.md`. Tasks not under TDD mode continue to use the existing advisory TDD discipline (RED/GREEN/REFACTOR commits with end-of-phase review checkpoint). Only the runtime gate and the blocking escalation are new.
