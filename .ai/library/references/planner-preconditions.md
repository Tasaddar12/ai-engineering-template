...
> Progressive-disclosure reference for `.ai/library/agents/planner.md`. The planner agent
...
> Contract). The slim pointer in `.ai/library/agents/planner.md` → `<task_breakdown>`
...
| Contract side | Workflow element | When it binds |
|---|---|---|
...
Workflow already models postconditions and invariants well. `<precondition>` closes
...
- `.ai/library/agents/executor.md` → `<execution_flow>` → precondition check step — the
