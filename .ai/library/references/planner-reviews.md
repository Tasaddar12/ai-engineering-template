...
**Execution contract:** REVIEWS.md is audit trail and feedback input, not a second execution contract. /workflow:execute-phase primarily consumes PLAN.md plus the normal phase context. Every current actionable review finding must therefore be incorporated into the relevant PLAN.md or explicitly deferred/rejected in that PLAN.md.
...
`.ai/library/workflows/plan-phase.md` (`<review_incorporation_contract>`) and
`.ai/library/agents/plan-checker.md` (Review Incorporation dimension) both point back to this section for
...
`git log -1 --format=%h -- <phase_dir>/<NN>-REVIEWS.md`, after `.ai/library/workflows/review.md`'s REVIEWS.md
