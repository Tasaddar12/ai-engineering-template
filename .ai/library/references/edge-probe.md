...
`@.ai/library/references/domain-probes.md`: `domain-probes` covers the
...
|----------------|---------------|--------------------------|---------------------------|
| Workflow SPEC | a SPEC Requirement | an Acceptance Criterion that `plan-phase` lifts into `must_haves.truths` | a non-inferable check in `must_haves.truths` (needs a held-out/PBT test) |
...
iterates over** (Workflow: a `must_haves.truth`); a `resolved`/`backstop` edge produces a test
