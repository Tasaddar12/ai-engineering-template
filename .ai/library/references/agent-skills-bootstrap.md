...
> see .ai/library/SOURCES.md#source-9118560644e86418 / #1601). This is the durable counterpart to the
> orchestrator-side injection documented under
> [Agent Skills Injection](../SOURCES.md#source-fec94568bc39d672).
...
2. **Query your configured skills.** Use **your own agent type** — the `name:` value in
   your frontmatter (e.g. an agent whose frontmatter says `name: executor` queries
   `executor`). The query is read-only and idempotent — it exits 0 with an empty
   block when nothing is configured for your type:

   ```bash
   _AGENT_SKILLS=$(workflow_run query agent-skills <YOUR-FRONTMATTER-NAME> 2>/dev/null || true)
   ```

   The runtime `workflow_run` resolver is the standard one from
