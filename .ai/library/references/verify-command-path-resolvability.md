...
`.ai/library/workflows/plan-phase.md` already runs the probe **before** spawning this checker and
...
invokes `workflow_run check verify-command-paths` itself. If `{VERIFY_PATHS}` is absent from the
