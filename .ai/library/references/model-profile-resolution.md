...
Resolve each agent's model through `workflow-tools`, then pass it to the `Agent()` spawn — or
...
```bash
AGENT_MODEL=$(workflow_run query resolve-model "<agent-type>" --raw)
```
...
@.ai/library/references/model-profiles.md
