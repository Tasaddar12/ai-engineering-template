...
Full technique bodies for `.ai/library/agents/debugger.md`, extracted per
...
6. **Shrinking (input-space bugs)** — when the bug triggers on a class of inputs, wrap it in a property (fast-check for JS/TS, Hypothesis for Python) and let the shrinker auto-minimize the counterexample; store the **minimized** input as the regression seed. See `.ai/library/references/debugger-repro-hardening.md`.
...
Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = ~/.claude/workflow-core
                  → writes to ~/.claude/workflow-core/hooks/
