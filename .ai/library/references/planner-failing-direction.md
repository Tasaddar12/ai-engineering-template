...
> Reference file for the planner agent. Loaded on-demand via `@` reference from the
> `<failing_direction_contract>` block of the planner spawn prompt in
> `.ai/library/workflows/plan-phase.md` — NOT from `.ai/library/agents/planner.md`, which is frozen
