...
Lazy-loaded by `.ai/library/workflows/plan-phase.md` step 7.95 (the gate) and the `<specless_probe_fallback>`
...
SPECLESS_CFG=$(workflow_run query config-get workflow.specless_probe_fallback 2>/dev/null || echo "true")
...
_workflow_lib() { for _d in "$_Workflow_RT/workflow-core/bin/lib" "$_Workflow_RT/bin/lib" "$_Workflow_RT/.claude/bin/lib" "$HOME/.claude/workflow-core/bin/lib" "$HOME/.claude/bin/lib"; do [ -f "$_d/$1" ] && { echo "$_d/$1"; return; }; done; }
SPEC_SECTION_JS=$(_workflow_lib spec-section.cjs)
if [ -z "$SPEC_SECTION_JS" ] && [ -f "$_Workflow_RT/tsconfig.build.json" ] && [ -f "$_Workflow_RT/src/spec-section.cts" ]; then
  npm --prefix "$_Workflow_RT" run build:lib 2>/dev/null || true; SPEC_SECTION_JS=$(_workflow_lib spec-section.cjs)
fi
[ -n "$SPEC_SECTION_JS" ] || { echo "ERROR: spec-section.cjs not found - reinstall Workflow or run build:lib." >&2; exit 1; }
...
```bash
# Resolve the compiled edge-probe.cjs against the Workflow install dir via RUNTIME_DIR (#448) — NOT the
# consuming project's git root — falling back to git toplevel / $HOME/.claude (spec-phase.md:198 idiom).
_Workflow_RT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
EDGE_PROBE_JS=$(for _c in \
  "$_Workflow_RT/workflow-core/bin/lib/edge-probe.cjs" "$_Workflow_RT/bin/lib/edge-probe.cjs" \
  "$_Workflow_RT/.claude/bin/lib/edge-probe.cjs" "$HOME/.claude/workflow-core/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/bin/lib/edge-probe.cjs"; do [ -f "$_c" ] && { echo "$_c"; break; }; done)
# Build ONLY inside a verified Workflow source checkout; --prefix pins npm so we never trigger the
# consuming project's build:lib. Never silent-skip (RR-04) — fail loud if unresolvable.
if [ -z "$EDGE_PROBE_JS" ]; then
  if [ -f "$_Workflow_RT/tsconfig.build.json" ] && [ -f "$_Workflow_RT/src/edge-probe.cts" ]; then
  ...
  [ -n "$EDGE_PROBE_JS" ] || { echo "ERROR: edge-probe.cjs not found — reinstall Workflow or run \`npm run build:lib\`." >&2; exit 1; }
fi
...
`insufficient_spec`), never a silent pass (#1154; `.ai/library/references/honest-verifier.md`). **Never
...
assumption. Pass `$COVERAGE` (+ the gate's `$SPECLESS_FALLBACK_DISABLED` note) into the planner
...
There is NO compiled prohibition engine and NO `node` invocation (ADR-550 D7b) — the planner runs
this in-prompt. Full two-stage protocol, canon-referral rule, and status×verification schema live in
`.ai/library/references/prohibition-probe.md` (do not inline it). Summary:
