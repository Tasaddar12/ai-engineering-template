0.5. **Inter-wave worktree base re-check (wave N+1 guard — #1369):**
   ...
   **Run this check at the start of every wave when `USE_WORKTREES != "false"` and
   ...
   if [ "$ISOLATION" = "harness-worktree" ] && [ "${USE_WORKTREES:-true}" != "false" ]; then
     _WAVE_DEGRADE=$(workflow_run query worktree.base-check --mode "$ISOLATION" --pick shouldDegrade 2>/dev/null || true)
     if [ "$_WAVE_DEGRADE" = "true" ]; then
       _WAVE_DEGRADE_MSG=$(workflow_run query worktree.base-check --mode "$ISOLATION" --pick message 2>/dev/null || true)
   ...
   **Why `worktree.baseRef:"head"` does not avoid this degrade (#48, #3659):** the runtime
   ...
   on runtimes where Workflow itself creates the worktrees (orchestrator-managed isolation:
