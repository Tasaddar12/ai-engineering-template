...
The field names are the plan-checker's schema (`.ai/library/agents/plan-checker.md` → `<issue_structure>`):
...
2. **Record**, where the host has a channel an arbitration loop reads. `review.md` emits one
   ...
   `<!-- workflow:plan-revision-conflicts:begin -->` and
   `<!-- workflow:plan-revision-conflicts:end -->`. When `workflow.plan_review_convergence` is enabled
...
3. **Resolve** — present the conflict and its alternatives to the user and ask which to take
   (pattern: `.ai/library/references/gate-prompts.md`): adopt a named alternative / override the
...
`/workflow:plan-review-convergence` — it runs *inside* that loop, so invoking it would be a cycle, and
...
2. Use gate prompt (pattern: yes-no from `.ai/library/references/gate-prompts.md`):
