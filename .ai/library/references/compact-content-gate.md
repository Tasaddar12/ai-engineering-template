...
```bash
COMPACT_CONTENT=$(workflow_run query config-get workflow.compact_content --raw 2>/dev/null || echo "false")
```

## Stream 1 — spine + detail (top-level, eagerly `@`-included workflows)

- **`COMPACT_CONTENT` is `"false"` (default):** Read every part under this workflow's own `detail/` directory (a sibling of this spine, e.g. `.ai/library/workflows/<name>/detail/*.md`) now, in full, before continuing past this point. Their content elaborates on the spine you are reading — treat everything they say as part of this document from here on.
...
For a `.ai/library/workflows/<name>/{modes,steps,templates}/*.md` fragment or a `.ai/templates/**`
...
**This rule is scoped per call site, not per file.** A `.ai/templates/**` file can have both
...
which kind it is; do not assume every mention of a `.ai/templates/**` path is a runtime `Read`
just because the directory's typical case is.

## Stream 2 — agent-skill payloads (the `workflow_run query agent-skills` CLI seam)

`.ai/library/agents/<name>.compact.md`, same directory, same stem, `.compact.md` suffix — registered and
...
not by a `workflow_run query config-get` prose instruction. There is nothing to state here for a
...
not: a missing sibling is disclosed in the served payload itself (a leading `<!-- workflow: no compact
