...
**For checkpoint format details, see `.ai/library/references/ui-brand.md`** -- a checkpoint is a `### CHECKPOINT: {Type}` heading followed by its content, then a `---` break and the bolded action prompt. Never a fixed-width drawn border: it wraps in a narrow pane.
...
- header: "Escalate"
- options: Accept gaps | Re-plan (via /workflow:plan-phase) | Debug (via /workflow:debug) | Retry
...
- header: "Scope"
- options: Quick task | Full plan (via /workflow:plan-phase) | Revise
