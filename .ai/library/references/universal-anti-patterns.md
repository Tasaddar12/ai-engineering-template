...
1. **Never** read agent definition files (`.ai/library/agents/*.md`) -- `subagent_type` auto-loads them. Reading agent definitions into the orchestrator wastes context for content automatically injected into subagent sessions.
...
3. **Read depth scales with context window** -- check `context_window` in `.planning/config.json`. At < 500000: read only frontmatter, status fields, or summaries. At >= 500000 (1M model): full body reads permitted when content is needed for inline decisions. See `.ai/library/references/context-budget.md` for the complete table.
...
10. **NEVER** use non-Workflow agent types (`general-purpose`, `Explore`, `Plan`, `Bash`, `feature-dev`, etc.) -- ALWAYS use `subagent_type: "workflow-{agent}"` (e.g., `phase-researcher`, `executor`, `planner`). Workflow agents have project-aware prompts, audit logging, and workflow context. Generic agents bypass all of this.
...
Reference: `.ai/library/references/questioning.md` for the full anti-pattern list.
...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `workflow_run query` for registered state/roadmap handlers (e.g. `state.update`, `state.advance-plan`, `roadmap.update-plan-progress`), or legacy `node …/workflow-tools.cjs` for CLI-only commands. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
24. **Do not** check for `mode === 'auto'` or `mode === 'autonomous'` -- Workflow uses `yolo` config flag. Check `yolo: true` for autonomous mode, absence or `false` for interactive mode.
25. **Prefer `workflow_run query`** for orchestration when a handler exists; when shelling out to the legacy CLI, go through the same `workflow_run` launcher rather than naming the shim file. The shim is not on PATH under any name ending in `.cjs`, and an agent that meets the bare filename falls back to searching the filesystem for it — on Git Bash for Windows that is a full-drive `find.exe` traversal (#3809). `workflow_run` resolves the CommonJS shim itself across every runtime home.
26. **Plan files MUST follow `{padded_phase}-{NN}-PLAN.md` pattern** (e.g., `01-01-PLAN.md`). Never use `PLAN-01.md`, `plan-01.md`, or any other variation -- workflow-tools detection depends on this exact pattern.
