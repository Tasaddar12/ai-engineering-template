...
This reference documents all artifact types in the Workflow planning taxonomy. Each type has a defined
...
- **Location**: `~/.claude/workflow-core/USER-PROFILE.md`
...
### Spike README.md (per-spike) / MANIFEST.md (per-project index, via /workflow:spike)
...
- **Lifecycle**: Created by `/workflow:spike` → Verified → Wrapped up by `/workflow-spike-wrap-up`
- **Location**: `.planning/spikes/NNN-name/README.md`, `.planning/spikes/MANIFEST.md`
- **Consumed by**: `/workflow-spike-wrap-up` for curation (Requirements pulled only from the idea key(s) it wraps); `pause-work` for spike context handoff
...
- **Lifecycle**: Created by `/workflow:sketch` → Evaluated → Wrapped up by `/workflow-sketch-wrap-up`
- **Location**: `.planning/sketches/NNN-name/README.md`, `.planning/sketches/NNN-name/index.html`, `.planning/sketches/MANIFEST.md`
- **Consumed by**: `/workflow-sketch-wrap-up` for curation; `pause-work` for sketch context handoff

### WRAP-UP-SUMMARY.md (per wrap-up session)
- **Shape**: Curation results, included/excluded items, feature/design area groupings
- **Lifecycle**: Created by `/workflow-spike-wrap-up` or `/workflow-sketch-wrap-up`
