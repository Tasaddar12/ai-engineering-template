...
```bash
# workflow:guard=supplied-root-pin (#4254) — run before the first Edit/Write and before every commit.
...
case "$PINNED_ROOT" in
  ''|'{PINNED_ROOT}') PIN_STAGE=pin-unbound; workflow_pin_fail ;;  # empty or unexpanded pin — fail closed, never warn-and-proceed
  /*) ;;                                                     # absolute POSIX form
  [A-Za-z]:/*|[A-Za-z]:"$BS"*) ;;                            # Windows drive form, forward- or backslash-separated
  *) PIN_STAGE=form-gate; workflow_pin_fail ;;                    # relative pin — never trustworthy across cwds
esac
...
`.ai/library/references/worktree-branch-check.md`, which the orchestrator embeds directly
