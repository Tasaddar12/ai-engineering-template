...
Model profiles control which Claude model each Workflow agent uses. This allows balancing quality vs token spend, or inheriting the currently selected session model.
...
| `discuss` | `assumptions-analyzer` |
| `research` | phase-researcher, project-researcher, research-synthesizer, codebase-mapper, ui-researcher |
| `execution` | executor, debugger, doc-writer |
| `verification` | verifier, plan-checker, integration-checker, nyquist-auditor, ui-checker, ui-auditor, doc-verifier, code-reviewer |
...
- **`model_overrides`** is per-agent precision (e.g. force `haiku` on `codebase-mapper` for a fan-out).
...
- **Required when using non-Anthropic providers** (OpenRouter, local models, etc.) — otherwise Workflow may call Anthropic models directly, incurring unexpected costs
- Use when: you want Workflow to follow your currently selected runtime model

## Using Non-Claude Runtimes (Codex, OpenCode, Gemini CLI, Kilo)

When installed for a non-Claude runtime, the Workflow installer sets `resolve_model_ids: "omit"` in `~/.workflow/defaults.json`. This returns an empty model parameter for all agents, so each agent uses the runtime's default model. No manual setup is needed.
...
If you're using Claude Code with OpenRouter, a local model, or any non-Anthropic provider, set the `inherit` profile to prevent Workflow from calling Anthropic models for subagents:
...
Without `inherit`, Workflow's default `balanced` profile spawns specific Anthropic models (`opus`, `sonnet`, `haiku`) for each agent type, which can result in additional API costs through your non-Anthropic provider.

## Advisor Tool (Claude Code)

Claude Code (v2.1.98+) can pair the session's executor model with a stronger **advisor** model that it consults mid-generation for strategy and course-correction (Anthropic's [advisor tool](https://platform.claude.com/docs/en/agents-and-tools/tool-use/advisor-tool)). This is a host-runtime feature, not a Workflow setting — Workflow selects each agent's *executor* model through the profile/tier system above; Claude Code supplies the advisor.

Set it once at the session level with `/advisor <model>` (or the `advisorModel` setting / `--advisor` flag). **Subagents inherit the session advisor automatically**, so every Workflow subagent an orchestrator spawns gets the same advisor with no per-agent configuration. It composes cleanly with Workflow's tiering: the profile keeps executors cheap where the work is mechanical, and the advisor adds a stronger reviewer inline on the turns that benefit.

### Candidate pairings

Per Anthropic's advisor-tool docs the advisor must be at least as capable as the executor. Candidate pairings by profile — evaluate on your own workload; the quality/cost characterizations below are Anthropic-reported, not Workflow guarantees:
...
The advisor is a single session-wide setting inherited by all subagents; there is **no per-agent advisor selection**, so Workflow cannot vary the advisor by role the way it varies the executor model (e.g. "no advisor on the Haiku mapper, a Fable 5 advisor on the Sonnet executor"). Per-agent advisor control is tracked upstream at [anthropics/claude-code#73072](https://github.com/anthropics/claude-code/issues/73072); until it lands, pick one session advisor that fits the most valuable agents in your run.
...
|---|---|---|
| `light` | codebase-mapper, pattern-mapper, research-synthesizer, plan-checker, integration-checker, nyquist-auditor, ui-checker, ui-auditor, doc-verifier | Cheap/fast — pure mappers, scanners, low-stakes audits |
| `standard` | executor, phase-researcher, project-researcher, verifier, doc-writer, ui-researcher | Default workhorse — research, writing, primary verification |
| `heavy` | planner, roadmapper, debugger | Deep reasoning — already at top, can't escalate further |
...
`~/.claude/agents/workflow-*.md` — Claude Code's Agent tool has no per-spawn
effort parameter, so per-agent frontmatter is the only channel. An effort
config change has no effect until `workflow_run effort sync --apply`
re-syncs the agent files. Codex agents instead pin
`model_reasoning_effort` in `.ai/library/agents/*.toml` at install time.
...
Overrides take precedence over the profile. Valid values: `opus`, `sonnet`, `haiku`, `fable`, `inherit`, or any fully-qualified model ID (e.g., `"o3"`, `"openai/o3"`, `"google/gemini-2.5-pro"`). `fable` is a Claude Code Agent-tool alias, not a Workflow profile tier — it has no column in the profile table above.
...
Runtime: `/workflow-set-profile <profile>`
...
Claude Code's `"opus"` alias maps to a specific model version. Organizations may block older opus versions while allowing newer ones. Workflow returns `"inherit"` for opus-tier agents, causing them to use whatever opus version the user has configured in their session. This avoids version conflicts and silent fallbacks to Sonnet.

**Why `inherit` profile?**
Some runtimes (including OpenCode) let users switch models at runtime (`/model`). The `inherit` profile keeps all Workflow subagents aligned to that live selection.
