...
Generic reference for consuming the `--raw` JSON output of `workflow_run loop render-hooks <point>`
...
- `ref.skill` present → dispatch via the Skill tool with skill id `workflow-<ref.skill>`.
...
- `ref.command` present → **validate it IN-CONTEXT first, before any shell use.** It comes
  ...
  ```bash
  workflow_run ${ref.command} --phase "${PHASE_NUMBER}" --raw
  ```
...
reviewer-lane dispatch by calling `workflow_run review-lane dispatch-step --cap-id <capId> --point
...
`contributions` / `gates`, and is never dispatched through `workflow_run loop render-hooks <point>` or
...
[Capability manifest reference](../SOURCES.md#source-5e5658f3386a2afc). `execute-plan.md`'s
per-task loop calls `workflow_run task resolve-content --plan <path> --task-id <tracker-id> --raw`
...
[ADR-3646](../SOURCES.md#source-aa19db77dda68830) for the full rationale,
