...
<purpose>
Interactive configuration of Workflow power-user knobs — plan bounce, node repair, subagent timeouts,
...
This is a companion to `/workflow:settings` — the common-case prompt there covers model profile,
...
All subsequent reads and writes go through `$Workflow_CONFIG_PATH`. Never hardcode
...
- `git.phase_branch_template` (default: `workflow/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `workflow/{milestone}-{slug}`)
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or workflow/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or workflow/{milestone}-{slug}>)",
...
This section lets the user inspect and override the built-in model IDs Workflow resolves for each
...
`workflow_run query config-set runtime <value>` before proceeding to Step C.
...
```bash
workflow_run query config-set model_profile_overrides.<runtime>.<tier> "<model-id>"
```

For each tier where the user chose "Clear override", remove the key by setting it to null:
```bash
workflow_run query config-set model_profile_overrides.<runtime>.<tier> null
```
...
Merge the new settings into the existing config at `$Workflow_CONFIG_PATH`. This merge is the
core correctness invariant: **preserve every unrelated key** — do not clobber siblings.

Apply each selected value via `workflow_run query config-set <key> <value>` so the central
...
workflow_run query config-set workflow.plan_bounce_passes 5
workflow_run query config-set workflow.subagent_timeout 300000
workflow_run query config-set git.base_branch main
workflow_run query config-set context_window 1000000
# Runtime model tier examples:
workflow_run query config-set runtime gemini
workflow_run query config-set model_profile_overrides.gemini.opus gemini-3-ultra
workflow_run query config-set model_profile_overrides.gemini.haiku null
```
...
route each write through `workflow_run query config-set` so sibling preservation is handled by
the central setter.
</step>
...
defines which AI models Workflow uses at each cost tier (low / medium / high), independently
of the `runtime` and `model_profile` selections above. Two paths are offered:

- **Known provider:** choose a provider and a budget level; Workflow materializes the canonical
...
```bash
cat "$Workflow_CONFIG_PATH" | python3 -c "import sys,json; c=json.load(sys.stdin); mp=c.get('model_policy',{}); print(json.dumps(mp,indent=2))" 2>/dev/null || echo "{}"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Known provider", description: "Choose a provider (Claude / OpenAI / Gemini / Qwen) and a budget level — Workflow writes the canonical tier mapping automatically." },
...
workflow_run query config-set model_policy.provider "<provider>"   # e.g., anthropic / anthropic-fable / openai / google / qwen
workflow_run query config-set model_policy.budget   "<budget>"    # high / medium / low
workflow_run query config-set model_policy.high     "<high-id>"
workflow_run query config-set model_policy.medium   "<medium-id>"
workflow_run query config-set model_policy.low      "<low-id>"

# Generic-provider path — write only tiers the user changed ("Keep current" skipped):
workflow_run query config-set model_policy.provider "custom"
workflow_run query config-set model_policy.budget   null
# Per-tier writes for each non-"Keep current" answer:
workflow_run query config-set model_policy.high     "<high-id>"   # omit if user chose "Keep current"
workflow_run query config-set model_policy.medium   "<medium-id>" # omit if user chose "Keep current"
workflow_run query config-set model_policy.low      "<low-id>"    # omit if user chose "Keep current"
```
...
```text
### Workflow ► ADVANCED SETTINGS UPDATED
...
These settings apply to future /workflow:plan-phase, /workflow:execute-phase, /workflow:discuss-phase,
and /workflow:ship runs.

For common-case toggles (model profile, research/plan_check/verifier, branching strategy,
UI/AI phase gates), use /workflow:settings.
```
</step>
...
<success_criteria>
- [ ] Current config read from resolved `$Workflow_CONFIG_PATH`
...
- [ ] Writes routed through `workflow_run query config-set` so unrelated keys are preserved
