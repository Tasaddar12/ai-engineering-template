...
- mempalace-curator — Ship-time MemPalace curation (diary, KG mirror, cross-project tunnels, wing-scoped prune); dispatched at ship:post when the mempalace capability is enabled.
</available_agent_types>
...
```bash
CONFIG=$(workflow_run query state.load)
```
...
```bash
BASE_BRANCH=$(workflow_run query git.base-branch)
```
</step>
...
1. **Verification passed?**
   ...
   STATUS=$(workflow_run query verification.status "${PHASE_DIR}" --pick status 2>/dev/null)
   ```
   ...
   ```bash
   NEXT_ACTION=$(workflow_run query verification.status "${PHASE_DIR}" --pick next_action 2>/dev/null)
   NEXT_COMMAND=$(workflow_run query verification.status "${PHASE_DIR}" --pick next_command 2>/dev/null)
   ```
...
3. **On correct branch?**
   ...
   IS_PROTECTED=$(workflow_run query git.base-branch --is-protected "$CURRENT_BRANCH") || IS_PROTECTED=""
...
6. **Capability ship gates (generic dispatch).**
   ...
   ```bash
   SHIP_PRE_HOOKS_JSON=$(workflow_run loop render-hooks ship:pre --raw)
   ...
   **For each active entry where `kind == "gate"`** (process in array order), following `.ai/library/references/loop-hook-dispatch.md`. Entries of any other `kind` are not gates and are not enforced here. Every gate is visited exactly once by this loop — the named branches below are specializations *within* it, never a separate pass, so no gate is evaluated twice.
   ...
   **Step 1 — evaluate the gate's `check`.** Dispatch by check shape; read the hook's `check` object in-context to pick the branch (the registry validates exactly one of `query`/`predicate`/`agentVerdict`). Two capability IDs carry a bespoke evaluation whose fail-closed semantics the declared predicate alone does not reproduce — take their branch, then rejoin at step 2:
   ...
   - **`capId == "security"`** — enforce against `SECURITY_FILE`:
     - **`SECURITY_FILE` is empty** → `block: true`, `SECURITY_SHIP_GATE_NO_REVIEW`:
       ...
       Run /workflow:secure-phase {phase} and resolve findings before shipping.
       ```
     - **`SECURITY_FILE` exists** → read its frontmatter `threats_open`. The gate passes **only** when `threats_open` is exactly `0`. For any other value — `threats_open` > 0, or a missing / non-numeric / unparsable field — **fail closed** with `block: true` and `SECURITY_SHIP_GATE_OPEN_THREATS` (the predicate is strict equality to `0`; never ship on an ambiguous value):
       ...
       Resolve open threats (or re-run /workflow:secure-phase {phase}) before shipping.
       ```
   ...
   - **`capId == "broken-windows"`** (issue #1950) — enforce against the ledger's typed status. The ledger lives at the **project root** (cross-phase, not phase-scoped):
     ...
     ```bash
     WINDOWS_STATUS_JSON=$(workflow_run windows status --raw 2>/dev/null || echo '')
     ...
     - **`WINDOWS_OPEN_COUNT` is a positive integer** → `block: true`, `WINDOWS_SHIP_GATE_OPEN`:
       ...
       Resolve each entry before shipping, or explicitly waive with a recorded reason:
         workflow_run windows fixed <id>      # defect resolved
         workflow_run windows waive <id> "<reason>"   # justified deferral (reason required)
       Then re-run /workflow:ship.
       ```
     - **`WINDOWS_OPEN_COUNT` is `"?"`, empty, or non-numeric** → **fail closed** with `block: true` and `WINDOWS_SHIP_GATE_READ_FAILED` (the gate is strict equality to `0`; never ship on an unreadable ledger):
       ...
       Inspect the file or run `workflow_run windows status --raw` to diagnose. The ledger
       may be malformed; fix it before shipping (an unparseable ledger is a broken window).
       ```

     The ledger is **optional and backward-compatible**: on a project where `workflow_run windows status` returns `open_count: 0` (no `.planning/WINDOWS.md` yet, or an empty ledger), the gate passes silently. It only blocks when at least one entry is `open`.
   ...
   - **Every other `capId`** — run the gate's own declared check through the generic evaluator. This arm is what makes a third-party capability's declared gate enforceable at all (#3559); before it existed, a gate whose `capId` was not named above was resolved and then silently dropped.
     ...
     ```bash
     GATE_RESULT=$(workflow_run check ${hook.check.query} "${PHASE_DIR}" --raw)
     CHECK_EXIT=$?
     ```
     ...
     ```bash
     GATE_RESULT=$(workflow_run check predicate --predicate '<hook.check.predicate as JSON>' --phase-dir "${PHASE_DIR}" --phase-number "${PHASE_NUMBER}" --raw)
     CHECK_EXIT=$?
     ```
...
```bash
CUSTOM_PR_SECTIONS=$(workflow_run query config-get ship.pr_body_sections --default '[]' 2>/dev/null || echo '[]')
```
...
Use these sections for lean/agile PRD material that should travel with the PR without making the core `/workflow:ship` body configurable:
...
PR_BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/workflow-pr-body-XXXXXX") && mv "$PR_BODY_FILE" "${PR_BODY_FILE}.md" && PR_BODY_FILE="${PR_BODY_FILE}.md" || exit 1
...
```bash
REVIEW_CMD=$(workflow_run query config-get workflow.code_review_command --raw 2>/dev/null || echo "")
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(workflow_run query state.load 2>/dev/null | head -20)
   ```
...
3. **Build review prompt and pipe to command via stdin:**
   ...
   REVIEW_STDERR_FILE=$(mktemp "${TMPDIR:-/tmp}/workflow-review-stderr-XXXXXX")
   REVIEW_OUTPUT=$(echo "${REVIEW_PROMPT}" | workflow_run run-with-timeout 120 -- ${REVIEW_CMD} 2>"${REVIEW_STDERR_FILE}")
   REVIEW_EXIT=$?
   ```
...
```bash
workflow_run query state.update "Last Activity" "$(date +%Y-%m-%d)"
workflow_run query state.update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```
...
```bash
workflow_run query commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER} [ci skip]" --files .planning/STATE.md
...
if [ "$STATUS" = "BLOCKED" ] && [ "$CHECKS" = "0" ] && [ "$REVIEW_DECISION" != "REVIEW_REQUIRED" ] && [ "$REVIEW_DECISION" != "CHANGES_REQUESTED" ] && git log -1 --format=%B "$SHIP_NOTE_SHA" | grep -q '\[ci skip\]'; then
  ...
  # workflow_run query commit requires a file list; use git directly for this intentionally empty commit.
...
```bash
SHIP_POST_HOOKS_JSON=$(workflow_run loop render-hooks ship:post --raw)
```
...
- If `ref.agent` is set, first show the spawn banner, then dispatch the agent named by `ref.agent` (use the exact `ref.agent` value as the subagent type — e.g. `mempalace-curator` — never `general-purpose`):
...
> **Runtime-aware dispatch (#2508 Phase 4).** Workflow workflows dispatch specialized subagents by role. Before dispatching on a built-in-only runtime (kimi-code — three built-ins only), resolve the role to a built-in via `workflow_run query resolve-dispatch-type --requested <role> --raw`. On named-dispatch runtimes (Claude/OpenCode/…) the role is returned unchanged; on kimi-code it maps to `coder`/`explore`/`plan` by role-suffix. The persona rides `${AGENT_SKILLS_<ROLE>}` (Phase 3) regardless. See @.ai/library/references/runtime-aware-dispatch.md.
  ...
  **Input validation (defense-in-depth) — do this IN-CONTEXT, before any shell use.** `ref.agent` originates in a capability manifest, which may be third-party. Check the value you read from `activeHooks` against `^[A-Za-z0-9][A-Za-z0-9._-]*$` yourself, the same way you read `activeHooks` itself — **never** by pasting it into a shell command to be tested there. A value carrying a quote, `;`, `` ` ``, `$(`, or a newline would terminate the assignment and run as its own statement *before* any shell-side check could execute, so a shell-side check is no protection at all.
  ...
  ```bash
  HOOK_AGENT_MODEL=$(workflow_run query resolve-model "<agent>" --raw 2>/dev/null || true)
  ```
...
- If `ref.skill` is set, dispatch with `Skill(skill="workflow-${ref.skill}", args="${PHASE_NUMBER} --auto ${Workflow_WS}")` (prepend `workflow-` to `ref.skill`).
...
- /workflow:complete-milestone (if last phase in milestone)
- /workflow:progress (to see what's next)
...
- /workflow:complete-milestone — if all phases in milestone are done
- /workflow:progress — see overall project state
- /workflow:execute-phase {next} — continue to next phase
</offer_next>
