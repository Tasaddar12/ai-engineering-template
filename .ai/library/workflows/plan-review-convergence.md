...
workflow-plan-phase N → workflow-review N --codex → workflow-plan-phase N --reviews → workflow-review N --codex → ...
Plan-phase runs inline (bare Skill at depth 0) so it can spawn planner/plan-checker at depth 1.
...
Workflow_WS=""
echo "$ARGUMENTS" | grep -qE '\-\-ws\s+\S+' && Workflow_WS=$(echo "$ARGUMENTS" | grep -oE '\-\-ws\s+\S+')
```
...
CONVERGENCE_ENABLED=$(workflow_run query config-get workflow.plan_review_convergence --raw 2>/dev/null || echo "false")
```

**If `CONVERGENCE_ENABLED` is not `"true"`:** Display and exit:

```text
workflow-plan-review-convergence is disabled (workflow.plan_review_convergence=false).

This feature automates the plan→review→replan loop using external AI reviewers.
Enable it with:

  workflow config-set workflow.plan_review_convergence true

Then re-run: /workflow:plan-review-convergence {PHASE}
```
...
# This block must stay AFTER the launcher preamble (below) because it calls `workflow_run` —
# do not move it back above the preamble in a future edit.
REVIEWER_FLAGS=""
for REVIEW_FLAG in $(workflow_run review-lane flags) --all; do
...
# (and, transitively, review.reviewer_instances). workflow-review sees the injected
...
# so workflow-review applies review.default_reviewers itself (rule 3). Only fall back
...
if [ -z "$REVIEWER_FLAGS" ]; then
  DEFAULT_REVIEWERS_JSON=$(workflow_run query config-get review.default_reviewers 2>/dev/null || echo "")
  if ! command -v jq >/dev/null 2>&1; then
    ...
    if [ "${DEFAULT_REVIEWERS_COUNT:-0}" -gt 0 ] 2>/dev/null; then
      : # leave REVIEWER_FLAGS empty — workflow-review applies review.default_reviewers itself
...
```bash
INIT=$(workflow_run init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(workflow_run roadmap get-phase "${PHASE}")
```
...
```text
### Workflow ► PLAN CONVERGENCE — Phase {phase_number}
...
```bash
PLAN_COUNT=$(workflow_run query find-phase "${PHASE}" | jq -r '.plan_count_all // 0')
```
...
Agent(
  ...
  prompt="Run /workflow:review for Phase {PHASE}.

Execute: Skill(skill='workflow-review', args='--phase {PHASE} {REVIEWER_FLAGS} {Workflow_WS}')
...
Definitions:
  ...
  ACTIONABLE — a non-HIGH review finding that would be invisible to /workflow:execute-phase unless it is incorporated into PLAN.md or explicitly deferred/rejected in PLAN.md.
...
4a. **Resolve effective authority** (deterministic — replaces manual `intel.enabled` reasoning):
   ```bash
   EFFECTIVE_AUTHORITY=$(workflow_run drift-guard authority --raw)
   ```
4. **Severity & gating** — classify each symbol's verdict using the seam (do not apply the table manually):
   ```bash
   # For each symbol, e.g.:
   RESULT=$(workflow_run drift-guard severity --status <verdict> --authority "$EFFECTIVE_AUTHORITY")
...
1. **Phase status — decided by the seam, not by judgment.** Do not eyeball this axis:
   ...
   ```bash
   DRIFT=$(workflow_run drift-guard phase-status --phase "${PHASE}")
...
**Report once, not twice — these belong to `plan-checker`:** a PLAN that omits a roadmap Success Criterion is scope reduction (Dimension 7b); a requirement ID the ROADMAP never defines is requirement coverage (Dimension 1); two PLAN.md files in one phase disagreeing is cross-plan data contracts (Dimension 9).
...
decision, capability guidance, or an existing plan constraint, recorded by `/workflow:plan-phase`
...
if OPEN_CONFLICTS=$(awk '
  ...
  saw_title && !in_owned && !done {
    if ($0 == "") next
    if ($0 == "<!-- workflow:plan-revision-conflicts:begin -->") { in_owned = 1; next }
    exit 2
  }
  in_owned && $0 == "<!-- workflow:plan-revision-conflicts:begin -->" { exit 2 }
  in_owned && !saw_heading && $0 == "" { next }
  in_owned && !saw_heading && $0 == "## Plan-Revision Conflicts" { saw_heading = 1; next }
  in_owned && !saw_heading { exit 2 }
  in_owned && $0 == "<!-- workflow:plan-revision-conflicts:end -->" {
    done = 1
    in_owned = 0
    print count
    exit
...
`/workflow:review` emits exactly one writer-owned slot immediately after the artifact title,
between `<!-- workflow:plan-revision-conflicts:begin -->` and
`<!-- workflow:plan-revision-conflicts:end -->`. Inside that slot, `/workflow:plan-phase` records each
...
**Only `/workflow:plan-phase` mutates the contents of this slot.** The review agent preserves the
...
```bash
workflow_run state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```

Display:
```text
### Workflow ► CONVERGENCE COMPLETE ✓
 ...
 Next: /workflow:execute-phase {PHASE}
```
...
To replan manually:  /workflow:plan-phase {PHASE} --reviews
To restart loop:     /workflow:plan-review-convergence {PHASE} {REVIEWER_FLAGS}
```
Exit workflow.
...
- [ ] Initial planning via inline Skill("workflow-plan-phase") if no plans exist — NOT wrapped in Agent() (bug #936: depth-1 Agent has no Agent tool)
- [ ] Review via Agent → Skill("workflow-review") — isolated Agent is correct; workflow-review is a Bash leaf with no sub-agent spawns; {Workflow_WS} forwarded
- [ ] Replan via inline Skill("workflow-plan-phase --reviews") — NOT wrapped in Agent(); inline lets plan-phase spawn planner/plan-checker at depth 1
...
- [ ] The review Agent fully completes workflow-review before returning (plan-phase runs inline — no Agent wrap)
