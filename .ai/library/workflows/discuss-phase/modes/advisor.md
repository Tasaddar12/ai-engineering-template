...
> **Lazy-loaded and gated.** The parent `.ai/library/workflows/discuss-phase.md` Reads
> this file ONLY when `ADVISOR_MODE` is true (i.e., when
> `$HOME/.claude/workflow-core/USER-PROFILE.md` exists). Skip the Read
...
ADVISOR_MODEL=$(workflow_run query resolve-model advisor-researcher --raw)
```
...
```bash
PROFILE_CONTENT=$(cat "$HOME/.claude/workflow-core/USER-PROFILE.md" 2>/dev/null || true)
```
