...
AGENT_SKILLS_CHECKER=$(workflow_run query agent-skills integration-checker)
```
...
```bash
integration_checker_model=$(workflow_run query resolve-model integration-checker --raw)
```
...
workflow_run query phases.list
```
...
PHASE_INFO=$(workflow_run query find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  workflow_run query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
Discovery only — never auto-calls `/workflow:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/workflow:validate-phase {N}` |

Phases needing validation: run `/workflow:validate-phase {N}` for each flagged phase.
...
/workflow:phase --insert <N> "Close gap: <REQ-ID> — <description>"
...
`/workflow:validate-phase <N>` for each flagged phase (and `/workflow:secure-phase
...
- /workflow:complete-milestone {version} — proceed anyway (accept tech debt)
...
/workflow:phase --insert <N> "Address tech debt: <area>"
/workflow:discuss-phase <N>
/workflow:plan-phase <N>
/workflow:execute-phase <N>
