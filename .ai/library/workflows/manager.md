...
INIT=$(workflow_run query init.manager)
...
- `manager_flags.discuss` — appended to `/workflow:discuss-phase` args (e.g. `"--auto --analyze"`)
- `manager_flags.plan` — appended to plan agent init command
- `manager_flags.execute` — appended to execute agent init command

These are empty strings by default. Set via: `workflow_run query config-set manager.flags.discuss "--auto --analyze"`
...
All {phase_count} phases verified complete. Ready for final steps:
  → /workflow:verify-work — run acceptance testing
  → /workflow:complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="workflow-verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="workflow-complete-milestone")` then exit.
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in Workflow config (default: 60 seconds, set to 0 to disable).
...
When the user selects a compound option, behavior depends on whether the runtime supports background dispatch of nesting-capable orchestrators — the Plan Phase N / Execute Phase N handlers below resolve it via `workflow_run query dispatch-should-flatten` (#1708):
...
- If `command` contains `execute-phase`, run `Skill(skill="workflow-execute-phase", args="{PHASE_NUM} {manager_flags.execute}")`.
- If `command` contains `verify-work`, run `Skill(skill="workflow-verify-work", args="{PHASE_NUM}")`.
- If `command` is missing or unrecognized, stop and show the recommendation row instead of guessing.

Inline discuss:

```
Skill(skill="workflow-discuss-phase", args="{PHASE_NUM} {manager_flags.discuss}")
```
...
Agent(
  ...
  prompt="You are running the Workflow plan-phase workflow for phase {N} of the project.
...
Skill(skill=\"workflow-plan-phase\", args=\"{N} --auto {manager_flags.plan}\")
...
Agent(
  ...
  run_in_background=true,
  prompt="You are running the Workflow execute-phase workflow for phase {N} of the project.
...
Skill(skill=\"workflow-execute-phase\", args=\"{N} {manager_flags.execute}\")
...
- Display the error clearly, then offer to fix it:
  ...
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill — use `Skill(skill="workflow-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="workflow-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill — use `Skill(skill="workflow-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="workflow-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
...
### Workflow ► SESSION END
 ...
 Resume anytime: /workflow:manager

---
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/workflow:manager` or `/workflow:progress` invocation.
