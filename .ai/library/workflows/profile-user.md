...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `workflow_run query` handlers and the user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @.ai/library/references/ui-brand.md (display patterns)
- @.ai/library/agents/user-profiler.md (profiler agent definition)
- @.ai/library/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/workflow-core/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/workflow-core/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
```
### Workflow > PROFILE YOUR CODING STYLE
...
✓ Stores profile at $HOME/.claude/workflow-core/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /workflow:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(workflow_run query scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(workflow_run query profile-sample --json 2>/dev/null)
```
...
```bash
PROFILER_MODEL=$(workflow_run query resolve-model user-profiler --raw)
```
...
**Spawn user-profiler agent using Task tool:**

Use the Task tool to spawn the `user-profiler` agent, passing `model="{PROFILER_MODEL}"` (omit the parameter per the rule above when the value is `"inherit"` or empty). Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `.ai/library/references/user-profiling.md`
...
Reference: @.ai/library/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(workflow_run query profile-questionnaire --json 2>/dev/null)
```
...
ANSWERS_PATH=$(mktemp "${TMPDIR:-/tmp}/workflow-profile-answers-XXXXXX") && mv "$ANSWERS_PATH" "${ANSWERS_PATH}.json" && ANSWERS_PATH="${ANSWERS_PATH}.json" || exit 1
```
...
```bash
ANALYSIS_RESULT=$(workflow_run query profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
# BSD/macOS mktemp only randomizes XXXXXX when it is the final path component, so make a
# suffixless temp then append the extension — portable across BSD + GNU (#1520).
ANALYSIS_PATH=$(mktemp "${TMPDIR:-/tmp}/workflow-profile-analysis-XXXXXX") && mv "$ANALYSIS_PATH" "${ANALYSIS_PATH}.json" && ANALYSIS_PATH="${ANALYSIS_PATH}.json" || exit 1
```
...
```bash
workflow_run query write-profile --input "$ANALYSIS_PATH" --json
```

Display: "✓ Profile written to $HOME/.claude/workflow-core/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/workflow-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/workflow-core/USER-PROFILE.md" and jump to step 10.
...
**For /workflow-dev-preferences (if selected):**

```bash
workflow_run query generate-dev-preferences --analysis "$ANALYSIS_PATH" --json
```

Display: "✓ Generated /workflow-dev-preferences at $HOME/.claude/skills/workflow-dev-preferences/SKILL.md"

**For CLAUDE.md profile section (if selected):**

```bash
workflow_run query generate-claude-profile --analysis "$ANALYSIS_PATH" --json
```
...
```bash
workflow_run query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any `workflow_run query` call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```
### Workflow > PROFILE COMPLETE ✓

Your profile:    $HOME/.claude/workflow-core/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /workflow-dev-preferences   $HOME/.claude/skills/workflow-dev-preferences/SKILL.md
...
- [ ] Artifacts generated sequentially via `workflow_run query` subcommands
