...
@.ai/library/references/git-integration.md
</required_reading>
...
INIT=$(workflow_run query init.execute-phase "${PHASE}")
...
# config settings can be fetched via workflow_run query config-get if needed
```
...
INLINE_THRESHOLD=$(workflow_run query config-get workflow.inline_plan_threshold --raw 2>/dev/null || echo "2")
...
HUMAN_VERIFY_MODE=$(workflow_run query config-get workflow.human_verify_mode --default end-of-phase --raw 2>/dev/null || echo "end-of-phase")
...
- **Pattern A:** read @.ai/library/references/dispatch-isolation-gate.md and run its
...
- **Pattern B: segments are NOT isolated, and must record that before dispatching.** Each
  ...
  ```bash
  ISOLATION=none
  workflow_run query dispatch-isolation --raw --force-isolation none >/dev/null 2>&1 || true
  ```
...
```bash
workflow_run query phases.list --type summaries --raw
...
Apply deviation rules from the executor agent definition (single source of truth):
...
exactly as specified in the canonical `.ai/library/references/tdd.md` reference — the
...
See .ai/library/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using the template at `.ai/templates/user-setup.md` (or its `.ai/templates/user-setup.compact.md` variant — resolve per `.ai/library/references/compact-content-gate.md` §"Streams 1b and 4"). Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use the template at `.ai/templates/summary.md` (or `summary.compact.md` — same `compact-content-gate.md` resolution as the USER-SETUP template above).
...
Update STATE.md using workflow_run query (or legacy workflow-tools) state mutations:
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  workflow_run query state.advance-plan

  # Recalculate progress bar from disk state
  workflow_run query state.update-progress

  # Record execution metrics
  workflow_run query state.record-metric \
...
workflow_run query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
workflow_run query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using workflow_run query (or legacy workflow-tools):
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  workflow_run query roadmap.update-plan-progress "${PHASE}"
fi
```
...
```bash
READY=$(workflow_run query requirements.ready-ids "${PLAN_PATH}" ${REQ_IDS} --raw)
READY_IDS=$(printf '%s' "$READY" | jq -r '.ready[]' 2>/dev/null | tr '\n' ' ')
if [ -n "$(printf '%s' "$READY_IDS" | tr -d '[:space:]')" ]; then
  workflow_run query requirements.mark-complete ${READY_IDS}
fi
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  workflow_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  workflow_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
...
```bash
PHASE_COUNTS=$(workflow_run query find-phase "${PHASE}")
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY — skip any plan whose `plan_id` matches a non-terminal async-job manifest (`external_job_waiting`; see `identify_plan`). Yolo: auto-continue. Interactive: show next plan, suggest `/workflow:execute-phase {phase}` + `/workflow:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/workflow:plan-phase {Z+1}` + `/workflow:verify-work {Z}` + `/workflow:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/workflow:complete-milestone` + `/workflow:verify-work` + `/workflow-add-phase` |
