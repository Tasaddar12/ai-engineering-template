...
OLLAMA_HOST=$(workflow_run query config-get review.ollama_host --raw 2>/dev/null || echo "")
...
LM_STUDIO_HOST=$(workflow_run query config-get review.lm_studio_host --raw 2>/dev/null || echo "")
...
LLAMA_CPP_HOST=$(workflow_run query config-get review.llama_cpp_host --raw 2>/dev/null || echo "")
...
# under "jq-dependent reviewer lanes" below parse structured JSON that workflow-tools
...
- Value must be a non-empty array of slug strings (configured via `workflow config-set review.default_reviewers '["gemini","codex"]'`)
...
<!-- workflow:section id="reviewer-instances-note-1" when="state:reviewer-instances-configured" -->
If `section_manifest` is `null` or `"reviewer-instances-note-1"` is in its `included` list: read and execute `.ai/library/workflows/review/steps/reviewer-instances-note-1.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
Then run /workflow:review again.
```
Exit.
...
```bash
INIT=$(workflow_run query init.review "${PHASE_ARG}")
...
RUN_DIR=$(mktemp -d "${TMPDIR:-/tmp}/workflow-review-XXXXXX")
...
Write to a temp file: `{run_dir}/workflow-review-prompt.md`
...
cp "$INSTRUCTIONS_BLOCK_FILE" "${RUN_DIR}/workflow-review-instructions.md"
cp "$ROADMAP_SECTION_FILE" "${RUN_DIR}/workflow-review-roadmap.md"
...
# or consensus step can resolve. The plan id keeps the workflow-review-plan-*.md glob
# prepare_trimmed_prompt_for_reviewer consumes.)
for PLAN_FILE in "${PHASE_DIR}"/*-PLAN.md; do
  PLAN_BASENAME=$(basename "$PLAN_FILE")
  PLAN_ID="${PLAN_BASENAME%-PLAN.md}"
  cp "$PLAN_FILE" "${RUN_DIR}/workflow-review-plan-${PLAN_ID}.md"
done
...
# (workflow-core#4099). Direct glob iteration (`for f in "${PHASE_DIR}"/*-PLAN.md`,
...
# Named to avoid BOTH existing RUN_DIR globs: `workflow-review-*.md` (reviewer
# reports, invoke_reviewers) and `workflow-review-plan-*.md` (the plan copies just
...
if [ -f ".planning/PROJECT.md" ]; then
  cp .planning/PROJECT.md "${RUN_DIR}/workflow-review-project.md"
fi
_CTX=( "${PHASE_DIR}"/*-CONTEXT.md )
if [ ${#_CTX[@]} -gt 0 ]; then
  cat "${_CTX[@]}" > "${RUN_DIR}/workflow-review-context.md"
fi
_RESEARCH=( "${PHASE_DIR}"/*-RESEARCH.md )
if [ ${#_RESEARCH[@]} -gt 0 ]; then
  cat "${_RESEARCH[@]}" > "${RUN_DIR}/workflow-review-research.md"
fi
if [ -f ".planning/REQUIREMENTS.md" ]; then
  cp .planning/REQUIREMENTS.md "${RUN_DIR}/workflow-review-requirements.md"
fi
...
cat "${RUN_DIR}/.plans-manifest.md" >> "${RUN_DIR}/workflow-review-instructions.md"
cat "${RUN_DIR}/.plans-manifest.md" >> "${RUN_DIR}/workflow-review-prompt.md"
```
...
until #4255 it was resolved by querying `plan-checker`, so every prompt-fed lane ran at that
...
<!-- workflow:section id="reviewer-instances-note-2" when="state:reviewer-instances-configured" -->
If `section_manifest` is `null` or `"reviewer-instances-note-2"` is in its `included` list: read and execute `.ai/library/workflows/review/steps/reviewer-instances-note-2.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
PARALLEL_LANES=$(workflow_run query config-get review.parallel_lanes --raw 2>/dev/null || echo "false")
...
prepare_trimmed_prompt_for_reviewer() {
  ...
  PLAN_FILE_ARGS=""
  for p in "$RUN_DIR"/workflow-review-plan-*.md; do
    [ -f "$p" ] && PLAN_FILE_ARGS="$PLAN_FILE_ARGS --plan-file $p"
  done
  PROJECT_ARG=""
  [ -f "$RUN_DIR/workflow-review-project.md" ] && PROJECT_ARG="--project-file $RUN_DIR/workflow-review-project.md"
  CONTEXT_ARG=""
  [ -f "$RUN_DIR/workflow-review-context.md" ] && CONTEXT_ARG="--context-file $RUN_DIR/workflow-review-context.md"
  RESEARCH_ARG=""
  [ -f "$RUN_DIR/workflow-review-research.md" ] && RESEARCH_ARG="--research-file $RUN_DIR/workflow-review-research.md"
  REQUIREMENTS_ARG=""
  [ -f "$RUN_DIR/workflow-review-requirements.md" ] && REQUIREMENTS_ARG="--requirements-file $RUN_DIR/workflow-review-requirements.md"

  workflow_run query prompt-budget \
    --budget "$REVIEWER_BUDGET" \
    --instructions-file "$RUN_DIR/workflow-review-instructions.md" \
    --roadmap-file "$RUN_DIR/workflow-review-roadmap.md" \
...
workflow_run query review-lane plan \
  --selected "$SELECTED_REVIEWERS" --run-dir "$RUN_DIR" --repo-root "$REPO_ROOT" --json \
  > "$RUN_DIR/workflow-review-lanes.json"
...
run_review_lane() {
  ...
  LANE_BUDGET=$(workflow_run query review-lane plan --selected "$SLUG" --run-dir "$RUN_DIR" \
  ...
  if [ -n "$LANE_BUDGET" ] && [ "$LANE_BUDGET" != "null" ] && [ "$LANE_BUDGET" -gt 0 ] 2>/dev/null; then
    TRIMMED="$RUN_DIR/workflow-review-prompt-$SLUG.md"
    if prepare_trimmed_prompt_for_reviewer "$SLUG" "$LANE_BUDGET" "$TRIMMED" \
         "$RUN_DIR/workflow-review-prompt-$SLUG.metadata.json"; then
      ...
      echo "$SLUG review skipped: prompt budget (${LANE_BUDGET} tokens) too small for the minimum review set." \
        > "$RUN_DIR/workflow-review-$SLUG.md"
  ...
  workflow_run query review-lane invoke --slug "$SLUG" \
    --run-dir "$RUN_DIR" --repo-root "$REPO_ROOT" $PROMPT_ARG $EXPLICIT_FLAG --json \
    > "$RUN_DIR/workflow-review-lane-result-$SLUG.json"
}
...
# `> "$RUN_DIR/workflow-review-lane-result-$SLUG.json"` — the same file, both truncating. The shared-append
...
# iteration under zsh whenever 2+ reviewers were selected (workflow-core#4109).
...
# Rewrapped through unquoted command substitution (workflow-core#4109) — see the
# dispatch loop above for why a bare `$DISPATCH_SLUGS` collapses under zsh.
for SLUG in $(printf '%s' "$DISPATCH_SLUGS"); do
  LANE_RESULT="$RUN_DIR/workflow-review-lane-result-$SLUG.json"
  if [ -f "$LANE_RESULT" ]; then
    cat "$LANE_RESULT" >> "$RUN_DIR/workflow-review-lane-results.jsonl"
  fi
done
```

Each lane leaves `{run_dir}/workflow-review-<slug>.md` — its review, or a diagnostic stub carrying the
...
Display progress:
```
### Workflow ► CROSS-AI REVIEW — Phase {N}
...
```bash
RUN_DIR="{run_dir}"
JSONL="$RUN_DIR/workflow-review-lane-results.jsonl"
LANE_LINES=0
...
if [ "${LANE_LINES:-0}" -eq 0 ]; then
  ...
  for SLUG in $(printf '%s' "$DISPATCH_SLUGS"); do
    ...
    STUB="$RUN_DIR/workflow-review-$SLUG.md"
...
for SLUG in $(printf '%s' "$DISPATCH_SLUGS"); do
  ...
  REVIEW_FILE="$RUN_DIR/workflow-review-$SLUG.md"
  ...
  node -e '
    const fs = require("fs");
    const { escapeRegex } = require("./workflow-core/bin/lib/pattern.cjs");
...
heading and before the end of the first exact `<!-- workflow:plan-revision-conflicts:begin -->` /
`<!-- workflow:plan-revision-conflicts:end -->` pair immediately after the artifact title, if present,
...
`.ai/library/references/reviewer-instances.md`.

**Resolved model (#2295):** each lane's `review-lane invoke --json` line in
`{run_dir}/workflow-review-lane-results.jsonl` carries a `model` object; render `models:` from
...
what closes the remaining `:`/`#`/leading-`-` cases. When Workflow applied a reasoning effort to
...
<!-- workflow:plan-revision-conflicts:begin -->
## Plan-Revision Conflicts
{preserved_plan_revision_conflict_entries}
<!-- workflow:plan-revision-conflicts:end -->
...
<!-- Sections are RENDERED from each lane's declared `reviewsSection`, in descriptor order.
     ...
     `workflow_run query review-lane sections --selected "$SELECTED_REVIEWERS"` emits
     `<slug><TAB><reviewsSection>` in order; for each row, emit:

         ## <reviewsSection> Review

         {contents of {run_dir}/workflow-review-<slug>.md}
...
gate above):
```bash
workflow_run query commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
run's assembled INPUTS there under the same `workflow-review-` prefix: the combined prompt, the
...
# #4097: `workflow-review-*.md` matches BOTH lane outputs (reports, diagnostic stubs) and the
...
# `workflow-review-prompt*` deliberately covers BOTH the combined prompt (`workflow-review-prompt.md`)
# and the per-lane trimmed prompts (`workflow-review-prompt-<slug>.md`).
_DIAG_MD=()
for f in "$RUN_DIR"/workflow-review-*.md; do
  case "$(basename "$f")" in
    workflow-review-prompt*|workflow-review-instructions*|workflow-review-plan-*|workflow-review-project*|workflow-review-roadmap*|workflow-review-context*|workflow-review-research*|workflow-review-requirements*) ;;
    *) _DIAG_MD+=("$f") ;;
  esac
done
_DIAG_ERR=()
for f in "$RUN_DIR"/workflow-review-*.err; do
  [ -s "$f" ] && _DIAG_ERR+=("$f")
done
...
- [ ] User knows how to use feedback (/workflow:plan-phase --reviews)
</success_criteria>
