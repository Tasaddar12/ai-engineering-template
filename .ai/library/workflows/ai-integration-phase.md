<purpose>
Generate an AI design contract (AI-SPEC.md) for phases that involve building AI systems. Orchestrates framework-selector → ai-researcher → domain-researcher → eval-planner with a validation gate. Inserts between discuss-phase and plan-phase in the Workflow lifecycle.
...
```bash
SELECTOR_MODEL=$(workflow_run query resolve-model framework-selector --pick model 2>/dev/null || true)
RESEARCHER_MODEL=$(workflow_run query resolve-model ai-researcher --pick model 2>/dev/null || true)
DOMAIN_MODEL=$(workflow_run query resolve-model domain-researcher --pick model 2>/dev/null || true)
PLANNER_MODEL=$(workflow_run query resolve-model eval-planner --pick model 2>/dev/null || true)
```

Check config:
```bash
AI_PHASE_ENABLED=$(workflow_run query config-get workflow.ai_integration_phase --raw 2>/dev/null || echo "true")
```

**If `AI_PHASE_ENABLED` is `false`:**
```
AI phase is disabled in config. Enable via /workflow:settings.
```
Exit workflow.
...
Recommended: run /workflow:discuss-phase {N} first to capture framework preferences.
...
## 5. Spawn framework-selector

Display:
```
### Workflow ► AI DESIGN CONTRACT — PHASE {N}: {name}

◆ Step 1/4 — Framework Selection...
```

Spawn `framework-selector` with:
```markdown
Read .ai/library/agents/framework-selector.md for instructions.
...
**If selector fails or returns empty:** Exit with error — "Framework selection failed. Re-run /workflow:ai-integration-phase {N} or answer the framework question in /workflow:discuss-phase {N} first."

## 6. Initialize AI-SPEC.md

Copy template:
```bash
cp ".ai/templates/AI-SPEC.md" "${PHASE_DIR}/${PADDED_PHASE}-AI-SPEC.md"
```
...
```markdown
Read .ai/library/agents/ai-researcher.md for instructions.
...
```markdown
Read .ai/library/agents/domain-researcher.md for instructions.
...
Spawn `eval-planner` with:
```markdown
Read .ai/library/agents/eval-planner.md for instructions.
...
```bash
workflow_run query commit "docs({phase_slug}): generate AI-SPEC.md — {primary_framework} + domain context + eval strategy" --files "${AI_SPEC_FILE}"
```

## 12. Display Completion

```
### Workflow ► AI-SPEC COMPLETE — PHASE {N}: {name}
...
Next step:
  /workflow:plan-phase {N}   — planner will consume AI-SPEC.md
```
