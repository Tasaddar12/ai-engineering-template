...
When `PLAN_STRATEGY=converge`, the planning step MUST invoke the plan-review convergence workflow instead of `workflow-plan-phase`. `--cross-ai` is an alias for `--converge`. Forward `CONVERGENCE_ARGS` exactly as parsed so reviewer flags and `--max-cycles N` retain the same meaning as they have on `/workflow:plan-review-convergence`.
...
INIT=$(workflow_run query init.milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
INIT_AUTONOMOUS=$(workflow_run query init.autonomous $CONVERGE_PARAM $CROSS_AI_PARAM)
...
# This block must stay AFTER the launcher preamble (above) because it calls `workflow_run` —
# do not move it back above the preamble in a future edit.
CONVERGENCE_ARGS=""
for REVIEW_FLAG in $(workflow_run review-lane flags) --all --text; do
...
<!-- workflow:section id="converge-fail-fast" when="state:plan-strategy-converge" -->
If `section_manifest` is `null` or `"converge-fail-fast"` is in its `included` list: read and execute `.ai/library/workflows/autonomous/steps/converge-fail-fast.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/workflow:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/workflow:new-milestone` first."
...
<!-- workflow:section id="converge-banner" when="state:plan-strategy-converge" -->
If `section_manifest` is `null` or `"converge-banner"` is in its `included` list: read and execute `.ai/library/workflows/autonomous/steps/converge-banner.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->

**Agent skills (delegated agents self-load):** This workflow delegates plan/execute/review via flat `Skill()` invocations rather than resolving `agent_skills` itself. Each consumer agent (`planner`, `executor`, `plan-checker`, `verifier`, …) self-loads its configured `.planning/config.json` `agent_skills` in its own mandatory init step per `@.ai/library/references/agent-skills-bootstrap.md`. This is the durable path that works on every runtime — including Cursor, where `Skill()`-delegated workflow bash init does not reliably execute. No per-delegation injection is needed here. See .ai/library/SOURCES.md#source-81c113487dc2a1ca.
...
```bash
INIT_MANAGER=$(workflow_run query init.manager)
...
STATE_PATH=$(_workflow_field "$INIT_MANAGER" state_path)
...
- `verification_deferred_human` -> `/workflow:verify-work <phase>`
- `verification_deferred_gaps` -> `/workflow:plan-phase <phase> --gaps`
...
```bash
DETAIL=$(workflow_run query roadmap.get-phase ${PHASE_NUM})
```
...
```
### Workflow ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
```
...
```bash
PHASE_STATE=$(workflow_run query init.phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(workflow_run query config-get workflow.skip_discuss --raw 2>/dev/null || echo "false")
```
...
```bash
workflow_run query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="workflow-discuss-phase", args="${PHASE_NUM}")
```
...
> Full instructions are in `.ai/library/references/autonomous-ui-design-contract.md`. Read that file now and follow it exactly.
...
Read and execute: `.ai/library/references/autonomous-ui-design-contract.md`
...
```bash
FLATTEN=$(workflow_run query dispatch-should-flatten --raw 2>/dev/null || echo "true")
```
...
- **If `FLATTEN` is `false`:** Dispatch plan as a background agent to keep the main context lean. While plan runs, the workflow can immediately start discussing the next phase (see step 4).
  ...
  <!-- workflow:section id="converge-dispatch-bg" when="state:plan-strategy-converge" -->
  If `section_manifest` is `null` or `"converge-dispatch-bg"` is in its `included` list: read and execute `.ai/library/workflows/autonomous/steps/converge-dispatch-bg.md`. Otherwise skip — do not read the file.
  <!-- /workflow:section -->
  ...
  Agent(
    ...
    prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"workflow-plan-phase\", args=\"${PHASE_NUM}\")"
  )
  ```
...
- **Otherwise (`FLATTEN` is `true` — run inline):** Run plan **inline** (do NOT background) so the plan-checker runs. The next phase's discuss does not overlap planning here — correctness over overlap.
  ...
  <!-- workflow:section id="converge-dispatch-inline" when="state:plan-strategy-converge" -->
  If `section_manifest` is `null` or `"converge-dispatch-inline"` is in its `included` list: read and execute `.ai/library/workflows/autonomous/steps/converge-dispatch-inline.md`. Otherwise skip — do not read the file.
  <!-- /workflow:section -->

  - Otherwise (local planning):

  ```
  Skill(skill="workflow-plan-phase", args="${PHASE_NUM}")
  ```

**If `INTERACTIVE` is NOT set (default):** Run plan inline.

<!-- workflow:section id="converge-loop" when="state:plan-strategy-converge" -->
If `section_manifest` is `null` or `"converge-loop"` is in its `included` list: read and execute `.ai/library/workflows/autonomous/steps/converge-loop.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Skill(skill=\"workflow-execute-phase\", args=\"${PHASE_NUM} --no-transition\")"
)
```
...
```
Skill(skill="workflow-execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="workflow-${ref.skill}", args="${PHASE_NUM}")
```
...
```
Skill(skill="workflow-code-review", args="${PHASE_NUM} --fix --auto")
```
...
```bash
VERIFY_STATUS=$(workflow_run query verification.status "${PHASE_DIR}" --pick status 2>/dev/null || true)
```
...
Display `Phase ${PHASE_NUM} ✅ ${PHASE_NAME} — Verification passed`, run `@.ai/library/workflows/transition.md`, then Proceed to iterate step.
...
On "All good — continue": set VERIFICATION frontmatter `status: passed`, display `Phase ${PHASE_NUM} ✅ Human validation passed`, run `@.ai/library/workflows/transition.md`, then iterate.
...
| Phase | State | Resume |
|-------|-------|--------|
| ${PHASE_NUM} | verification_deferred_human | /workflow:verify-work ${PHASE_NUM} |
```

Append/update this STATE.md section, display `Phase ${PHASE_NUM} ⏭ verification_deferred_human — resume with /workflow:verify-work ${PHASE_NUM}`, then handle_blocker: "Human verification deferred for phase ${PHASE_NUM}."
...
```
Skill(skill="workflow-plan-phase", args="${PHASE_NUM} --gaps")
```
...
On "Continue anyway": record `verification_deferred_gaps` using the table below, display `Phase ${PHASE_NUM} ⏭ verification_deferred_gaps — resume with /workflow:plan-phase ${PHASE_NUM} --gaps`, then handle_blocker: "Verification gaps deferred for phase ${PHASE_NUM}."
...
| Phase | State | Resume |
|-------|-------|--------|
| ${PHASE_NUM} | verification_deferred_gaps | /workflow:plan-phase ${PHASE_NUM} --gaps |
```

Append/update this STATE.md section, display `Phase ${PHASE_NUM} ⏭ verification_deferred_gaps — resume with /workflow:plan-phase ${PHASE_NUM} --gaps`, then handle_blocker: "Verification gaps deferred for phase ${PHASE_NUM}."
...
HOOKS_JSON=$(workflow_run loop render-hooks verify:post --raw)
```
...
(i.e. prepend `workflow-` to `ref.skill` — so `ui-review` → `workflow-ui-review`.)
...
> Full instructions are in `.ai/library/references/autonomous-smart-discuss.md`. Read that file now and follow it exactly.

Smart discuss is an autonomous-optimized variant of `workflow-discuss-phase`. It proposes grey area answers in batch tables — the user accepts or overrides per area — and writes an identical CONTEXT.md to what discuss-phase produces.

**Inputs:** `PHASE_NUM` from execute_phase.

Read and execute: `.ai/library/references/autonomous-smart-discuss.md`
...
```
### Workflow ► AUTONOMOUS ▸ --to ${TO_PHASE} REACHED

 Completed through phase ${TO_PHASE} as requested.
 Remaining phases were not executed.

 Resume with: /workflow:autonomous --from ${next_incomplete_phase}
```

Proceed to lifecycle step (partial completion skips audit/complete/cleanup). Exit cleanly.

**Otherwise:** After each phase, re-read manager projection, then read STATE.md fresh (same fence — a single `workflow_run query init.manager` fetch backs both the JSON re-filter below and the raw re-read, no double-fetch):
...
```
### Workflow ► AUTONOMOUS ▸ PHASE ${ONLY_PHASE} COMPLETE ✓

 Phase ${ONLY_PHASE}: ${PHASE_NAME} — Done
 Mode: Single phase (--only)

 Lifecycle skipped — run /workflow:autonomous without --only
...
```
Skill(skill="workflow-audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /workflow:audit-milestone to review, then /workflow:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /workflow:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="workflow-complete-milestone", args="${milestone_version}")
```
...
if [[ "$INIT_MANAGER" == @file:* ]]; then INIT_MANAGER=$(cat "${INIT_MANAGER#@file:}"); fi
_workflow_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
ARCHIVE_DIR=$(_workflow_field "$INIT_MANAGER" archive_dir)
...
```
### Workflow ► AUTONOMOUS ▸ STOPPED

 Completed: {list of completed phases}
 Skipped: {list of skipped phases}
 Remaining: {list of remaining phases}

 Resume with: /workflow:autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}${TO_PHASE ? " --to " + TO_PHASE : ""}
```
...
- [ ] Audit invoked via Skill(skill="workflow-audit-milestone")
...
- [ ] `--interactive` runs discuss inline via workflow-discuss-phase (asks questions, waits for user)
...
- [ ] `--converge` routes planning through `workflow-plan-review-convergence`
...
- [ ] Default autonomous planning remains `workflow-plan-phase` when convergence is not requested
</success_criteria>
