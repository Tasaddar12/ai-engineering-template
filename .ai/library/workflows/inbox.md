...
```
===================================================================
  Workflow INBOX TRIAGE — {repo} — {date}
===================================================================
...
```bash
gh issue close {number} --comment "Closed by Workflow inbox triage: this issue is missing required fields per the issue template. Missing: {list}. Please reopen with a complete submission. See CONTRIBUTING.md for requirements."
```

For PRs with gate violations:
```bash
gh pr close {number} --comment "Closed by Workflow inbox triage: this PR does not meet the issue-first requirement. {specific violation}. See CONTRIBUTING.md for the correct process."
```
...
- /workflow:review — Run cross-AI peer review on a specific phase plan
- /workflow:ship — Create a PR from completed work
- /workflow:progress — See overall project state
- /workflow:inbox --label — Re-run with auto-labeling enabled
</offer_next>
