...
**First run `reconcile_gaps`** (above) so gaps already fixed by `/workflow:execute-phase --gaps-only` are marked `resolved` before testing resumes (#1921).
...
- Follow @.ai/library/workflows/diagnose-issues.md
...
---
```
This lets `/workflow:verify-work` reconcile resolved gaps on resume (a gap whose plan has a matching `*-SUMMARY.md` is marked `status: resolved`, not re-diagnosed as a fresh blocker).
</downstream_consumer>
""",
