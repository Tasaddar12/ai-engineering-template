**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@.ai/library/references/verify-mvp-mode.md`. Briefly:
...
PHASE_GOAL=$(workflow_run query roadmap.get-phase "${phase_number}" ${Workflow_WS} --pick goal)
...
if [ "$USER_STORY_VALID" != "true" ]; then
  ...
  echo "Run /workflow mvp-phase ${phase_number} to set a user-story goal before verifying."
  exit 1
fi
```
