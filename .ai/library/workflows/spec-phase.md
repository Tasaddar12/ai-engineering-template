...
INIT=$(workflow_run init phase-op "${PHASE}")
...
If "Skip": Exit with message: "Existing SPEC.md unchanged. Run /workflow:discuss-phase [X] to continue."
...
ones). Reference: @.ai/library/references/edge-probe.md.
...
```bash
# Resolve the compiled edge-probe.cjs against the Workflow install dir via RUNTIME_DIR (#448)
...
EDGE_PROBE_JS=$(for _c in \
  "$_Workflow_RT/workflow-core/bin/lib/edge-probe.cjs" \
  "$_Workflow_RT/bin/lib/edge-probe.cjs" \
  "$_Workflow_RT/.claude/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/workflow-core/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)

# Graceful degradation — never silent skip (RR-04). Build ONLY when $_Workflow_RT is a verified
# Workflow source checkout (has tsconfig.build.json + src/edge-probe.cts), and pin npm to it with
...
# prepublishOnly, so this build path only matters in a Workflow dev checkout (review High).
if [ -z "$EDGE_PROBE_JS" ]; then
  ...
  if [ -z "$EDGE_PROBE_JS" ]; then
    echo "ERROR: edge-probe.cjs not found — reinstall Workflow or run \`npm run build:lib\` in your Workflow checkout." >&2
    exit 1
  fi
fi
...
requirement list). Reference: @.ai/library/references/prohibition-probe.md — the
...
3. **Canon-referral (ADR-550 D6, PROB-13).** A kept candidate that is canon security/compliance
   ...
   /workflow:secure-phase + eslint; not minted here"*) and DROP it. Minting canon items duplicates
   /workflow:secure-phase and drowns the bespoke signal.
   ...
   - **Keep it** → write a NEGATIVE acceptance criterion (a must-NOT line) into Acceptance
     ...
     - **Capture the wired-check descriptor on `test`-tier (#1278, SOFT).** When a prohibition is
       ...
       - `check_violation_fixture` (#1279) — path to a KNOWN-BAD subject the wired check is run
         ...
         read its subject from the `Workflow_PROHIB_SUBJECT` env var so the prover can inject this fixture.
       - `check_clean_fixture` (#1346; **REQUIRED for `node-test` as of #1906**) — path to a
         ...
         `Workflow_PROHIB_SUBJECT` merely being set. For a `node-test` this is **mandatory**: omit it and
         ...
         linted file, no `Workflow_PROHIB_SUBJECT` indirection.)
...
```bash
workflow_run query commit "spec(phase-${phase_number}): add SPEC.md for ${phase_name} — ${requirement_count} requirements (#2213)" --files "${phase_dir}/${padded_phase}-SPEC.md"
```
...
- User directed to /workflow:discuss-phase as next step
