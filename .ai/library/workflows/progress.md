...
INIT=$(workflow_run query init.progress $FORENSIC_PARAM)
...
`/workflow:new-project` — resumes initialization from the first missing artifact; existing PROJECT.md (and any already-created artifacts) are kept, not regenerated.
...
Run /workflow:new-project to start a new project.
```

Exit.

If missing STATE.md: suggest `/workflow:new-project`.

**If ROADMAP.md missing but PROJECT.md exists (and `init_incomplete` is false):**

This means a milestone was completed and archived. Go to **Route F** (between milestones).

If missing both ROADMAP.md and PROJECT.md: suggest `/workflow:new-project`.
</step>

<step name="load">
**Use structured extraction from `workflow_run query`:**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(workflow_run query roadmap.analyze)`
- `STATE=$(workflow_run query state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  workflow_run query summary-extract <path> --fields one_liner
  ```
...
**Generate progress bar from `workflow_run query progress` / `progress.json`, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(workflow_run query progress.bar --raw)
```
...
## Pending Todos
- [count] pending — /workflow:capture --list to review

## Open Windows
- [count] open in `.planning/WINDOWS.md` — /workflow:ship blocks while any remain
(Only show this section if count > 0; suppressed when ledger is empty or absent)

```bash
WINDOWS_STATUS=$(workflow_run windows status --raw 2>/dev/null || echo '')
...
Render `Open Windows` only when `$WINDOWS_OPEN` is greater than `0` (or `$WINDOWS_WAIVED` is greater than `0`, so an auditable deferral history remains visible). Phrase: `{WINDOWS_OPEN} open, {WINDOWS_WAIVED} waived — resolves with /workflow:ship gate; inspect via workflow_run windows status`. The ledger is cross-phase; the count is the project total, not the current phase's.


## Active Debug Sessions
- [count] active — /workflow:debug to continue
...
<!-- workflow:section id="mvp-display" when="state:phase-mvp-mode" -->
If `section_manifest` is `null` or `"mvp-display"` is in its `included` list: read and execute `.ai/library/workflows/progress/steps/mvp-display.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
**If `INCOMPLETE_PHASE` is non-empty:** emit a one-line resume notice in the routing output and route to `/workflow:execute-phase ${INCOMPLETE_PHASE}` instead of running Step 1's current-phase routing. The progress report (already displayed by the `report` step above) gives the user full project status before this routing decision is shown.
...
`/workflow:execute-phase ${INCOMPLETE_PHASE} ${Workflow_WS}`
...
```bash
PHASE_COUNTS=$(workflow_run query find-phase "${CURRENT_PHASE}")
...
```bash
DEBT=$(workflow_run query audit-uat --raw 2>/dev/null)
...
Review: `/workflow:audit-uat ${Workflow_WS}` — full cross-phase audit
Resume testing: `/workflow:verify-work {phase} ${Workflow_WS}` — retest specific phase
```
...
```bash
PHASE_SECTION=$(workflow_run query roadmap.get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/workflow:plan-phase {phase-number} ${Workflow_WS}`
...
`/workflow:discuss-phase {phase}` — gather context and clarify approach

---

**Also available:**
- `/workflow:ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/workflow:plan-phase {phase}` — skip discussion, plan directly
- `/workflow:discuss-phase {phase}` — include assumptions check before planning
...
`/workflow:discuss-phase {phase} ${Workflow_WS}` — gather context and clarify approach

---

**Also available:**
- `/workflow:plan-phase {phase} ${Workflow_WS}` — skip discussion, plan directly
- `/workflow:discuss-phase {phase} ${Workflow_WS}` — include assumptions check before planning
...
`/workflow:plan-phase {phase} --gaps ${Workflow_WS}`

---

**Also available:**
- `/workflow:execute-phase {phase} ${Workflow_WS}` — execute phase plans
- `/workflow:verify-work {phase} ${Workflow_WS}` — run more UAT testing
...
`/workflow:verify-work {phase} ${Workflow_WS}` — resume testing from where you left off

---

**Also available:**
- `/workflow:audit-uat ${Workflow_WS}` — full cross-phase UAT audit
- `/workflow:execute-phase {phase} ${Workflow_WS}` — execute phase plans
...
`/workflow:execute-phase {phase} ${Workflow_WS}` — resumes at the verification gates
...
`/workflow:execute-phase {phase} ${Workflow_WS}` — regenerate verification
...
```
`/workflow:verify-work {phase} ${Workflow_WS}` — re-run verification against the latest summaries
```
...
`/workflow:verify-work {phase} ${Workflow_WS}` — resume human verification
...
```bash
NEXT_PHASE_SECTION=$(workflow_run query roadmap.get-phase "$((Z+1))" 2>/dev/null)
...
`/workflow:discuss-phase {Z+1}` — gather context and clarify approach

---

**Also available:**
- `/workflow:ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
- `/workflow:plan-phase {Z+1}` — skip discussion, plan directly
- `/workflow:verify-work {Z}` — user acceptance test before continuing
...
`/workflow:discuss-phase {Z+1} ${Workflow_WS}` — gather context and clarify approach

---

**Also available:**
- `/workflow:plan-phase {Z+1} ${Workflow_WS}` — skip discussion, plan directly
- `/workflow:verify-work {Z} ${Workflow_WS}` — user acceptance test before continuing
...
`/workflow:complete-milestone ${Workflow_WS}`

---

**Also available:**
- `/workflow:verify-work ${Workflow_WS}` — user acceptance test before completing milestone
...
`/workflow:new-milestone ${Workflow_WS}`
...
- Phase complete but next phase not planned → offer `/workflow:plan-phase [next] ${Workflow_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/workflow:resume-work ${Workflow_WS}`
</step>

<!-- workflow:section id="forensic-audit" when="flag:--forensic" -->
If `section_manifest` is `null` or `"forensic-audit"` is in its `included` list: read and execute `.ai/library/workflows/progress/steps/forensic-audit.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
- [ ] Smart routing: /workflow:execute-phase if plans exist, /workflow:plan-phase if not
- [ ] User confirms before any action
- [ ] Seamless handoff to appropriate workflow command
      </success_criteria>
