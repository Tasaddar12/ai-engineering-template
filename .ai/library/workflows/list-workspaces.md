...
<purpose>
List all Workflow workspaces found in ~/workflow-workspaces/ with their status.
</purpose>
...
INIT=$(workflow_run query init.list-workspaces)
...
```
No workspaces found in ~/workflow-workspaces/

Create one with:
  /workflow:workspace --new --name my-workspace --repos repo1,repo2
```
...
```
Workflow Workspaces (~/workflow-workspaces/)

| Name | Repos | Strategy | Workflow Project |
|------|-------|----------|-------------|
| feature-a | 3 | worktree | Yes |
| feature-b | 2 | clone | No |

Manage:
  cd ~/workflow-workspaces/<name>     # Enter a workspace
  /workflow:workspace --remove <name>  # Remove a workspace
```
...
- **Workflow Project** — whether `.planning/PROJECT.md` exists (Yes/No)
