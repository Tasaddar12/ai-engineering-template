...
Invoked by `/workflow:update --reapply` (`.ai/library/commands/update.md`).

After a Workflow update wipes and reinstalls files, this workflow merges user's previously saved local modifications back into the new version. Uses three-way comparison (pristine baseline, user-modified backup, newly installed version) to reliably distinguish user customizations from version drift.

**Critical invariant:** Every file in `workflow-local-patches/` was backed up because the installer's hash comparison detected it was modified. The workflow must NEVER conclude "no custom content" for any backed-up file — that is a logical contradiction. When in doubt, classify as CONFLICT requiring user review, not SKIP.
...
if [ -n "$KILO_CONFIG_DIR" ]; then
  candidate="$(expand_home "$KILO_CONFIG_DIR")/workflow-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$KILO_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$KILO_CONFIG")")/workflow-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/kilo/workflow-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$OPENCODE_CONFIG_DIR")/workflow-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$OPENCODE_CONFIG")")/workflow-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/opencode/workflow-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$GEMINI_CONFIG_DIR" ]; then
  candidate="$(expand_home "$GEMINI_CONFIG_DIR")/workflow-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CODEX_HOME" ]; then
  candidate="$(expand_home "$CODEX_HOME")/workflow-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CLAUDE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$CLAUDE_CONFIG_DIR")/workflow-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/workflow-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/kilo/workflow-local-patches"
  elif [ -d "$HOME/.config/opencode/workflow-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/opencode/workflow-local-patches"
  elif [ -d "$HOME/.opencode/workflow-local-patches" ]; then
    PATCHES_DIR="$HOME/.opencode/workflow-local-patches"
  elif [ -d "$HOME/.gemini/workflow-local-patches" ]; then
    PATCHES_DIR="$HOME/.gemini/workflow-local-patches"
  elif [ -d "$HOME/.codex/workflow-local-patches" ]; then
    PATCHES_DIR="$HOME/.codex/workflow-local-patches"
  else
    PATCHES_DIR="$HOME/.claude/workflow-local-patches"
  fi
fi
# Local install fallback — check all runtime directories
if [ ! -d "$PATCHES_DIR" ]; then
  for dir in .config/kilo .kilo .config/opencode .opencode .gemini .codex .claude; do
    if [ -d "./$dir/workflow-local-patches" ]; then
      PATCHES_DIR="./$dir/workflow-local-patches"
      break
    fi
  done
fi
...
Local patches are automatically saved when you run /workflow:update
after modifying any Workflow workflow, command, or agent files.
```
Exit.

## Step 2: Determine baseline for three-way comparison

The quality of the merge depends on having a **pristine baseline** — the original unmodified version of each file from the pre-update Workflow release. This enables three-way comparison:
- **Pristine baseline** (original Workflow file before any user edits)
- **User's version** (backed up in `workflow-local-patches/`)
...
CLASSIFY_OUTPUT="$(node "${Workflow_HOME}/workflow-core/bin/verify-reapply-patches.cjs" "${CLASSIFY_ARGS[@]}")"
...
- This is what ends the re-graft cycle: because the installed file stays identical to what
  ...
  the file drops out of `workflow-local-patches/` on the next cycle.
...
merge paths below, unchanged. If the classifier cannot run at all (e.g. `Workflow_HOME`
...
1. **Read the backed-up version** (user's modified copy from `workflow-local-patches/`)
2. **Read the newly installed version** (current file after update)
3. **If available, read the pristine baseline** (from git history or `workflow-pristine/`)
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `$HOME/.claude/`), variable additions (`${Workflow_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
...
git -C "$CONFIG_DIR" log --oneline --no-merges -- "{file_path}" | grep -v "workflow:update\|workflow-update\|Workflow update\|workflow-install"
```
...
Run the verifier as a child process (the workflow-tools binary directory is not required — the script ships under `workflow-core/bin/` in the source repo and is installed to `${Workflow_HOME}/workflow-core/bin/`):
...
VERIFY_OUTPUT="$(node "${Workflow_HOME}/workflow-core/bin/verify-reapply-patches.cjs" "${VERIFY_ARGS[@]}")"
VERIFY_STATUS=$?
```
...
workflow-pristine/ snapshot on disk does not match the hash recorded in backup-meta.json
(pristine drift — the snapshot was refreshed to a newer Workflow version after the backup
...
Resolve before re-running:
  ...
  (c) If the upstream changes are acceptable, update the backup-meta.json
      ...
      re-run /workflow:update --reapply to re-verify with the refreshed baseline.

Then re-run /workflow:update --reapply to re-verify.
```
...
/workflow:update --reapply to retry with full verification.
```
...
Ask user:
- "Keep patch backups for reference?" → preserve `workflow-local-patches/`
- "Clean up patch backups?" → remove `workflow-local-patches/` directory
...
- [ ] Three-way merge used when pristine baseline available (git history or workflow-pristine/)
