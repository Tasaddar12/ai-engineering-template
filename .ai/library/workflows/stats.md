...
STATS=$(workflow_run query stats.json)
...
If no `.planning/` directory exists, inform the user to run `/workflow:new-project` first.
</step>

<step name="mvp_summary">
**MVP phase summary.** Read all phases via `workflow_run query roadmap.analyze` (Phase 1's `cmdRoadmapAnalyze` surfaces a `mode` field per phase). Count phases by mode:
