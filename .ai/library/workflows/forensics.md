...
Post-mortem investigation for failed or stuck Workflow workflows. Analyzes git history,
...
3. {Recovery command if applicable — e.g., `/workflow:resume-work`, `/workflow:execute-phase N`}

---

*Report generated by `/workflow:forensics`. All paths redacted for portability.*
```
...
BUG_LABEL=$(gh label list --repo .ai/library/SOURCES.md#source-39174a1e68428fbd --search "bug" --json name -q '.[0].name' 2>/dev/null)
LABEL_FLAG=""
if [ -n "$BUG_LABEL" ]; then
...
gh issue create \
  --repo .ai/library/SOURCES.md#source-39174a1e68428fbd \
