...
Saves artifacts to `.planning/spikes/`. Companion to `/workflow:spike --wrap-up`.
...
**b. Findings skills** — glob `./.claude/skills/spike-findings-*/SKILL.md` and read any that exist, plus their `.ai/library/references/*.md`. These contain curated knowledge from prior wrap-ups.
...
**b. Findings skills:** Glob for `./.claude/skills/spike-findings-*/SKILL.md` and read any that exist, plus their `.ai/library/references/*.md` files.
...
```bash
workflow_run query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files .planning/spikes/NNN-descriptive-name/ .planning/spikes/MANIFEST.md
```
...
```bash
workflow_run query commit "docs(spikes): update conventions" --files .planning/spikes/CONVENTIONS.md
```
</step>

<step name="report">
```
### Workflow ► SPIKE COMPLETE ✓
...
**Also available:**
- `/workflow:spike` — spike more ideas (or run with no argument for frontier mode)
- `/workflow:plan-phase` — start planning the real implementation
- `/workflow:explore` — continue exploring the idea
