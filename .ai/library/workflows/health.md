...
```bash
workflow_run query validate.context \
...
```bash
workflow_run query validate.health $REPAIR_FLAG $BACKFILL_FLAG
```
...
USE_WORKTREES=$(workflow_run query config-get workflow.use_worktrees --raw 2>/dev/null)
if [ "$INSPECTED_ISOLATION" = "none" ] && [ -n "$USE_WORKTREES" ] && [ "$USE_WORKTREES" != "false" ]; then
  if [ "$INSPECTED_RESOLVED" = "true" ]; then
    echo "W025: the project config sets workflow.use_worktrees to a non-false value, but this runtime has no usable executor-isolation primitive — dispatch.isolation resolves to none, declared as none, or fail-closed because the capability could not be determined — so /workflow:execute-phase and /workflow:quick will fail closed. Fix: run /workflow:settings and answer No to Worktrees, or set workflow.use_worktrees: false in the project config (the active workstream's config.json when one is active). Set it explicitly rather than deleting the key — an absent key resolves to false only on an emit whose default was stamped to false, and to true otherwise."
  else
    echo "W025: the project config sets workflow.use_worktrees to a non-false value, and Workflow could not resolve this runtime's executor-isolation capability ('workflow_run query inspect-dispatch-isolation' failed or returned nothing) — so it cannot tell whether /workflow:execute-phase and /workflow:quick will fail closed on that value. This is a report of an unverifiable config, NOT a finding that the runtime declares no primitive. Fix: re-run once the workflow-tools shim resolves; if the warning persists, run /workflow:settings and answer No to Worktrees."
  fi
fi
```
...
- [E001] config.json: JSON parse error at line 5
  Fix: Run /workflow:health --repair to reset to defaults

- [E002] PROJECT.md not found
  Fix: Run /workflow:new-project to create
```
...
```
---
N issues can be auto-repaired. Run: /workflow:health --repair
```
</step>
...
```
Would you like to run /workflow:health --repair to fix N issues automatically?
```
...
```bash
workflow_run query validate.health
```
...
| W010 | warning | Workflow agent installation missing or incomplete | No |
...
| W019 | warning | Unrecognized .planning/ root file — not a canonical Workflow artifact | No |
...
| W028 | warning | A Workflow-owned install scope shadows another on this machine | No |
