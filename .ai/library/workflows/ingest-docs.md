...
INIT=$(workflow_run init ingest-docs)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
CLASSIFIER_MODEL=$(workflow_run query resolve-model doc-classifier --raw)
SYNTHESIZER_MODEL=$(workflow_run query resolve-model doc-synthesizer --raw)
ROADMAPPER_MODEL=$(workflow_run query resolve-model roadmapper --raw)
```
...
If user passed `--mode new` but `.planning/` already exists: display warning and require explicit confirm via `AskUserQuestion` (approve-revise-abort from `.ai/library/references/gate-prompts.md`) before overwriting.
...
```
Workflow > Discovered {N} docs, which exceeds the v1 cap of 50.
      Use --manifest to narrow the set to ≤ 50 files, or run
      /workflow:ingest-docs again with a narrower <path>.
```

Exit without proceeding.

**Display discovered set** and request approval (see `.ai/library/references/gate-prompts.md` — `yes-no-pick` pattern works; or `approve-revise-abort`):
...
For each discovered doc, spawn `doc-classifier` in parallel. In Claude Code, issue all Task calls in a single message with multiple tool uses so the harness runs them concurrently. For Copilot / sequential runtimes, fall back to sequential dispatch.
...
- `<required_reading>` — `.ai/library/agents/doc-classifier.md` (the agent definition itself)
...
Spawn `doc-synthesizer` once (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze):
...
Agent({
  ...
  prompt: "
    ...
    <required_reading>
    - .ai/library/agents/doc-synthesizer.md
...
Apply the safety semantics from `.ai/library/references/doc-conflict-engine.md`. Operation noun: `ingest`.
...
```
Workflow > BLOCKED: {N} blockers must be resolved before ingest can proceed.
```
...
Optionally display `Workflow > No conflicts. Auto-resolved: {N}.` Absence of conflicts is not authorization to write: proceed to the routing gate for the active mode — new mode's routing gate (next step) or merge mode's merge-diff approve-revise-abort gate — which decides whether destination files are created.
...
Delegate to `roadmapper` (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze):

```
Agent({
  subagent_type: "roadmapper",
...
- Next step: `/workflow:plan-phase 1` (new) or `/workflow:plan-phase N` (merge, pointing at the first newly-added phase); for intel-only runs there is no roadmap yet — point at `/workflow:new-project` (or a later re-run with `--mode merge` once scaffold files exist), never `/workflow:plan-phase`
