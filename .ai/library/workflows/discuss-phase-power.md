...
<trigger>
This workflow executes when `--power` flag is present in ARGUMENTS to `/workflow:discuss-phase`.
...
Consider running /workflow:discuss-phase {N} again to refine before planning.
```
...
Next step: /workflow:plan-phase {N}
```
</step>
