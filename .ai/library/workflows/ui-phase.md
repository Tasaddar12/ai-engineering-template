<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates ui-researcher and ui-checker with a revision loop. Inserts between discuss-phase and plan-phase in the lifecycle.
...
AGENT_SKILLS_UI=$(workflow_run query agent-skills ui-researcher)
AGENT_SKILLS_UI_CHECKER=$(workflow_run query agent-skills ui-checker)
```
...
```bash
UI_RESEARCHER_MODEL=$(workflow_run query resolve-model ui-researcher --raw)
UI_CHECKER_MODEL=$(workflow_run query resolve-model ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(workflow_run query config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
```

**If `UI_ENABLED` is `false`:**
```
UI phase is disabled in config. Enable via /workflow:settings.
```
Exit workflow.
...
Recommended: run /workflow:discuss-phase {N} first to capture design preferences.
...
⚡ Sketch findings detected: {SKETCH_FINDINGS_PATH}
   Validated design decisions from /workflow:sketch will be loaded into the UI researcher.
...
## 5. Spawn ui-researcher

Display:
```
### Workflow ► UI DESIGN CONTRACT — PHASE {N}
...
```markdown
Read .ai/library/agents/ui-researcher.md for instructions.
...
- {SKETCH_FINDINGS_PATH} (Sketch Findings — validated design decisions, CSS patterns, visual direction from /workflow:sketch, if exists)
</required_reading>
...
Template: .ai/templates/UI-SPEC.md
</output>
...
```markdown
Read .ai/library/agents/ui-checker.md for instructions.
...
**If `revision_count` < 2:**
- Re-spawn ui-researcher with revision context. `revision_count` is incremented on the
...
2. Edit manually — open UI-SPEC.md in editor, re-run /workflow:ui-phase
...
@.ai/library/references/ui-consideration-probe.md.
...
```bash
# Resolve the compiled ui-consideration-probe.cjs against the Workflow install dir via RUNTIME_DIR
...
_Workflow_RT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
UI_PROBE_JS=$(for _c in \
  "$_Workflow_RT/workflow-core/bin/lib/ui-consideration-probe.cjs" \
  "$_Workflow_RT/bin/lib/ui-consideration-probe.cjs" \
  "$_Workflow_RT/.claude/bin/lib/ui-consideration-probe.cjs" \
  "$HOME/.claude/workflow-core/bin/lib/ui-consideration-probe.cjs" \
  "$HOME/.claude/bin/lib/ui-consideration-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)

# Graceful degradation — never a silent skip. Build ONLY when $_Workflow_RT is a verified Workflow source
...
# compiled .cjs via prepublishOnly, so this path only matters in a Workflow dev checkout.
if [ -z "$UI_PROBE_JS" ]; then
  if [ -f "$_Workflow_RT/tsconfig.build.json" ] && [ -f "$_Workflow_RT/src/ui-consideration-probe.cts" ]; then
    npm --prefix "$_Workflow_RT" run build:lib 2>/dev/null || true
    UI_PROBE_JS=$(for _c in \
      "$_Workflow_RT/workflow-core/bin/lib/ui-consideration-probe.cjs" \
      "$_Workflow_RT/bin/lib/ui-consideration-probe.cjs" \
      "$_Workflow_RT/.claude/bin/lib/ui-consideration-probe.cjs" \
  ...
  if [ -z "$UI_PROBE_JS" ]; then
    echo "ERROR: ui-consideration-probe.cjs not found — reinstall Workflow or run \`npm run build:lib\` in your Workflow checkout." >&2
    exit 1
  fi
fi
...
`/clear` then: `/workflow:plan-phase {N}`

{If CONTEXT.md does NOT exist:}
**Discuss Phase {N}** — gather implementation context before planning

`/clear` then: `/workflow:discuss-phase {N}`

(or `/workflow:plan-phase {N}` to skip discussion)
...
```bash
workflow_run query commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
...
- [ ] ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] ui-checker spawned with UI-SPEC.md
