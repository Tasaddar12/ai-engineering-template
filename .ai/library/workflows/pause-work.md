...
<purpose>
Create structured `.planning/HANDOFF.json` and `.continue-here.md` handoff files to preserve complete work state across sessions. The JSON provides machine-readable state for `/workflow:resume-work`; the markdown provides human-readable context.
</purpose>
...
timestamp=$(workflow_run query current-timestamp full --raw)
```
...
<step name="commit">
```bash
workflow_run query commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
...
To resume: /workflow:resume-work
