...
- doc-writer — Writes and updates project documentation files
- doc-verifier — Verifies factual claims in docs against the live codebase
</available_agent_types>

<process>

**Compact Content Gate.** Read and follow `.ai/library/references/compact-content-gate.md` now — it states the `workflow.compact_content` check and the resolution rule this spine defers to. When it directs a Read, read `.ai/library/workflows/docs-update/detail/elaboration.md` in full before continuing past this point; its content elaborates on three steps below (sequential_generation, fix_loop, verify_only_report).
...
INIT=$(workflow_run query docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(workflow_run query agent-skills doc-writer)
...
INIT_DOCS_UPDATE=$(workflow_run query init.docs-update)
if [[ "$INIT_DOCS_UPDATE" == @file:* ]]; then INIT_DOCS_UPDATE=$(cat "${INIT_DOCS_UPDATE#@file:}"); fi
DOC_VERIFIER_MODEL=$(workflow_run query resolve-model doc-verifier --raw)
```
...
- `existing_docs` — array of `{path, has_workflow_marker}` objects for existing Markdown files
...
- These will be passed to doc-verifier in the verify_docs step for accuracy checking
- If inaccuracies are found, they will be dispatched to doc-writer in `fix` mode for surgical corrections
...
4. For each gap the user selects:
   ...
   - The doc-writer will receive a `doc_assignment` with `type: "custom"` and a description of what to document, using the project's source files as content discovery targets
...
3. If no docs in the queue have `has_workflow_marker: false` in the `existing_docs` array: skip to detect_runtime_capabilities.

**For each queued doc where `has_workflow_marker` is false (hand-written doc detected):**

Present the following choice using `AskUserQuestion` if available, or inline prompt otherwise:

```
{filename} appears to be hand-written (no Workflow marker found).
...
How should this file be handled?
  ...
  [3] regenerate  -- Overwrite with a fresh Workflow-generated doc.
```
...
Spawn 3 parallel doc-writer agents for Wave 1 docs: README, ARCHITECTURE, CONFIGURATION (each runs in a subagent — no output until they return, ~1–5 min; expected, not a freeze).
...
1. Print: `◆ Spawning doc verifier for {doc_path}... (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)`
   Spawn the `doc-verifier` agent (or invoke sequentially if Task tool is unavailable) with a `<verify_assignment>` block:
   ```xml
   <verify_assignment>
...
1. Print: `◆ Spawning doc verifier for {doc_path}... (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)`
   Spawn the `doc-verifier` agent with the same `<verify_assignment>` block as above.
...
Non-canonical docs with failures ARE eligible for the fix_loop. When a non-canonical doc has `claims_failed > 0`, dispatch it to doc-writer in `fix` mode with the failures array — the writer's fix mode does surgical corrections on specific lines regardless of doc type (no template needed). The writer MUST NOT restructure, rephrase, or reformat any content beyond the failing claims.
...
For each doc with a failure, per iteration:
   a. Read the current file content from disk. Record the pre-fix line count:
      ...
      ```
   b. Spawn `doc-writer` with the `<doc_assignment>` block above.
...
Exact iteration bookkeeping and the regression-halt report wording: `.ai/library/workflows/docs-update/detail/elaboration.md` § 2.
</step>

<step name="verify_only_report">
**Reached when `--verify-only` is present in `$ARGUMENTS`** — an early-exit reporting mode: do not proceed to dispatch, generation, commit, or report steps after this step. Spawn `doc-verifier` (read-only) for every file in `existing_docs`, count `<!-- VERIFY:` markers in each, and present a combined claims-checked/passed/failed/markers table plus a "how to fix" pointer to `/workflow:docs-update` (or `--force` to regenerate everything). Clean up `.planning/tmp/verify-*.json` afterward. End the workflow here.

Exact table format and failure-detail wording: `.ai/library/workflows/docs-update/detail/elaboration.md` § 3.
</step>
...
3. Re-run /workflow:docs-update to regenerate clean docs
```
...
```bash
workflow_run query commit "docs: generate project documentation" \
...
```
Run `/workflow:docs-update --verify-only` to fact-check generated docs against the codebase.
```
...
- [ ] Generated docs contain zero Workflow methodology content
...
- [ ] Hand-written docs (no Workflow marker) prompted for preserve/supplement/regenerate before dispatch (unless --force)
...
- [ ] --verify-only invokes doc-verifier for full fact-checking (not just VERIFY marker count)
</success_criteria>
