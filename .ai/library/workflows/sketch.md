...
Companion to `/workflow:sketch --wrap-up`.
...
**b. Findings skills** — glob `./.claude/skills/sketch-findings-*/SKILL.md` and read any that exist, plus their `.ai/library/references/*.md`. These contain curated design decisions from prior wrap-ups.
...
**a.** Glob for `./.claude/skills/spike-findings-*/SKILL.md` and read any that exist, plus their `.ai/library/references/*.md`. These contain validated patterns and requirements.
...
```bash
workflow_run query commit "docs(sketch-NNN): [winning direction] — [key visual insight]" --files .planning/sketches/NNN-descriptive-name/ .planning/sketches/MANIFEST.md
```
...
**Also available:**
- `/workflow:sketch` — sketch more (or run with no argument for frontier mode)
- `/workflow:plan-phase` — start building the real UI
- `/workflow:spike` — spike technical feasibility of a design pattern
