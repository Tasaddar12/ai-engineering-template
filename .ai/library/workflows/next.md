...
<purpose>
Detect current project state and automatically advance to the next logical Workflow workflow step.
...
# Get state snapshot
workflow_run query state.json 2>/dev/null || echo "{}"
```
...
```
No Workflow project detected. Run `/workflow:new-project` to get started.
```
Exit.
</step>
...
Run `/workflow:health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
**Hard invariant: any phase with PLAN.md files lacking matching SUMMARY.md files must be completed before `/workflow:progress --next` routes to any forward action.**

This catches the common failure mode where a session died mid-execution (hang, token exhaustion, API connection drop) and STATE.md's `current_phase` got advanced past the phase that actually has unfinished work. Without this gate, `/workflow:progress --next` would route by `current_phase` and silently skip the partially-executed phase.
...
Scan ALL phases in ROADMAP order (lowest-numbered to highest) for incomplete-execution state. Use `workflow_run query roadmap.analyze` to get the phase list, then for each phase number `N` query `workflow_run query find-phase <N>` JSON and inspect its `plans` and `summaries` arrays. A phase is **incomplete-execution** when `plans.length > summaries.length` (at least one PLAN.md has no matching SUMMARY.md).
...
```bash
INCOMPLETE_PHASE=""
ROADMAP_JSON=$(workflow_run query roadmap.analyze)
...
if [ $? -ne 0 ] || [ -z "$ROADMAP_JSON" ]; then
  ...
  for PHASE_NUM in $(echo "$ROADMAP_JSON" | jq -r '.phases[] | (.number // .phase_number // empty)'); do
    PHASE_JSON=$(workflow_run query find-phase "$PHASE_NUM")
...
**If `INCOMPLETE_PHASE` is non-empty:** route to `/workflow:execute-phase $INCOMPLETE_PHASE` and exit. Display a one-line notice before invoking:

```
▶ Resuming incomplete Phase ${INCOMPLETE_PHASE} (plans without summaries detected)
  /workflow:execute-phase ${INCOMPLETE_PHASE}
...
Scan all phases that precede the current phase in ROADMAP.md order for incomplete work. For each prior phase number `N`, use `workflow_run query find-phase <N>` JSON (plans, summaries, incomplete_plans, etc.) to inspect that phase.
...
**Source phase:** {src}
**Deferred at:** {date} during /workflow:progress --next advancement to Phase {dest}
**Plans:**
- [ ] {N}-{M}: {slug} (ran, no SUMMARY.md)
```
2. Commit the deferral record:
```bash
workflow_run query commit "docs: defer incomplete Phase {src} items to backlog" \
...
⚠ Pending exploratory work:
  ...
  Resume with `/workflow:spike` or `/workflow:sketch`, or continue with phase work below.
```
...
→ Next action: `/workflow:discuss-phase <first-phase>`
...
if [ "$PLAN_STRATEGY" = "converge" ]; then
  ...
  if [ "$CONVERGENCE_ENABLED" != "true" ]; then
    printf '%s\n' \
      '/workflow:progress --next --converge is disabled (workflow.plan_review_convergence=false).' \
      '' \
...
▶ **Next step:** `/workflow-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/workflow:progress --next` is zero-friction advancement.

**Route 3 convergence override:** When the routing decision is Route 3 (plan) and `PLAN_STRATEGY=converge`, invoke `/workflow:plan-review-convergence <current-phase> ${CONVERGENCE_ARGS}` instead of `/workflow:plan-phase <current-phase>`.

**If `--auto` was passed:** after the determined command completes, automatically re-invoke `/workflow:progress --next --auto` (forwarding `--converge`/`--cross-ai` and any reviewer flags if they were originally passed) to continue chaining to the next step. Repeat until one of:
- A milestone completes (`/workflow:complete-milestone` is reached)
...
Resume with: `/workflow:progress --next --auto` once resolved.
```
</step>
...
- [ ] Scan uses `workflow_run` (canonical resolver form); errors are surfaced rather than suppressed
...
- [ ] `--converge` routes Route 3 planning through `workflow-plan-review-convergence`
...
- [ ] Default planning remains `workflow-plan-phase` when convergence is not requested
</success_criteria>
