...
```bash
workflow_run query estimate-calibrate
```
...
The planner reads it on the next `/workflow:plan-phase`, so estimates improve for THIS project
over time.
