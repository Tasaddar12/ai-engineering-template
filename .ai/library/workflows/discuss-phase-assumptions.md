...
- assumptions-analyzer — Analyzes codebase to surface implementation assumptions
</available_agent_types>
...
INIT=$(workflow_run query init.discuss-phase-assumptions "${PHASE}" $AUTO_PARAM)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ANALYZER=$(workflow_run query agent-skills assumptions-analyzer)
# #2072: resolve the routed model so model_overrides / models.discuss are honored
# (the resolver maps assumptions-analyzer → phaseType "discuss"); thread it below.
ANALYZER_MODEL=$(workflow_run query resolve-model assumptions-analyzer --raw)
```
...
Use /workflow:progress to see available phases.
```
Exit workflow.
...
- When spawning the assumptions-analyzer, pass the lens list so it can flag which lenses apply
...
Spawn a `assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```
Agent(subagent_type="assumptions-analyzer", model="{ANALYZER_MODEL}", prompt="""
...
```bash
workflow_run query commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
`/workflow:plan-phase ${PHASE}`

---

**Also available:**
- `/workflow:plan-phase ${PHASE} --skip-research` — plan without research
- `/workflow:ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
<!-- workflow:section id="auto-advance-dispatch" when="state:auto-advance-active" -->
If `section_manifest` is `null` or `"auto-advance-dispatch"` is in its `included` list: read and execute `.ai/library/workflows/discuss-phase-assumptions/steps/auto-advance-dispatch.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
