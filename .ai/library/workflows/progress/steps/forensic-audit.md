...
Glob every phase directory's SCOPE BOUNDARY log (executor writes out-of-scope discoveries here per `.ai/library/agents/executor.md`):
```bash
...
- Check 2 (orphaned handoff): `Read the handoff file(s) and resume from where work was paused: /workflow:resume-work ${Workflow_WS}`
...
- Check 1 (STATE inconsistency): `Run /workflow:verify-work ${PHASE} ${Workflow_WS} to reconcile state`
</step>
