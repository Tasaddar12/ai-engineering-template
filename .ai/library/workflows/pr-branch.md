...
- **default** (`planning.pr_strict: false`) — the PR branch contains code changes and
  structural planning state. Reviewers don't see Workflow transient artifacts (PLAN.md,
...
- **strict** (`planning.pr_strict: true`) — *every* .planning/ path is filtered out,
  ...
  keeps real git history (so `/workflow:undo` and revert paths have something to restore) and
...
TARGET=${1:-$(workflow_run query git.base-branch)}
```
...
```bash
PR_STRICT=$(workflow_run query config-get planning.pr_strict --raw 2>/dev/null)
...
```bash
SUB_REPOS_JSON=$(workflow_run query config-get planning.sub_repos 2>/dev/null)
...
RESULT=$(workflow_run query pr-subrepo "$COMMIT_MSG" \
  --repo "$REPO_REL" \
  --branch "$SUB_BRANCH")
SUBREPO_EXIT=$?
```
...
for HASH in $(printf '%s' "$INCLUDED_COMMITS"); do
  ...
  if [ -n "$(git diff --name-only --diff-filter=U)" ]; then
    ...
    echo "Resolve the conflict against $TARGET, then re-run /workflow:pr-branch." >&2
    exit 1
  fi
...
Or use /workflow:ship to create the PR automatically.
```
