...
- Example: `/workflow-add-phase Add authentication` → description = "Add authentication"
- Example: `/workflow-add-phase Fix critical performance issues` → description = "Fix critical performance issues"

If no arguments provided:

```
ERROR: Phase description required
Usage: /workflow-add-phase <description>
Example: /workflow-add-phase Add authentication system
```
...
INIT=$(workflow_run query init.phase-op "0")
...
<step name="add_phase">
**Delegate the phase addition to `workflow_run query phase.add`:**

```bash
RESULT=$(workflow_run query phase.add "${description}")
```
...
**Also available:**
- `/workflow-add-phase <description>` — add another phase
- Review roadmap
...
<success_criteria>
- [ ] `workflow_run query phase.add` executed successfully
