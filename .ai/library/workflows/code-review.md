...
<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns code-reviewer agent, commits REVIEW.md, and presents results to user. When --fix is passed, delegates to code-review-fix.md after review to auto-apply findings via code-fixer.
</purpose>
...
- code-reviewer: Reviews source files for bugs and quality issues
- code-fixer: Applies fixes to code review findings (used via dispatch_fix → code-review-fix.md when --fix is passed)
</available_agent_types>
...
FLAGS_JSON=$(node -e "
  const { parseCodeReviewFlags } = require('./workflow-core/bin/lib/code-review-flags.cjs');
...
INIT=$(workflow_run query init.code-review "${PHASE_ARG}" $FIX_PARAM)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_REVIEWER=$(workflow_run query agent-skills code-reviewer)
# #2072: resolve the routed model so model_overrides / models.verification are honored
# (the resolver maps code-reviewer → phaseType "verification"); thread it below.
REVIEWER_MODEL=$(workflow_run query resolve-model code-reviewer --raw)
```
...
```
Error: Phase ${PHASE_ARG} not found. Run /workflow:progress to see available phases.
```
...
```bash
CODE_REVIEW_ENABLED=$(workflow_run query config-get workflow.code_review --raw 2>/dev/null || echo "true")
```
...
# #3503: anchor the grep to Workflow's own conventional-commit phase scopes — the
# subject-line formats this system itself emits (docs(phase-N): from
# execute-phase.md, plan scopes feat(N-MM):/test(N-MM): from .ai/library/references/tdd.md,
...
# into a base unboundedly before the phase, while Workflow's own scope commits
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$DIFF_BASE" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /workflow:code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
...
1. **Expand tilde paths:** SUMMARY.md `key-files` entries may record a `~/...`-prefixed path (e.g. `.ai/library/workflows/verify-work.md`). Bash only tilde-expands a literal `~` written in source text, never one arriving as the value of an already-expanded variable, so every later `[ -f "$file" ]` check must see a real, expanded path or it misclassifies the file as deleted.
```bash
EXPANDED_FILES=()
...
```bash
CONFIG_DEPTH=$(workflow_run query config-get workflow.code_review_depth --raw 2>/dev/null || echo "")
DEPTH_OVERRIDES=$(workflow_run query config-get workflow.code_review_depth_overrides --default '[]' 2>/dev/null || echo '[]')
REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
...
DEPTH_JSON=$(echo "$DEPTH_PAYLOAD" | node -e "
  const { resolveCodeReviewDepth } = require('./workflow-core/bin/lib/code-review-depth.cjs');
...
<!-- workflow:section id="structural-pre-pass" when="state:fallow-enabled" -->
If `section_manifest` is `null` or `"structural-pre-pass"` is in its `included` list: read and execute `.ai/library/workflows/code-review/steps/structural-pre-pass.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
scope alongside the internal `code-reviewer` agent below. **No canonical flag present is
...
This step is itself opt-in at the capability layer (see `.ai/library/references/loop-hook-dispatch.md`
...
CODE_REVIEW_POINT=$(workflow_run query config-get workflow.code_review_point --raw 2>"$CODE_REVIEW_POINT_STDERR") || {
...
EXPLICIT_JOINED=$(workflow_run review-lane explicit-from-argv -- "$@" 2>"$EXPLICIT_JOINED_STDERR") || {
  ...
  # the whole `/workflow:code-review` run for users who never asked for a reviewer lane in the first
...
if [ ${#EXPLICIT_REVIEWER_SLUGS[@]} -gt 0 ] && [ -z "$DIFF_BASE" ]; then
  ...
  # #4209 R5: a dedicated run-scoped temp dir (same `${TMPDIR:-/tmp}/workflow-review-*` convention
  # review.md's gather_context step uses), not $PHASE_DIR directly — lane artifacts
  # (workflow-review-prompt.md, workflow-review-<slug>.md/.err) are read-once evidence for THIS run, never
  ...
  LANE_RUN_DIR=$(mktemp -d "${TMPDIR:-/tmp}/workflow-review-lanes-XXXXXX")
  DISPATCH_JSON=$(printf '%s\n' "${REVIEW_FILES[@]}" | workflow_run review-lane dispatch-step \
...
if [ -n "$FALLOW_JSON_PATH" ] && [ -f "$FALLOW_JSON_PATH" ]; then
  ...
  FALLOW_SRC="$FALLOW_JSON_PATH" FALLOW_OUT="$FALLOW_NORMALIZED_PATH" node -e "
    const fs = require('fs');
    const { normalizeFallowReportFile } = require('./workflow-core/bin/lib/fallow-runner.cjs');
...
```
Agent(subagent_type="code-reviewer", model="{REVIEWER_MODEL}", prompt="
<required_reading>
${FILES_TO_READ}
</required_reading>
...
No REVIEW.md created. You can retry with /workflow:code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with /workflow:code-review ${PHASE_ARG}"
fi
```
</step>

<!-- workflow:section id="dispatch-fix" when="flag:--fix" -->
If `section_manifest` is `null` or `"dispatch-fix"` is in its `included` list: read and execute `.ai/library/workflows/code-review/steps/dispatch-fix.md`. Otherwise skip — do not read the file; proceed to `present_results`.
<!-- /workflow:section -->
...
Next steps:
  /workflow:code-review ${PHASE_NUMBER} --fix  — Auto-fix issues
