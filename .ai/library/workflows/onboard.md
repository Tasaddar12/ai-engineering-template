...
@.ai/library/references/run-resolver.md
...
Run the standard `workflow_run` resolver from the reference above, then run the projection from the runtime root:

```bash
# If --fast was parsed from $ARGUMENTS:
INIT=$(workflow_run --cwd "$_Workflow_RUNTIME_ROOT" init onboard --fast --raw)
# Otherwise:
INIT=$(workflow_run --cwd "$_Workflow_RUNTIME_ROOT" init onboard --raw)
...
- `ONBOARDING_ROOT={git_worktree_root || _Workflow_RUNTIME_ROOT}`.
...
```bash
workflow_run --cwd "$ONBOARDING_ROOT" query commit "docs: create onboarding summary" --files .planning/onboarding/SUMMARY.md
```
