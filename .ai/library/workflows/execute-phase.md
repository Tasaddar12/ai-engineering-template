...
- **Claude Code:** Uses `Agent(subagent_type="executor", ...)` — backgrounded by default; verify completion
...
- **Other runtimes:** If `Agent`/`agent` tool is genuinely unavailable (e.g. a backgrounded
  ...
  Code), you MUST spawn executor agents — inline execution is not authorized. Check for
...
@.ai/library/references/context-budget.md
...
- verifier — Verifies phase completion, checks quality gates
...
- integration-checker — Checks cross-phase integration
- nyquist-auditor — Validates verification coverage
- ui-researcher — Researches UI/UX approaches
- ui-checker — Reviews UI implementation quality
- ui-auditor — Audits UI against design requirements
</available_agent_types>

<process>

**Compact Content Gate.** Read and follow `.ai/library/references/compact-content-gate.md` now — it states the `workflow.compact_content` check and the resolution rule this spine defers to. When it directs a Read, read `.ai/library/workflows/execute-phase/detail/elaboration.md` in full before continuing past this point; its content elaborates on two steps below (check_interactive_mode, cross_ai_delegation).
...
INIT=$(workflow_run query init.execute-phase "${PHASE_ARG}" $WAVE_PARAM)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(workflow_run query agent-skills executor)
```
...
@.ai/library/references/execute-phase-response-language.md
...
EXECUTOR_STALL_INTERVAL_MINUTES=$(workflow_run query config-get executor.stall_detect_interval_minutes --raw 2>/dev/null || echo "5")
EXECUTOR_STALL_THRESHOLD_MINUTES=$(workflow_run query config-get executor.stall_threshold_minutes --raw 2>/dev/null || echo "10")
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @.ai/library/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @.ai/library/references/planner-antipatterns.md
...
Check if the current runtime is Copilot by testing for the `@executor` agent pattern
...
MVP_MODE=$(workflow_run query phase.mvp-mode "${PHASE_NUMBER}" $MVP_FLAG_ARG --pick active)
EXECUTE_POST_HOOKS_JSON=$(workflow_run loop render-hooks execute:post --raw)
TDD_MODE=$(workflow_run loop render-hooks execute:post --active-cap tdd)
```
...
if [ "$TDD_MODE" = "true" ]; then
  ...
  if [ "$IS_BEHAVIOR_ADDING" = "true" ]; then
    ...
    if [ -z "$RED_COMMIT" ]; then
      workflow_run query state.update last_gate_trip "${PLAN_ID}/${TASK_ID}" || true
...
```
Pure doc-only / config-only / test-only tasks return `is_behavior_adding=false` and are exempt. When the gate trips, Read `.ai/library/references/execute-mvp-tdd.md` for the exact halt report format.
</step>
...
**Parse `--interactive` flag from $ARGUMENTS.** If present, switch to interactive execution mode: plans run sequentially **inline** (no subagent spawning, ignoring wave grouping), reading `execute-plan.md` directly rather than dispatching `executor`. **Once per plan** (not per task), present a 4-option menu (execute / review-first / skip / stop) before starting that plan's tasks. Once executing, tasks run one at a time with only a brief pause after each — the agent stops mid-plan only if the user actually types something, it does not re-show the menu. After all plans, proceed to verification as normal. Full flow (the exact presentation format, the review-first sub-branch): `.ai/library/workflows/execute-phase/detail/elaboration.md` § 1.
...
```bash
DEFAULT_BRANCH=$(workflow_run query git.base-branch 2>/dev/null \
...
```bash
workflow_run query state.begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(workflow_run query phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
VERIFY_STATUS=$(workflow_run query verification status "${PHASE_DIR}" --pick status)
...
3. **No filter is active, and every filtered plan was filtered by `has_summary` alone** (no
   ...
   - **`VERIFY_STATUS == missing`**: the plans are all summarized but the run never reached the
     ...
     independent `workflow_run` calls — nothing it reads is produced only by `execute_waves` or
...
```bash
# workflow:guard=orchestrator-cwd-drift
...
tooling and `/workflow:manager`'s background-completion handler can grep partial
...
@.ai/library/references/execute-phase-wave-guard.md

@.ai/library/references/execute-phase-context-guard.md
...
2.75. **Execute:wave:pre capability dispatch:**
   ...
   ```bash
   WAVE_PRE_HOOKS_JSON=$(workflow_run loop render-hooks execute:wave:pre --raw)
   ```

   **Contribution dispatch:** inject every `kind == "contribution"` fragment per @.ai/library/references/loop-hook-dispatch.md (skip when none); one naming an alternate wave dispatch replaces step 3's inline loop.

   **Step dispatch:** `kind == "step"` per @.ai/library/references/loop-hook-dispatch.md; never blocks or redirects executor spawning. ⚠ Validate `ref.command` in-context before any shell use.
...
3. **Spawn executor agents:**
   ...
   **Executor routing (#1689/#3370).** Per plan, run `.ai/library/workflows/execute-phase/steps/per-plan-executor-routing.md` to set `EXECUTOR_TYPE` for `subagent_type="{EXECUTOR_TYPE}"` below.

   **TDD-applicability resolution (#4266/#4272).** Run `.ai/library/workflows/execute-phase/steps/tdd-applicability-resolution.md`.
   ...
   **Worktree mode** (`USE_WORKTREES` and `USE_WORKTREES_FOR_PLAN` not `false`):
   ...
   EXPECTED_BRANCH=$(git rev-parse --abbrev-ref HEAD)
    if [ "${USE_WORKTREES:-true}" != "false" ] && [ "${USE_WORKTREES_FOR_PLAN:-true}" != "false" ] && [ -z "${WAVE_WORKTREE_MANIFEST:-}" ]; then
     M=$(mktemp "${TMPDIR:-/tmp}/workflow-worktree-wave-XXXXXX") && mv "$M" "$M.json" && WAVE_WORKTREE_MANIFEST="$M.json" || exit 1  # XXXXXX must be path-final on BSD/macOS (#1520)
   ...
   Agent(
    ...
    description="Execute plan {plan_number} of phase {phase_number}",
     ...
     prompt="
       ...
       [workflow:dispatch phase="{phase_number}" plan="{plan_id}"]
       Commit each task atomically. Create SUMMARY.md.
       Do NOT update STATE.md or ROADMAP.md — the orchestrator owns those writes after all worktree agents in the wave complete.
       </objective>

       <worktree_branch_check>
       ORCHESTRATOR build-time embed (NOT a sub-agent runtime step): before this dispatch, read `.ai/library/references/worktree-branch-check.md`, substitute `{EXPECTED_BASE}` with the base SHA captured above ({EXPECTED_BASE}), and replace this note with that fragment's `<worktree_branch_check>` block so the dispatched prompt carries the runnable guard verbatim — do not pass this instruction through in its place.
       Per-commit HEAD/cwd-drift/path-guard: `.ai/library/agents/executor.md` steps 0/0a/0b + `.ai/library/references/worktree-path-safety.md` (in <execution_context>).
       </worktree_branch_check>
       ...
       ${TDD_APPLICABLE ? '- `.ai/library/references/tdd.md`' : ''}  # #3990/#4265: type: tdd, tdd="true", or workflow.tdd_mode
       - `.ai/library/references/worktree-path-safety.md`
       ${CONTEXT_WINDOW < 200000 ? '' : '- `.ai/library/references/executor-examples.md`'}
       </execution_context>
...
4. **Wait for all agents in wave to complete.**
   ...
   **Completion reconciliation (EVERY runtime — any spawn whose terminal response may not arrive):**
   ...
   If a spawned agent does not return a normal terminal completion response — or its session ends abnormally (interrupted, aborted, closed, killed, timed out, `turn_aborted`, including ends the orchestrator itself initiated) — do NOT block indefinitely and do NOT classify the plan as failed yet. Read and execute `.ai/library/workflows/execute-phase/steps/completion-reconciliation.md` — reconcile the plan artifacts FIRST, classify SECOND: SUMMARY present AND matching recent commits → complete (proceed to step 5, do NOT re-dispatch); no completion evidence → the failure handler. Verify, never wait.
...
5. **Post-wave hook validation (parallel mode only):** Hooks run on every executor commit by default (#2924); this post-wave run only fires when `workflow.worktree_skip_hooks=true` opted out of per-commit hooks:
   ...
   if [ "$SKIP_HOOKS" = "true" ]; then
     # Stash uncommitted changes under a named ref so we always pop (bare `git stash` strands them on hook/script failure). #3542: `refs/stash` is shared across worktrees, so this helper runs ONLY in the orchestrator's main checkout after all wave worktrees have been merged + removed; executors are forbidden from running any `git stash` subcommand (see `<destructive_git_prohibition>` in `.ai/library/agents/executor.md`).
     STASHED=false
     if (! git diff --quiet || ! git diff --cached --quiet) && git stash push -u -m "workflow-post-wave-hook-$$" >/dev/null 2>&1; then STASHED=true; fi
     git hook run pre-commit 2>&1 || echo "⚠ Pre-commit hooks failed — review before continuing"
     [ "$STASHED" = "true" ] && (git stash pop >/dev/null 2>&1 || echo "⚠ Could not pop workflow-post-wave-hook stash — recover manually")
   fi
   ```
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   **Manifest source of truth (#3384):** Cleanup consumes the `WAVE_WORKTREE_MANIFEST` created and populated during executor dispatch in step 3. Do not recreate or truncate it here.
   ...
   workflow_run query worktree.cleanup-wave --manifest "$WAVE_WORKTREE_MANIFEST" || exit 1
   ```
   ...
   **Cleanup-tail snippet (use after any wave whose merges did not flow through the templated path above):**
   ...
   WT_PATHS_FILE=$(mktemp "${TMPDIR:-/tmp}/workflow-worktree-paths-XXXXXX")
...
5.6. **Post-merge build & test gate:**
   ...
   Read and execute `.ai/library/workflows/execute-phase/steps/post-merge-gate.md`.
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       workflow_run query roadmap.update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done
     ...
     if ! git diff --quiet .planning/ROADMAP.md .planning/STATE.md 2>/dev/null; then
       workflow_run query commit "docs(phase-${PHASE_NUMBER}): update tracking after wave ${N}" --files .planning/ROADMAP.md .planning/STATE.md
     fi
...
5.75. **Execute:wave:post capability dispatch:**
   ...
   ```bash
   WAVE_POST_HOOKS_JSON=$(workflow_run loop render-hooks execute:wave:post --raw)
   ```
   ...
   **Contribution dispatch:** inject every `kind == "contribution"` fragment per @.ai/library/references/loop-hook-dispatch.md (skip when none), before the gates below.

   **Step dispatch:** dispatch every `kind == "step"` hook per @.ai/library/references/loop-hook-dispatch.md (skip when none) — not one shape of one. A step here is advisory: it never blocks wave completion. ⚠ **Validate `ref.command` in-context before any shell use** (third-party manifest input) — loop-hook-dispatch.md § `step`. **`ref.skill == "code-review"` (#3661):** the generic contract's bare skill dispatch carries no phase argument, but `code-review.md`'s `initialize` step requires one (`PHASE_ARG="${1}"`) or it reports "Phase not found" and exits — pass it explicitly, mirroring step `code_review_gate` below: `Skill(skill="workflow-code-review", args="${PHASE_NUMBER}")`.

   **For each active entry where `kind == "gate"`** (process in array order): read and execute `.ai/library/workflows/execute-phase/steps/wave-post-gate-hooks.md` for the full evaluation contract (check validation, `onError`, blocking semantics, mapper spawn). When all active gates are processed without a blocking halt, continue to step 5.8.
...
7. **Handle failures:**
   **Step 7.0 — classify before branching (#3095):**
   ```bash
   CLASS_JSON=$(workflow_run query agent.classify-failure -- "$AGENT_RETURN_BODY")
...
@.ai/library/references/execute-phase-quota-recovery.md

@.ai/library/references/execute-phase-between-wave-reset.md
...
```bash
AUTO_MODE=$(workflow_run query check auto-mode --pick active 2>/dev/null)
```
...
Dispatch every `kind == "step"` hook per @.ai/library/references/loop-hook-dispatch.md (skip when none). The secure-phase routing below applies when that specific hook is active.
...
⚠ Security enforcement enabled — run before advancing:
  /workflow:secure-phase {PHASE} ${Workflow_WS}
```

If an active secure-phase step hook exists AND SECURITY.md exists: check frontmatter `threats_open`. If > 0:
```
⚠ Security gate: {threats_open} threats open
  /workflow:secure-phase {PHASE} — resolve before advancing
```
</step>

<!-- workflow:section id="partial-wave" when="flag:--wave" -->
If `section_manifest` is `null` or `"partial-wave"` is in its `included` list: read and execute `.ai/library/workflows/execute-phase/steps/partial-wave.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
**Capability gate:**
```bash
EXECUTE_POST_HOOKS_JSON=${EXECUTE_POST_HOOKS_JSON:-$(workflow_run loop render-hooks execute:post --raw)}
```

Dispatch `kind == "step"` hooks per @.ai/library/references/loop-hook-dispatch.md. `ref.skill == "code-review"`:
...
**Invoke review:**
```
Skill(skill="workflow-${ref.skill}", args="${PHASE_NUMBER}")
```
...
/workflow:code-review ${PHASE_NUMBER} --fix
```
...
**TDD review escalation (overrides the advisory default for the `tdd.review-checkpoint` gate only).** The tdd `execute:post` gate is declared `blocking: false`, so by the generic contract above it displays its `message`/table and continues. There is ONE documented exception (see `.ai/library/references/execute-mvp-tdd.md`): when `TDD_MODE=true` AND `GATE_RESULT.block == true` (one or more TDD plans miss a RED or GREEN gate commit; #4011 — no MVP condition), the end-of-phase TDD review escalates from advisory to **blocking under TDD** — refuse to mark the phase complete and present:

```
Phase blocked: {N} TDD plan(s) violate the RED→GREEN gate sequence under TDD.
Resolve and re-run /workflow execute-phase, or override with /workflow execute-phase {phase} --force-mvp-gate to ship anyway.
```
...
<!-- workflow:section id="gap-closure-artifacts" when="state:gap-closure-phase" -->
If `section_manifest` is `null` or `"gap-closure-artifacts"` is in its `included` list: read and execute `.ai/library/workflows/execute-phase/steps/gap-closure-artifacts.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->

<!-- workflow:section id="regression-gate" when="state:has-prior-phases" -->
If `section_manifest` is `null` or `"regression-gate"` is in its `included` list: read and execute `.ai/library/workflows/execute-phase/steps/regression-gate.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
```bash
VERIFIER_SKILLS=$(workflow_run query agent-skills verifier)
```
...
```bash
VERIFICATION=$(workflow_run query verification.status "$PHASE_DIR" 2>/dev/null)
...
Commit the file:
```bash
workflow_run query commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-UAT.md"
```
...
`/workflow:verify-work {X} ${Workflow_WS}`
...
**If user acknowledges without reporting issues (including "ok", "noted", "ack", "got it", "approved", "done", "yes", "pass", or similar):** Stop. The phase remains pending. No further orchestrator action — wait for the user to run `/workflow:verify-work`.

**If user reports issues now:** Proceed to gap closure.

**If gaps_found:**
@.ai/library/references/execute-phase-requirement-revert.md
```
...
`/workflow:plan-phase {X} --gaps ${Workflow_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/workflow:verify-work {X} ${Workflow_WS}` — manual testing first
```

Gap closure cycle: `/workflow:plan-phase {X} --gaps ${Workflow_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/workflow:execute-phase {X} --gaps-only ${Workflow_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(workflow_run query phase.complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/workflow:progress` and `/workflow:audit-uat`.
```

```bash
workflow_run query commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
`/workflow:extract-learnings`) and copies it to the global learnings store at
`~/.workflow/knowledge/`.

**Check config gate:**
```bash
GL_ENABLED=$(workflow_run query config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
workflow_run query learnings.copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
if [ ${#CLOSED[@]} -gt 0 ]; then
  ...
  workflow_run query commit "docs(phase-${PHASE_NUMBER}): close ${#CLOSED[@]} resolved todo(s)" --files "${ADDED[@]}" .planning/STATE.md --files-removed "${REMOVED[@]}" || true
...
@.ai/library/workflows/transition.md
</step>
...
<failure_handling>
- **Quota / rate-limit (any runtime — #3095):** Agent return body contains a sentinel like `usage limit`, `rate limit`, `429`, `too many requests`, `RESOURCE_EXHAUSTED`, `usage_limit_reached`. Route via `workflow_run query agent.classify-failure` → `class: "quota-exceeded"`. Do not offer retry-now; the right action is wait-for-reset and resume.
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not Workflow. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/workflow:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
