<purpose>
Interactive configuration of third-party integrations for Workflow — search API keys
(Brave / Firecrawl / Exa), code-review CLI routing (`review.models.<cli>`), and
agent-skill injection (`agent_skills.<agent-type>`). Writes to
`.planning/config.json` via `workflow-tools` so unrelated keys are
preserved, never clobbered.

This command is deliberately separate from `/workflow:settings` (workflow toggles)
and any `/workflow-settings-advanced` tuning surface. It exists because API keys and
...
- **`config-set` output is masked** for keys in the secret set
  ...
  `workflow-core/bin/lib/secrets.cjs`.
...
Store `$Workflow_CONFIG_PATH`. Every subsequent read/write uses it.
</step>
...
```bash
BRAVE=$(workflow_run query config-get brave_search --raw --default null)
FIRECRAWL=$(workflow_run query config-get firecrawl --raw --default null)
EXA=$(workflow_run query config-get exa_search --raw --default null)
SEARCH_GITIGNORED=$(workflow_run query config-get search_gitignored --raw --default false)
```
...
```bash
workflow_run query config-set brave_search "<value>"     # masked in output
workflow_run query config-set firecrawl "<value>"        # masked in output
workflow_run query config-set exa_search "<value>"       # masked in output
workflow_run query config-set search_gitignored true|false
```

For "Clear", write `null`:

```bash
workflow_run query config-set brave_search null
```
</step>
...
```bash
workflow_run query config-set review.models.<cli> "<model id>"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "executor", description: "Skills injected when spawning executor agents" },
      { label: "planner", description: "Skills injected when spawning planner agents" },
      { label: "verifier", description: "Skills injected when spawning verifier agents" },
...
resolution at spawn time (#3651 — `.ai/library/references/planning-config.md`:
...
```bash
workflow_run query config-set agent_skills.<slug> '["skills/alpha","skills/beta"]'
```
...
Quick commands:
- /workflow:settings — workflow toggles and model profile
- /workflow-set-profile <profile> — switch model profile
```
</step>
...
<success_criteria>
- [ ] Current config read from `$Workflow_CONFIG_PATH`
