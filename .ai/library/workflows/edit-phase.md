...
Examples:
  `/workflow-edit-phase 5`       → phase = 5, force = false
  `/workflow-edit-phase 5 --force` → phase = 5, force = true
  `/workflow-edit-phase 12.1`    → phase = 12.1, force = false

If no argument provided:

```
ERROR: Phase number required
Usage: /workflow-edit-phase <phase-number> [--force]
Example: /workflow-edit-phase 5
Example: /workflow-edit-phase 5 --force
```
...
Run /workflow:new-project to initialize.
```
Exit.
</step>
...
```bash
PHASE_DATA=$(workflow_run query roadmap get-phase "${target}")
```
...
Available phases can be seen with /workflow:progress.
```
...
To edit anyway, run:
  /workflow-edit-phase {target} --force
```
...
```bash
ALL_PHASES=$(workflow_run query roadmap analyze)
```
...
```bash
SCOPE_BEFORE=$(workflow_run query roadmap milestone-scope)
```
...
```bash
SCOPE_AFTER=$(workflow_run query roadmap milestone-scope)
```
...
```bash
workflow_run query state.add-roadmap-evolution \
  --phase {target} \
  --action edited \
...
- `/workflow:progress` — view updated roadmap
- `/workflow:plan-phase {target}` — re-plan this phase (if needed)
- `/workflow:discuss-phase {target}` — discuss implementation approach
