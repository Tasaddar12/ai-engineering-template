...
- executor — executes a specific, scoped code change
</available_agent_types>
...
INIT=$(workflow_run query audit-uat 2>/dev/null || echo "{}")
...
```bash
EXECUTOR_MODEL=$(workflow_run query resolve-model executor --raw)
```
...
**b. Run tests:**
```bash
AUDIT_TEST_CMD=$(workflow_run query config-get workflow.test_command --default "" --raw 2>/dev/null || true)
...
AUDIT_TEST_CMD=$(workflow_run query normalize-test-command "$AUDIT_TEST_CMD" --cwd . 2>/dev/null || echo "$AUDIT_TEST_CMD")
TEST_GATE_TIMEOUT=$(workflow_run query config-get workflow.test_gate_timeout --raw 2>/dev/null || echo "600")
workflow_run run-with-timeout "$TEST_GATE_TIMEOUT" -- bash -c "$AUDIT_TEST_CMD" 2>&1 | tail -20
