...
<required_reading>
@.ai/library/references/continuation-format.md
</required_reading>
...
INIT=$(workflow_run query init.resume)
...
**If `init_incomplete` is true (#4040 — interrupted bootstrap):** `.planning/` exists but initialization never finished — one or more of `REQUIREMENTS.md`, `ROADMAP.md`, `STATE.md` were never created. This is NOT a STATE.md-reconstruction case (there is no project history to reconstruct from). Route to initialization recovery: resume `/workflow:new-project`, which continues from the first missing artifact and keeps the existing PROJECT.md and any already-created artifacts. Do not proceed to load_state.
...
**If `planning_exists` is false:** This is a new project - route to /workflow:new-project
</step>
...
- This is the primary resumption source — structured data from `/workflow:pause-work`
...
📋 [N] pending todos — /workflow:capture --list to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/workflow:execute-phase {phase} ${Workflow_WS})
   OR
1. Discuss Phase 3 context (/workflow:discuss-phase 3 ${Workflow_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/workflow:plan-phase 3 ${Workflow_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show direct next command:
  ...
  **Also available:**
  - `/workflow:discuss-phase [N] ${Workflow_WS}` — gather context first
  - `/workflow:plan-phase --research-phase [N] ${Workflow_WS}` — investigate unknowns
