...
Todos are captured during work sessions with /workflow-add-todo.

---

Would you like to:

1. Continue with current phase (/workflow:progress)
2. Add a todo now (/workflow-add-todo)
```
...
- `/workflow:capture --list` → show all
- `/workflow:capture --list api` → filter to area:api only
</step>
...
- `/workflow:capture --list [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /workflow-add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/workflow-add-phase [description from todo]`
...
workflow_run query commit "docs: start work on todo - [title]" --files .planning/todos/completed/[filename] .planning/STATE.md
```
