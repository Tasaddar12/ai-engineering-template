...
<required_reading>
@.ai/library/references/domain-probes.md
@.ai/library/references/gate-prompts.md
@.ai/library/references/universal-anti-patterns.md
</required_reading>
...
| When | Read |
|---|---|
| `--power` in $ARGUMENTS | `.ai/library/workflows/discuss-phase/modes/power.md` (then exit standard flow) |
| `--all` in $ARGUMENTS | `.ai/library/workflows/discuss-phase/modes/all.md` overlay |
| `--auto` in $ARGUMENTS | `.ai/library/workflows/discuss-phase/modes/auto.md` + `.ai/library/workflows/discuss-phase/modes/chain.md` (auto-advance) |
| `--chain` in $ARGUMENTS | `.ai/library/workflows/discuss-phase/modes/default.md` + `.ai/library/workflows/discuss-phase/modes/chain.md` |
| `--text` in $ARGUMENTS or `workflow.text_mode: true` | `.ai/library/workflows/discuss-phase/modes/text.md` overlay |
| `--batch` in $ARGUMENTS | `.ai/library/workflows/discuss-phase/modes/batch.md` overlay |
| `--analyze` in $ARGUMENTS | `.ai/library/workflows/discuss-phase/modes/analyze.md` overlay |
| ADVISOR_MODE = true (USER-PROFILE.md exists) | `.ai/library/workflows/discuss-phase/modes/advisor.md` |
| no flags above | `.ai/library/workflows/discuss-phase/modes/default.md` |
| in `write_context` step | `.ai/library/workflows/discuss-phase/templates/context.md` |
| in `git_commit` step | `.ai/library/workflows/discuss-phase/templates/discussion-log.md` |
| writing checkpoints | `.ai/library/workflows/discuss-phase/templates/checkpoint.json` |
...
**Text mode** (`--text` or `workflow.text_mode: true`): follow `.ai/library/workflows/discuss-phase/modes/text.md` — do not use AskUserQuestion at all.
</answer_validation>

<process>

**Express path available:** If you already have a PRD or acceptance criteria document, use `/workflow:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
INIT=$(workflow_run query init.phase-op "${PHASE}"); [[ "$INIT" == @file:* ]] && INIT=$(cat "${INIT#@file:}")
AGENT_SKILLS_ADVISOR=$(workflow_run query agent-skills advisor-researcher)
```
...
Use /workflow:progress ${Workflow_WS} to see available phases.
```
Exit workflow.
...
if [ -f "$HOME/.claude/workflow-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
- If `--power` in $ARGUMENTS: `Read(.ai/library/workflows/discuss-phase/modes/power.md)` and execute it end-to-end. Do NOT continue with the steps below.
- Otherwise, continue. Per-flag overlay reads happen at their relevant steps:
  - `--all` → Read `.ai/library/workflows/discuss-phase/modes/all.md` before `present_gray_areas`.
  - `--auto` → Read `.ai/library/workflows/discuss-phase/modes/auto.md` before `check_existing` (it overrides several steps).
  - `--chain` → Read `.ai/library/workflows/discuss-phase/modes/chain.md` before `auto_advance`.
  - `--text` (or `workflow.text_mode: true`) → Read `.ai/library/workflows/discuss-phase/modes/text.md` before any AskUserQuestion call.
  - `--batch` → Read `.ai/library/workflows/discuss-phase/modes/batch.md` before `discuss_areas`.
  - `--analyze` → Read `.ai/library/workflows/discuss-phase/modes/analyze.md` before `discuss_areas`.
  - `ADVISOR_MODE = true` → Read `.ai/library/workflows/discuss-phase/modes/advisor.md` before `analyze_phase` (it changes the discussion flow and adds an `advisor_research` substep).
  - No flags → Read `.ai/library/workflows/discuss-phase/modes/default.md` before `discuss_areas`.
...
Check if a SPEC.md (from `/workflow:spec-phase`) exists for this phase. SPEC.md locks requirements before implementation decisions.
...
**Note:** SPEC.md files named `AI-SPEC.md` (from `/workflow:ai-integration-phase`) are excluded — different purpose.
</step>
...
If raw spikes/sketches exist but no findings skill, note: `⚠ Unpackaged spikes/sketches detected — run /workflow:spike --wrap-up or /workflow:sketch --wrap-up to make findings available.`
...
```bash
TODO_MATCHES=$(workflow_run query todo.match-phase "${PHASE_NUMBER}")
```
...
Read `@.ai/library/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```bash
DISCUSS_PRE_HOOKS_JSON=$(workflow_run loop render-hooks discuss:pre --raw)
```
Apply each entry in `activeHooks` per @.ai/library/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
- **Advisor mode (ADVISOR_MODE = true):** follow `.ai/library/workflows/discuss-phase/modes/advisor.md` — research-backed comparison tables, table-first selection.
- **--auto:** follow `.ai/library/workflows/discuss-phase/modes/auto.md` — Claude picks recommended option for every question; no AskUserQuestion. Single-pass cap enforced.
- **Default (no flags):** follow `.ai/library/workflows/discuss-phase/modes/default.md` — 4 single-question turns per area, then check whether to continue.

Overlays (combine with the active mode):
- `--text` → `.ai/library/workflows/discuss-phase/modes/text.md` (replace AskUserQuestion with plain-text numbered lists)
- `--batch` → `.ai/library/workflows/discuss-phase/modes/batch.md` (group 2–5 questions per turn)
- `--analyze` → `.ai/library/workflows/discuss-phase/modes/analyze.md` (trade-off table before each question)
...
- **Incremental checkpoint** — after each area completes, write `${phase_dir}/${padded_phase}-DISCUSS-CHECKPOINT.json`. Read `.ai/library/workflows/discuss-phase/templates/checkpoint.json` for the schema. The checkpoint is structured state, not the canonical CONTEXT.md (`write_context` produces the canonical output). On session resume, the parent's `check_existing` step detects the checkpoint and offers to resume.
...
```
Read(.ai/library/workflows/discuss-phase/templates/context.md)
```
...
```bash
DISCUSS_POST_HOOKS_JSON=$(workflow_run loop render-hooks discuss:post --raw)
```
Apply each entry in `activeHooks` per @.ai/library/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
...
`/workflow:plan-phase ${PHASE} ${Workflow_WS}`

---

**Also available:** `--chain` for auto plan+execute after; `/workflow:plan-phase ${PHASE} --skip-research ${Workflow_WS}` to plan without research; `/workflow:ui-phase ${PHASE} ${Workflow_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
...
```
Read(.ai/library/workflows/discuss-phase/templates/discussion-log.md)
```
...
```bash
workflow_run query commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
workflow_run query commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>

<step name="auto_advance">
Auto-advance behavior is defined in `.ai/library/workflows/discuss-phase/modes/chain.md`.
