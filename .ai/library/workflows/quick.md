<purpose>
Execute small, ad-hoc tasks with Workflow guarantees (atomic commits, STATE.md tracking). Quick mode spawns planner (quick mode) + executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
- code-reviewer — Reviews source files for bugs, security issues, and code quality
</available_agent_types>
...
```
### Workflow ► QUICK TASK (DISCUSS + VALIDATE)
...
```
### Workflow ► QUICK TASK (DISCUSS + RESEARCH)
...
```
### Workflow ► QUICK TASK (RESEARCH + VALIDATE)
...
INIT=$(workflow_run query init.quick "$DESCRIPTION" $DISCUSS_PARAM $RESEARCH_PARAM $VALIDATE_PARAM $FULL_PARAM)
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/workflow:new-project` first.
...
**Step 4 ordering (#3894):** Check whether `workflow.research_before_questions` is enabled in `.planning/config.json` (or the config from init context) — the same check `/workflow:discuss-phase` and `/workflow:new-project` already make. When **enabled**, execute the research-phase section BELOW BEFORE the discussion-phase section: a gray-area answer given without research is written to `<quick_id>-CONTEXT.md` as a locked decision downstream agents are told not to revisit, so the evidence must come first. When **false or unset**, keep the written order (discussion, then research) — behavior unchanged.

<!-- workflow:section id="discussion-phase" when="flag:--discuss" -->
If `section_manifest` is `null` or `"discussion-phase"` is in its `included` list: read and execute `.ai/library/workflows/quick/steps/discussion-phase.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->

---

<!-- workflow:section id="research-phase" when="flag:--research" -->
If `section_manifest` is `null` or `"research-phase"` is in its `included` list: read and execute `.ai/library/workflows/quick/steps/research-phase.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
<!-- workflow:section id="plan-checker-loop" when="flag:--validate" -->
If `section_manifest` is `null` or `"plan-checker-loop"` is in its `included` list: read and execute `.ai/library/workflows/quick/steps/plan-checker-loop.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->

---

<!-- workflow:section id="worktree-pre-dispatch-commit" when="state:worktrees-enabled" -->
If `section_manifest` is `null` or `"worktree-pre-dispatch-commit"` is in its `included` list: read and execute `.ai/library/workflows/quick/steps/worktree-pre-dispatch-commit.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
if [ "$ISOLATION" = "harness-worktree" ] && [ "${USE_WORKTREES:-true}" != "false" ]; then
  _QUICK_SHOULD_DEGRADE=$(workflow_run query worktree.base-check --mode "$ISOLATION" --pick shouldDegrade 2>/dev/null || true)
  if [ "$_QUICK_SHOULD_DEGRADE" = "true" ]; then
    _QUICK_DEGRADE_MSG=$(workflow_run query worktree.base-check --mode "$ISOLATION" --pick message 2>/dev/null || true)
...
# (hooks/workflow-agent-isolation-guard.js, hooks/workflow-cursor-subagent-start.js via
...
if [ "$ISOLATION" = "harness-worktree" ]; then   # keyed on ISOLATION like every other dispatch-coupled branch (#2652)
  ...
  QUICK_WORKTREE_MANIFEST=$(mktemp "${TMPDIR:-/tmp}/workflow-quick-worktree-XXXXXX") && mv "$QUICK_WORKTREE_MANIFEST" "${QUICK_WORKTREE_MANIFEST}.json" && QUICK_WORKTREE_MANIFEST="${QUICK_WORKTREE_MANIFEST}.json" || exit 1
...
ORCHESTRATOR build-time embed (NOT a sub-agent runtime step): before this dispatch, read \`.ai/library/references/worktree-branch-check.md\`, substitute \`{EXPECTED_BASE}\` with the base SHA captured above (${EXPECTED_BASE}), substitute \`{EXPECTED_BASE_ALTERNATE}\` with \`${QUICK_PLAN_PARENT}\` when it differs from \`${EXPECTED_BASE}\` (otherwise empty), and replace this note with that fragment's \`<worktree_branch_check>\` block so the dispatched prompt carries the runnable guard verbatim — do not pass this instruction through in its place.
</worktree_branch_check>
...
1. **Worktree cleanup:** If the executor ran isolated (`ISOLATION = "harness-worktree"` at dispatch), merge the worktree branch back and clean up:
   ...
   workflow_run query worktree.cleanup-wave --manifest "$QUICK_WORKTREE_MANIFEST" || exit 1
   ```
...
**Generic step dispatch (#3606):** dispatch every `kind == "step"` hook from `EXECUTE_POST_HOOKS_JSON` per @.ai/library/references/loop-hook-dispatch.md (skip silently when none); each step is advisory and best-effort. The code-review specialization below is one such hook, not a replacement for the generic dispatch.
...
<!-- workflow:section id="quick-verification" when="flag:--validate" -->
If `section_manifest` is `null` or `"quick-verification"` is in its `included` list: read and execute `.ai/library/workflows/quick/steps/quick-verification.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
**Note:** If the table already exists in a legacy (pre-registry) column format, first run `workflow_run quick-tasks-migrate` — the maintainer-decided repair path (#3730) that rewrites the table onto the canonical schema, losslessly bucketing unmapped columns into Description. It is a silent no-op when the table is already canonical or the section is absent, so running it before the first append of every quick task migrates exactly once and never prompts otherwise. After migration, use the canonical column format below.
...
For a schema-safe append outside this workflow (e.g. from fast.md, which has neither a quick id nor a task directory), `workflow_run quick-tasks-append --task <text>` performs an equivalent-shape write via the shared, schema-backed `appendQuickTaskRow` helper (#2133, ADR-2143 §3/§7) — the `#` cell is a positional ordinal and `Directory` reads `—`, since no id/directory was supplied. A caller that DOES have a real `${quick_id}` and task directory can pass `--quick-id <id> --slug <slug>` (or `--directory <link>` directly) to get the byte-identical row this step renders above (#3356).
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `workflow_run query commit` command handles already-committed files gracefully.
...
workflow_run query commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
Workflow > QUICK TASK COMPLETE (VALIDATED)
...
Ready for next task: /workflow:quick ${Workflow_WS}
```
