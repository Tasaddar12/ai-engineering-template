...
item is a no-op, same guarantee `/workflow:quick` relies on):
...
workflow_run quick-batch complete \
  --batch "$BATCH_ID" \
  --quick-id "$quick_id" \
...
if [ "$COMMIT_DOCS" != "false" ]; then
  ...
  workflow_run query commit "docs(quick-batch-${BATCH_ID}): ${ITEM_COUNT} item(s)" --files "${BATCH_ARTIFACT_FILES[@]}"
fi
```

**Final report.** Re-load the batch (`workflow_run quick-batch resume --batch
"$BATCH_ID" --raw` — read-only in effect when nothing changed) and summarize
by status:

```
---
Workflow > QUICK BATCH COMPLETE
...
${failed_count + human_needed_count > 0 ? 'Resume after resolving: /workflow:quick-batch --resume ' + BATCH_ID : ''}
---
```
...
`/workflow:quick-batch --resume ${BATCH_ID}` — `resumeBatch`'s own propagation
