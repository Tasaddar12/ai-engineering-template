...
iterations — identical cap to `/workflow:quick --validate`'s own loop
(`.ai/library/workflows/quick/steps/plan-checker-loop.md`), just run per item
...
Display banner:
```
### Workflow ► CHECKING PLAN ${quick_id}
...
`/workflow:quick`'s own loop — an entry with a missing/unrecognized severity
...
working around it. Full contract: `.ai/library/references/planner-revision.md`.
