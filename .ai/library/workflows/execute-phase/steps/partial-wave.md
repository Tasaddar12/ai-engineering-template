...
POST_PLAN_INDEX=$(workflow_run query phase-plan-index "${PHASE_NUMBER}")
```
...
/workflow:execute-phase {phase} ${Workflow_WS}                # Continue remaining waves
/workflow:execute-phase {phase} --wave {next} ${Workflow_WS}  # Run the next wave explicitly
```
