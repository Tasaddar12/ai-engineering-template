...
@.ai/library/references/dispatch-isolation-gate.md, the canonical statement of the
...
# as .ai/library/references/dispatch-isolation-gate.md does — this site declares that gate
...
_ISOLATION_RAW=$(workflow_run query dispatch-isolation --raw --phase "${PHASE_NUMBER:-}" 2>/dev/null)
_ISOLATION_RC=$?
...
if [ "$ISOLATION" = "none" ] && [ "$USE_WORKTREES" != "false" ]; then
  if [ "$ISOLATION_RESOLVED" = "true" ]; then
    ...
    echo "FATAL: could not resolve this runtime's executor-isolation capability — 'workflow_run query dispatch-isolation' failed or returned nothing, so Workflow cannot tell whether isolation is available. Refusing to dispatch rather than guess (a guard that cannot verify must not answer 'safe'). Re-run once the workflow-tools shim resolves, or set workflow.use_worktrees=false to run sequentially on purpose." >&2
  fi
  exit 1
fi

# Sweep orphaned locked worktrees from prior crashed sessions (#3707).
[ "$ISOLATION" != "none" ] && workflow_run query worktree.reap-orphans 2>/dev/null || true
# Auto-degrade if HEAD diverged from the fork base (#683) — both isolation models.
if [ "$ISOLATION" != "none" ]; then
  _SHOULD_DEGRADE=$(workflow_run query worktree.base-check --mode "$ISOLATION" --pick shouldDegrade 2>/dev/null || true)
  if [ "$_SHOULD_DEGRADE" = "true" ]; then
    _DEGRADE_MSG=$(workflow_run query worktree.base-check --mode "$ISOLATION" --pick message 2>/dev/null || true)
...
# base-check degrade is decided here, not inside workflow-tools.cjs) through the
...
# guard hooks (hooks/workflow-agent-isolation-guard.js,
# hooks/workflow-cursor-subagent-start.js) read this sentinel instead of
...
# precise.
workflow_run query dispatch-isolation --raw --phase "${PHASE_NUMBER:-}" --force-isolation "$ISOLATION" >/dev/null 2>&1 || true
```
...
|---|---|---|
| `harness-worktree` | host-driven | Pass the host's own declared isolation flag (`harnessFlag`) on each executor dispatch and let the harness create + bind the worktree. Workflow runs no git. |
| `orchestrator-worktree` | Workflow-driven | Workflow creates the worktree (`worktree create`), then process-spawns the executor bound to it via the resolved `exec` argv/cwd. Workflow performs all git operations. |
...
# workflow-core/bin/workflow-tools.cjs) — there is no separate "now also record the
# flag" step, and therefore no flagless window between recording the mode and
# recording the flag.
HARNESS_FLAG=$(workflow_run query dispatch-isolation --json --phase "${PHASE_NUMBER:-}" 2>/dev/null \
...
#      never a skip — a child without these texts is not a executor.
...
#      decision.
#   3. Inline the executor ROLE DEFINITION: read `.ai/library/agents/executor.md`
...
#      agent_skills for executor, and only an UNCONFIGURED non-Claude
...
_TDD_APPLICABLE_RAW=$(workflow_run query phase.tdd-applicable "{phase_dir}/{plan_file}" --pick applicable 2>/dev/null)
_TDD_APPLICABLE_RC=$?
if [ $_TDD_APPLICABLE_RC -ne 0 ]; then
  echo "FATAL: could not resolve TDD-applicability for plan {plan_number} — 'workflow_run query phase.tdd-applicable' failed. Refusing to guess whether this dispatch needs the TDD procedure. Halting." >&2
  exit 1
fi
...
<execution_context>
You are the executor agent running in a git worktree Workflow created for you.
...
- .ai/library/agents/executor.md (the ROLE DEFINITION you are executing — its steps
...
CREATE_JSON=$(workflow_run query worktree.create \
...
EXEC_JSON=$(workflow_run query dispatch-isolation --json \
  --cwd-target "$WT_PATH" \
...
`worktree create` records the entry in `$WAVE_WORKTREE_MANIFEST` itself, so **do not** call `worktree.record-agent` for these plans — that verb is the harness-path counterpart, used because the harness creates the worktree behind Workflow's back. Double-recording is deduped by path+branch, but the create verb is the single writer here.
...
The executor never touches `STATE.md`/`ROADMAP.md`, and that guard needs no new code — `execute-plan` auto-detects worktree mode via the `IS_WORKTREE` (`.git`-is-a-file) primitive, which a Workflow-created worktree trips identically to a harness-created one.
