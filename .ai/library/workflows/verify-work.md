...
-->
<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /workflow:plan-phase --gaps.
...
**Compact Content Gate.** Read and follow `.ai/library/references/compact-content-gate.md` now — it states the `workflow.compact_content` check and the resolution rule this spine defers to. When it directs a Read, read `.ai/library/workflows/verify-work/detail/elaboration.md` in full before continuing past this point; its content elaborates on the resume/reconcile steps and the full gap-closure sub-flow below.
...
INIT=$(workflow_run query init.verify-work "${PHASE_ARG}" ${Workflow_WS})
...
MVP_MODE=$(workflow_run query phase.mvp-mode "${phase_number}" ${Workflow_WS} --pick active)
```
</step>
...
```bash
VERIFY_PRE_HOOKS_JSON=$(workflow_run loop render-hooks verify:pre --raw)
...
**Contribution dispatch:** inject every `kind == "contribution"` fragment per @.ai/library/references/loop-hook-dispatch.md (skip when none), before the steps and gates below.
...
`workflow_run check ${hook.check.query} "${PHASE_DIR}" --raw`; a `predicate` gate
runs `workflow_run check predicate --predicate '<hook.check.predicate as JSON>' --phase-dir "${PHASE_DIR}" --raw`):

```bash
GATE_RESULT=$(workflow_run check "${hook_check_query}" "${PHASE_DIR}" --raw)
...
- **Step 1 — command failure:** if the `workflow_run check ...` invocation itself
...
Provide a phase number to start testing (e.g., /workflow:verify-work 4)
```
...
<!-- workflow:section id="automated-ui-verification" when="state:ui-phase-active" -->
If `section_manifest` is `null` or `"automated-ui-verification"` is in its `included` list: read and execute `.ai/library/workflows/verify-work/steps/automated-ui-verification.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
<!-- workflow:section id="mvp-uat-framing" when="state:phase-mvp-mode" -->
If `section_manifest` is `null` or `"mvp-uat-framing"` is in its `included` list: read and execute `.ai/library/workflows/verify-work/steps/mvp-uat-framing.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
```bash
COVERAGE=$(workflow_run query uat.classify-coverage --summary "$SUMMARY_FILE")
```
...
```bash
CHECKPOINT=$(workflow_run query uat.render-checkpoint --file "$uat_path" --raw)
...
Exact resume-announcement wording: `.ai/library/workflows/verify-work/detail/elaboration.md` § 2.
</step>
...
Commit the UAT file:
```bash
workflow_run query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
**Generic step dispatch:** dispatch every `kind == "step"` hook from `VERIFY_POST_HOOKS_JSON` per @.ai/library/references/loop-hook-dispatch.md (skip silently when none). Each step is advisory and best-effort — honor `onError` and continue. The secure-phase handling below is an additional specialization of one such hook, not a replacement for the generic dispatch.
...
```
Skill(skill="workflow-${ref.skill}", args="{phase}")
```
...
```
⚠ Security enforcement enabled — /workflow:secure-phase {phase} did not produce SECURITY.md.
...
- `/workflow:secure-phase {phase}` — security review (required before advancing)
- `/workflow:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
...
```bash
PHASE_DIR=$(printf '%s' "$INIT" | jq -r '.phase_dir // empty')
VERIFICATION_FILE=$(workflow_run query verification.resolve-file "$PHASE_DIR" --raw 2>/dev/null)
VERIFICATION_STATUS=$(workflow_run query verification.status "$PHASE_DIR" 2>/dev/null)
...
if [ "$VERIFICATION_STATUS_VALUE" = "human_needed" ]; then
  workflow_run query frontmatter.set "$VERIFICATION_FILE" --field status --value passed
fi
```
...
- `/workflow:verify-work {phase}` — re-run verification against the latest summaries
```

Otherwise, check the shared UAT-plus-verification completion predicate before transition:

```bash
PHASE_COMPLETE=$(workflow_run phase uat-passed "{phase}" --require-verification)
...
- `/workflow:execute-phase {phase}` — regenerate execution verification
- `/workflow:verify-work {phase}` — resume UAT if blockers remain
```
...
Read and follow `.ai/library/workflows/transition.md`.
...
- `/workflow:plan-phase {next}` — Plan next phase
- `/workflow:execute-phase {next}` — Execute next phase
- `/workflow:secure-phase {phase}` — security review
- `/workflow:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
`audit-open` is CJS-only until registered on `workflow_run query`:

```bash
workflow_run query audit-open --json
```
...
If user declines: stop. User resolves items and re-runs `/workflow:verify-work`.
...
Spawn planner in --gaps mode against the UAT (with diagnoses), `{state_path}` (Project State), and `{roadmap_path}` (Roadmap). Each created PLAN.md MUST carry `gap_closure: true` and `gap_ids: [...]` in its frontmatter (#1921) so a later verify-work resume can reconcile it.
...
Spawn plan-checker against the fix plans (iteration_count starts at 1), model="{checker_model}" (omit on inherit/empty, #2517).
...
Exact Agent() prompt fields: `.ai/library/workflows/verify-work/detail/elaboration.md` § 2.
</step>
...
`.ai/library/references/planner-revision.md`, which you load in revision mode.
...
3. Abandon (exit, user runs /workflow:plan-phase manually)

Then wait for the user to pick one.

Exact Agent() prompt fields (revision_context, required_reading): `.ai/library/workflows/verify-work/detail/elaboration.md` § 3.
</step>
...
`/clear` then `/workflow:execute-phase {phase} --gaps-only`
...
- [ ] Ready for `/workflow:execute-phase --gaps-only` when complete
</success_criteria>
