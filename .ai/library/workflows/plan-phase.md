...
-->
<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates phase-researcher, planner, and plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@.ai/library/references/revision-loop.md
@.ai/library/references/gate-prompts.md
@.ai/library/references/agent-contracts.md
@.ai/library/references/gates.md
</required_reading>

<available_agent_types>
Valid Workflow subagent types (use exact names — do not fall back to 'general-purpose'):
- phase-researcher — Researches technical approaches for a phase
- pattern-mapper — Analyzes codebase for existing patterns, produces PATTERNS.md
- planner — Creates detailed plans from phase scope
- plan-checker — Reviews plan quality before execution
</available_agent_types>

<runtime_compatibility>
**Subagent spawning — top-level Claude Code:**
The Agent tool IS available in a top-level Claude Code session. Always spawn
phase-researcher, planner, and plan-checker as separate Agent() calls.
...
Agent() call for phase-researcher, planner, and plan-checker. Only
...
Read and follow `.ai/library/references/compact-content-gate.md` now — it states the `workflow.compact_content` check and the resolution rule this spine defers to. When it directs a Read, read `.ai/library/workflows/plan-phase/detail/elaboration.md` in full before continuing past this point; its content elaborates on several steps below.
...
INIT=$(workflow_run query init.plan-phase "$PHASE" $GRAN_PARAM $PRD_PARAM $INGEST_PARAM $RESEARCH_PHASE_PARAM $REVIEWS_PARAM $CHUNKED_PARAM)
...
CONTEXT_WINDOW=$(workflow_run query config-get context_window --raw 2>/dev/null || echo "200000")
MVP_MODE_CFG=$(workflow_run query config-get workflow.mvp_mode --raw 2>/dev/null || echo "false")
```

When the tdd capability's `workflow.tdd_mode` is active (resolved via the plan:pre render-hooks), the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `.ai/library/references/tdd.md`. The TDD guidance is injected via the tdd capability's contribution hook at §5.6; no inline config-get is needed.
...
if [[ "$ARGUMENTS" =~ (^|[[:space:]])--tdd([[:space:]]|$) ]]; then
  workflow_run query config-set workflow.tdd_mode true 2>/dev/null || true
fi
...
if [ "$MVP_MODE" = "true" ] && [ "$padded_phase" = "01" ]; then
  PRIOR_SUMMARIES=$(workflow_run query phases.list --type summaries --pick count 2>/dev/null)
  if [ "$PRIOR_SUMMARIES" = "0" ]; then WALKING_SKELETON=true; fi
fi
```

When `WALKING_SKELETON=true`:
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `.ai/library/references/skeleton-template.md` — the planner reads it when producing SKELETON.md (lazy; not loaded on non-skeleton runs).
...
```bash
CHUNKED_CFG=$(workflow_run query config-get workflow.plan_chunked --raw 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
<!-- workflow:section id="reviews-prerequisite" when="flag:--reviews" -->
If `section_manifest` is `null` or `"reviews-prerequisite"` is in its `included` list: read and execute `.ai/library/workflows/plan-phase/steps/reviews-prerequisite.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
```bash
MVP_MODE=$(workflow_run query phase.mvp-mode "${PHASE}" $MVP_FLAG_ARG --pick active)
```

<!-- workflow:section id="prd-express-gate" when="flag:--prd" -->
If `section_manifest` is `null` or `"prd-express-gate"` is in its `included` list: read and execute `.ai/library/workflows/plan-phase/steps/prd-express-gate.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->

<!-- workflow:section id="adr-ingest-express-path" when="flag:--ingest" -->
If `section_manifest` is `null` or `"adr-ingest-express-path"` is in its `included` list: read and execute `.ai/library/workflows/plan-phase/steps/adr-ingest-express-path.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /workflow:plan-phase {X} ${Workflow_WS}:

  /workflow:discuss-phase {X} ${Workflow_WS}
  ```
...
If `AI_SPEC_FILE` is non-empty, pass `AI_SPEC_PATH` and `FRAMEWORK_LINE` to the planner in step 8 so it can reference the AI design contract. If it is empty, the active `ai-integration` capability hook in step 5.6 handles any AI-system nudge or `/workflow:ai-integration-phase` dispatch.
...
```bash
DRIFT=$(workflow_run verify context-drift "${PHASE}" 2>/dev/null || echo '{"skipped":true}')
```
...
- If `DRIFT.block` is `true` (opt-in `workflow.context_drift_action: block`): **exit the
  ...
  `/workflow:plan-phase {X}` once the named artifacts are regenerated, or at disabling the check with
  `workflow_run query config-set workflow.context_drift_action warn` if the flag was a false positive.
...
<!-- workflow:section id="research-only-modifiers" when="flag:--research-phase" -->
If `section_manifest` is `null` or `"research-only-modifiers"` is in its `included` list: read and execute `.ai/library/workflows/plan-phase/steps/research-only-modifiers.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
```bash
if workflow_run query teams-status --active >/dev/null 2>&1; then
  echo "⚠️  CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS detected. Workflow's multi-agent orchestration is not validated under claude-code agent-teams and may stall (a subagent's completion can fail to route to the orchestrator). Recommend disabling agent-teams for Workflow workflows. See .ai/library/SOURCES.md#source-e913f58c6c12f8fe" >&2
fi
```

```bash
PHASE_DESC=$(workflow_run query roadmap.get-phase "${PHASE}" --pick section)
if [ -z "${PLAN_PRE_HOOKS_JSON:-}" ]; then
  PLAN_PRE_HOOKS_JSON=$(workflow_run loop render-hooks plan:pre --raw)
fi
```
...
<!-- workflow:section id="research-only-early-exit" when="flag:--research-phase" -->
If `section_manifest` is `null` or `"research-only-early-exit"` is in its `included` list: read and execute `.ai/library/workflows/plan-phase/steps/research-only-early-exit.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
**If found:**
1. Read template: `.ai/templates/VALIDATION.md`
...
**Contribution dispatch (#3606):** inject every `kind == "contribution"` fragment from `PLAN_PRE_HOOKS_JSON` per @.ai/library/references/loop-hook-dispatch.md, in array order, into the role each entry's `into` names — planner-targeted ones land in the prompt block below, orchestrator-targeted ones in your working context. The security specialization below is one such contribution, not a replacement for the generic dispatch.
...
```
### Workflow ► SECURITY THREAT MODEL REQUIRED (ASVS L{SECURITY_ASVS})
...
```bash
PLAN_PRE_HOOKS_JSON=${PLAN_PRE_HOOKS_JSON:-$(workflow_run loop render-hooks plan:pre --raw)}
...
- If `ref.skill` is set, dispatch with `Skill(skill="workflow-${ref.skill}", args="${PHASE} --auto ${Workflow_WS}")` when pipeline mode allows auto-chaining. Prepend `workflow-` to `ref.skill` — `ui-phase` → `workflow-ui-phase`.
...
- In pipeline / `--auto` mode, invoke the hook's `ref.skill` via `Skill(skill="workflow-${ref.skill}", args="${PHASE} --auto ${Workflow_WS}")`.
- In manual mode, display the existing non-blocking `/workflow:ai-integration-phase {N}` recommendation and let the user continue planning without AI-SPEC or stop to run the capability workflow first.
...
```bash
GATE=$(workflow_run check ui-plan-gate "${PHASE}" --raw)
```
...
```bash
AUTO_CHAIN=$(workflow_run query check auto-mode --pick auto_chain_active 2>/dev/null)
...
```
Skill(skill="workflow-${ref.skill}", args="${PHASE} --auto ${Workflow_WS}")
```
...
`/workflow:ui-phase {N} ${Workflow_WS}` — generate UI design contract before planning

---
Also available:
- `/workflow:plan-phase {N} --skip-ui ${Workflow_WS}` — plan without UI-SPEC (not recommended for frontend phases)
```
...
```bash
DRIFT=$(workflow_run verify codebase-drift 2>/dev/null || echo '{"skipped":true}')
```
...
verbatim (it ends with a `/workflow:map-codebase` pointer) and continue — planning proceeds whether or not the
...
STATE_PATH=$(_workflow_field "$INIT" state_path)
ROADMAP_PATH=$(_workflow_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_workflow_field "$INIT" requirements_path)
RESEARCH_PATH=$(_workflow_field "$INIT" research_path)
VERIFICATION_PATH=$(_workflow_field "$INIT" verification_path)
UAT_PATH=$(_workflow_field "$INIT" uat_path)
CONTEXT_PATH=$(_workflow_field "$INIT" context_path)
REVIEWS_PATH=$(_workflow_field "$INIT" reviews_path)
PATTERNS_PATH=$(_workflow_field "$INIT" patterns_path)
...
PHASE_DIR_FOR_SPEC=$(_workflow_field "$INIT" phase_dir)
...
1. Re-run: `/workflow:plan-phase {PHASE} --research ${Workflow_WS}`
2. Disable Nyquist with the exact command:
   `workflow_run query config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 7.8 (or Step 8 if pattern mapper is disabled) only if user selects 2 or 3.

## 7.8. Spawn pattern-mapper Agent (Optional)

Pattern mapper activation is owned by the `pattern-mapper` capability's `plan:pre` step hook. Read `PLAN_PRE_HOOKS_JSON` and skip if no active step hook has `capId == "pattern-mapper"` and `ref.agent == "pattern-mapper"`. Also skip if no CONTEXT.md and no RESEARCH.md exist for this phase (nothing to extract file lists from).

**If PATTERNS.md already exists** (`PATTERNS_PATH` is non-empty from step 7): Skip to step 8 (use existing).

Display banner:
```
### Workflow ► PATTERN MAPPING PHASE {X}
...
```bash
workflow_run intel api-surface
...
after `$SPEC_FILE` (Step 7), before the planner spawn (Step 8).

**Read and run** the gate + edge probe in `.ai/library/references/specless-probe-fallback.md`
...
Read+execute `.ai/library/workflows/plan-phase/steps/stall-detection-helpers.md` (defines
`workflow_stall_should_recover`/`workflow_stall_watch`, and how `{outputFile}` below is bound;
...
- {context_path} (USER DECISIONS from /workflow:discuss-phase)
...
**If Mode is reviews:** REVIEWS.md is feedback input, not a hidden execution contract. /workflow:execute-phase primarily consumes PLAN.md plus the normal phase context, so every current actionable review finding must become visible in the relevant PLAN.md before planning can pass.
...
- explicitly document a deferral/rejection rationale in the relevant PLAN.md, using the Review Dispositions Ledger in `.ai/library/references/planner-reviews.md`.
...
never a gitignored install/runtime mirror (e.g. `<root>/.workflow/capabilities/<id>/...`
...
(`plugins/*/.workflow/capabilities/<id>/...`, root `capabilities/<id>/...`). A
...
Rules + worked examples: @.ai/library/references/planner-failing-direction.md
</failing_direction_contract>
<!-- workflow:protected:end -->
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `.ai/library/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
**WALKING_SKELETON:** ${WALKING_SKELETON} (when true, the first deliverable must be a Walking Skeleton — Read the template at `.ai/library/references/skeleton-template.md` and produce SKELETON.md alongside PLAN.md.)
**Granularity:** {granularity}

${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `.ai/library/references/planner-mvp-mode.md` now and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
...
else-branch below, per §A/§B/§C of `.ai/library/references/specless-probe-fallback.md`
...
**ORCHESTRATOR RULE — ALL RUNTIMES:** `TS=$(date +%s)`; repeat `PLANNER_STALL_RESULT=$(workflow_stall_watch "$TS" "{outputFile}" "${PHASE_DIR}"'/*-PLAN.md' "## PLANNING COMPLETE" "## PHASE SPLIT RECOMMENDED" "## ⚠ Source Audit" "## CHECKPOINT REACHED" "## PLANNING INCONCLUSIVE")` while waiting/active — `marker_received` -> step 9; `stalled` -> 9a.

**If `CHUNKED_MODE` is `true`:** Skip the Agent() call above — proceed to step 8.5 instead.

<!-- workflow:section id="chunked-planning-mode" when="state:chunked-mode" -->
If `section_manifest` is `null` or `"chunked-planning-mode"` is in its `included` list: read and execute `.ai/library/workflows/plan-phase/steps/chunked-planning-mode.md`. Otherwise skip — do not read the file.
<!-- /workflow:section -->
...
```bash
VERIFY_PATHS=$(workflow_run check verify-command-paths "${PHASE}" --raw)
FAILING_DIRECTIONS=$(workflow_run check verify-failure-directions "${PHASE}" --raw)
```
...
**ORCHESTRATOR RULE — ALL RUNTIMES:** `TS=$(date +%s)`; repeat `CHECKER_STALL_RESULT=$(workflow_stall_watch "$TS" "{outputFile}" "${PHASE_DIR}"'/*-PLAN.md' "## VERIFICATION PASSED" "## ISSUES FOUND")` while waiting/active.
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/workflow:debug` to investigate root causes."
...
`.ai/library/references/planner-revision.md`.
...
**ORCHESTRATOR RULE — ALL RUNTIMES:** (7.99; no marker, mtimes only) `TS=$(date +%s)`; repeat `PLANNER_STALL_RESULT=$(workflow_stall_watch "$TS" "{outputFile}" "${PHASE_DIR}"'/*-PLAN.md')` while waiting/active — `stalled` -> 1) Accept as revised, to step 13, 2) Retry, 3) Stop.

**If the planner returns `## REVISION_CONFLICT`:** follow the shared Conflict Return protocol in
`.ai/library/references/revision-loop.md`, with this workflow's bindings:

```bash
if ! CONVERGENCE_ENABLED=$(workflow_run query config-get workflow.plan_review_convergence --raw 2>/dev/null); then
...
if [ "${CONVERGENCE_ENABLED}" = "true" ] && [ -n "${REVIEWS_FILE}" ]; then
  ...
  END='<!-- workflow:plan-revision-conflicts:end -->'
...
```bash
GATE_CFG=$(workflow_run query config-get workflow.context_coverage_gate --raw 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  ...
  if [ -n "$CONTEXT_PATH" ]; then
    GATE_RESULT=$(workflow_run query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
```bash
workflow_run query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
workflow_run query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
workflow_run query commit "docs(${PADDED_PHASE}): create phase plan" --files "${PHASE_DIR}"/*-PLAN.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
PLAN_POST_HOOKS_JSON=$(workflow_run loop render-hooks plan:post --raw)
PHASE_REQ_IDS=$(workflow_run query init.plan-phase "$PHASE" --pick phase_req_ids 2>/dev/null)
...
**Step and contribution dispatch:** dispatch every `kind == "step"` hook and inject every `kind == "contribution"` fragment per @.ai/library/references/loop-hook-dispatch.md (skip each kind silently when none), before gate evaluation below.
...
```bash
# named-query gate:
GATE_RESULT=$(workflow_run check ${hook.check.query} "${PHASE_DIR}" "${PHASE_REQ_IDS}" --raw)
CHECK_EXIT=$?
```
...
```bash
GATE_RESULT=$(workflow_run check predicate --predicate '<hook.check.predicate as JSON>' --phase-dir "${PHASE_DIR}" --phase-req-ids "${PHASE_REQ_IDS}" --raw)
CHECK_EXIT=$?
```
...
3. **Sync chain flag with intent** — if user invoked manually (no `--auto` and no `--chain`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]] && [[ ! "$ARGUMENTS" =~ --chain ]]; then
     workflow_run query config-set workflow._auto_chain_active false || true
   fi
   ```
...
Display banner:
```
### Workflow ► AUTO-ADVANCING TO EXECUTE
...
```
Skill(skill="workflow-execute-phase", args="${PHASE} --auto --no-transition ${Workflow_WS}")
```
...
### Workflow ► PHASE ${PHASE} COMPLETE ✓
  ...
  Next: /workflow:discuss-phase ${NEXT_PHASE} --auto ${Workflow_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /workflow:execute-phase ${PHASE} ${Workflow_WS}
  ```
...
/workflow:execute-phase {X} ${GAPS_EXEC_FLAG} ${Workflow_WS}

---

**Also available:**
- cat .planning/phases/{phase-dir}/*-PLAN.md — review plans
- /workflow:plan-phase {X} --research — re-research first
- /workflow:review --phase {X} --all — peer review plans with external AIs
- /workflow:plan-phase {X} --reviews — replan incorporating review feedback

---
</offer_next>

<windows_troubleshooting>
Read `.ai/library/workflows/plan-phase/steps/windows-troubleshooting.md` if plan-phase freezes on Windows during agent spawning (stdio deadlocks with MCP servers, anthropics/claude-code#28126) — it covers force-kill, orphaned-node cleanup, stale task-dir cleanup, reducing the MCP server count, and the `--skip-research` fallback.
</windows_troubleshooting>
...
- [ ] phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] plan-checker spawned with CONTEXT.md
