...
1. .ai/templates/milestone.md
2. .ai/templates/milestone-archive.md
...
**ROADMAP archive** uses `.ai/templates/milestone-archive.md` — includes milestone header (status, phases, date), full phase details, milestone summary (decisions, issues, tech debt).
...
**Compact Content Gate.** Read and follow `.ai/library/references/compact-content-gate.md` now — it states the `workflow.compact_content` check and the resolution rule this spine defers to. When it directs a Read, read `.ai/library/workflows/complete-milestone/detail/elaboration.md` in full before continuing past this point; its content elaborates on the audit-acknowledge branch and the handle_branches step below.
...
[R] Resolve — stop and fix items, then re-run /workflow:complete-milestone
...
2. **Run verification first** — `/workflow:verify-work {phase}` or `/workflow:execute-phase {phase}`
...
2. **Run audit first** — `/workflow:audit-milestone` to assess gap severity
3. **Abort** — return to development
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  workflow_run query summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `workflow_run query milestone.complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to `workflow_run query milestone.complete`:**

```bash
ARCHIVE=$(workflow_run query milestone.complete "v[X.Y]" --name "[Milestone Name]" --confirm $ARCHIVE_QUICK_FLAG)
```
...
**Phase archival (default-on):** `milestone complete` archives phase directories to `milestones/v[X.Y]-phases/` by default (#1871), so the next `/workflow:new-milestone` never inherits un-archived dirs. No manual `mkdir`/`mv` or `--archive-phases` flag is needed.
...
```bash
workflow_run query milestone complete v[X.Y] --no-archive-phases --confirm
```
...
```bash
INIT_REORG=$(workflow_run query init.complete-milestone)
...
ROADMAP_PATH=$(_workflow_field "$INIT_REORG" roadmap_path)
...
_workflow_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
ROADMAP_PATH=$(_workflow_field "$INIT_REORG" roadmap_path)
printf '%s\n' "$ROADMAP_PATH" > .planning/.workflow-allow-shrink
...
if [[ "$INIT_REORG" == @file:* ]]; then INIT_REORG=$(cat "${INIT_REORG#@file:}"); fi
_workflow_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_workflow_field "$INIT_REORG" state_path)
ROADMAP_PATH=$(_workflow_field "$INIT_REORG" roadmap_path)
ARCHIVE_DIR=$(_workflow_field "$INIT_REORG" archive_dir)
MILESTONES_PATH=$(_workflow_field "$INIT_REORG" milestones_path)
PROJECT_PATH=$(_workflow_field "$INIT_REORG" project_path)
workflow_run query commit "chore: archive v[X.Y] milestone files" --files "${ARCHIVE_DIR}/v[X.Y]-ROADMAP.md" "${ARCHIVE_DIR}/v[X.Y]-REQUIREMENTS.md" "${ARCHIVE_DIR}/v[X.Y]-MILESTONE-AUDIT.md" "$MILESTONES_PATH" "$PROJECT_PATH" "$STATE_PATH" "$ROADMAP_PATH"
```
...
if [[ "$INIT_REORG" == @file:* ]]; then INIT_REORG=$(cat "${INIT_REORG#@file:}"); fi
_workflow_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
REQUIREMENTS_PATH=$(_workflow_field "$INIT_REORG" requirements_path)
...
**If doesn't exist:** Create from template at `.ai/templates/retrospective.md`.
...
**Commit:**
```bash
workflow_run query commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
Exact bash for each of the four options (squash, history-preserving merge, delete, keep): `.ai/library/workflows/complete-milestone/detail/elaboration.md` § 2.

</step>

<!-- workflow:section id="git-tag" when="state:git-create-tag" -->
If `section_manifest` is `null` or `"git-tag"` is in its `included` list: read and execute `.ai/library/workflows/complete-milestone/steps/git-tag.md`. Otherwise skip — do not read the file; proceed to `git_commit_milestone`.
<!-- /workflow:section -->
...
- [ ] User knows next step (/workflow:new-milestone)
