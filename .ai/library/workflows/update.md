<purpose>
Check for Workflow updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Workflow_TOOLS=""
for cand in \
  "$PREFERRED_CONFIG_DIR/workflow-core/bin/workflow-tools.cjs" \
  "$HOME/.claude/workflow-core/bin/workflow-tools.cjs"; do
  if [ -n "$cand" ] && [ -f "$cand" ]; then Workflow_TOOLS="$cand"; break; fi
done
# Last resort: the workflow-tools shim on PATH — resolved to its absolute path and
# invoked via the variable (never a bare `workflow-tools` command; see #2851).
if [ -z "$Workflow_TOOLS" ] && command -v workflow-tools >/dev/null 2>&1; then
  Workflow_TOOLS="$(command -v workflow-tools)"
fi

UC=""
if [ -n "$Workflow_TOOLS" ]; then
  case "$Workflow_TOOLS" in
    *.cjs) UC="$(node "$Workflow_TOOLS" update-context --config-dir "$PREFERRED_CONFIG_DIR" --runtime "$PREFERRED_RUNTIME" --json 2>/dev/null)" ;;
    *)     UC="$("$Workflow_TOOLS" update-context --config-dir "$PREFERRED_CONFIG_DIR" --runtime "$PREFERRED_RUNTIME" --json 2>/dev/null)" ;;
  esac
fi
...
if [ -n "$UC" ]; then
  ...
  Workflow_DIR="$(uc_field workflowDir)"
else
...
- Line 4 = resolved Workflow config dir (e.g. `/Users/me/.claude`, `/Users/me/.gemini`); empty when no installed target is resolved. Capture this as `Workflow_DIR` and pass it to subsequent steps so they don't re-derive the runtime path.
...
**If `INSTALL_SCOPE` is `UNKNOWN`, `TARGET_RUNTIME` is empty, or `Workflow_DIR` is empty:** this gate takes precedence over the VERSION-missing case below — a fully-unresolved target also reports version `0.0.0`, and must exit here rather than fall through to "proceed to install".

```text
UPDATE_TARGET_UNRESOLVED

Workflow could not resolve an installed update target. No update was performed.

Rerun from a valid installed runtime: `/workflow:update`. For a fresh installation, run `npx -y --package=@openworkflow/workflow-core@latest -- workflow-core --global`.
```
...
**Section-manifest gate (#2994):** reuse the `$Workflow_TOOLS` already resolved by `get_installed_version` above — do NOT copy the canonical launcher preamble here, it assigns the SAME `$Workflow_TOOLS` variable via a different (fixed-candidate) resolution and would silently override the value `backup_custom_files`/`restore_custom_files` (later steps) still depend on. Forward `--next`/`--rc` from `$ARGUMENTS` so `init.update`'s `state:next-channel` fact matches this step's own case-statement:

```bash
INIT_UPDATE=""
if [ -n "$Workflow_TOOLS" ]; then
  case "$Workflow_TOOLS" in
    *.cjs) INIT_UPDATE="$(node "$Workflow_TOOLS" query init.update $([ "$TAG" = "next" ] && echo --next) 2>/dev/null)" ;;
    *)     INIT_UPDATE="$("$Workflow_TOOLS" query init.update $([ "$TAG" = "next" ] && echo --next) 2>/dev/null)" ;;
  esac
fi
...
The `Workflow_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`~/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. An unresolved target exits in `get_installed_version` before this step.
...
if LATEST_RESULT="$(node "$Workflow_DIR/workflow-core/bin/check-latest-version.cjs" --json --tag "$TAG" 2>/dev/null)"; then
  LATEST_STATUS=0
else
  LATEST_STATUS=$?
fi
...
To update manually: `npx -y --package=@openworkflow/workflow-core@{TAG} -- workflow-core --global`
```
...
<!-- workflow:section id="channel-banner" when="state:next-channel" -->
If `section_manifest` (from `INIT_UPDATE`) is `null` or `"channel-banner"` is in its `included` list: read and execute `.ai/library/workflows/update/steps/channel-banner.md`. Otherwise (default stable channel) skip — do not read the file; the output must match the prior stable behavior exactly, with no channel line.
<!-- /workflow:section -->
...
Running /workflow:update would install the npm release (A.B.C) and downgrade
...
1. Fetch changelog from GitHub raw URL and save to a temp file, e.g. `/tmp/workflow-changelog-$$.md`.
...
```bash
CHANGELOG_TMP="/tmp/workflow-changelog-$$.md"
curl -fsSL ".ai/library/SOURCES.md#source-2ef692f907994d0b" -o "$CHANGELOG_TMP" 2>/dev/null \
  || wget -qO "$CHANGELOG_TMP" ".ai/library/SOURCES.md#source-2ef692f907994d0b" 2>/dev/null

Workflow_CHANGESET_CLI="$Workflow_DIR/scripts/changeset/cli.cjs"
if [ ! -f "$Workflow_CHANGESET_CLI" ]; then
  CHANGELOG_PREVIEW="(Changelog CLI not found at $Workflow_CHANGESET_CLI — reinstall Workflow to restore preview. Update will still proceed.)"
else
  EXTRACT_JSON=$(node "$Workflow_CHANGESET_CLI" extract \
  ...
  if [ "$EXTRACT_EXIT" -eq 2 ]; then
    ...
    CHANGELOG_PREVIEW=$(node "$Workflow_CHANGESET_CLI" extract \
      --from "$INSTALLED_VERSION" \
      --to "$LATEST_VERSION" \
...
If you've modified any Workflow files directly, they'll be automatically backed up to `workflow-local-patches/` and can be reapplied with `/workflow:update --reapply` after the update.
```
...
Workflow-managed directories. These are files that exist on disk but are NOT listed
in `workflow-file-manifest.json` — i.e., files the user added themselves that the
...
even when custom files exist (bug #1997). Use `workflow_run query detect-custom-files`
or the bundled `workflow_run detect-custom-files` path — both resolve paths
...
# get_installed_version emits it as Workflow_DIR for a resolved LOCAL or GLOBAL install.
# The unresolved-target gate exits before this step; the empty guard remains defensive.
RUNTIME_DIR="$Workflow_DIR"
```
...
```bash
CUSTOM_JSON=''
if [ -f "$Workflow_TOOLS" ] && [ -n "$RUNTIME_DIR" ]; then
  CUSTOM_JSON=$(node "$Workflow_TOOLS" detect-custom-files --config-dir "$RUNTIME_DIR" 2>/dev/null)
fi
...
Back up each custom file to `$RUNTIME_DIR/workflow-user-files-backup/` before the
installer wipes the directories:

```bash
BACKUP_DIR="$RUNTIME_DIR/workflow-user-files-backup"
mkdir -p "$BACKUP_DIR"
...
```
⚠️  Found N custom file(s) inside Workflow-managed directories.
    These have been backed up to workflow-user-files-backup/ before the update.
...
**If LOCAL install:**
```bash
npx -y --package=@openworkflow/workflow-core@"$TAG" -- workflow-core "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y --package=@openworkflow/workflow-core@"$TAG" -- workflow-core "$RUNTIME_FLAG" --global
```
...
for dir in "${CACHE_DIRS[@]}"; do
  if [ -n "$dir" ]; then
    rm -f "$dir/cache/workflow-update-check"*.json
  fi
done
...
for dir in .claude .config/opencode .opencode .gemini/antigravity-ide .gemini/antigravity-cli .gemini/antigravity .agents .agent .config/kilo .kilo .codex .cursor .codeium/windsurf .augment .trae .qwen .hermes .codebuddy .cline; do
  rm -f "./$dir/cache/workflow-update-check"*.json
  rm -f "$HOME/$dir/cache/workflow-update-check"*.json
done

# Clear the shared tool-agnostic cache written by workflow-check-update.js hook (#2784).
# The hook uses ~/.cache/workflow/workflow-update-check.json (legacy) or a per-package name
# like workflow-update-check-openworkflow-workflow-core.json; the glob clears all variants so the
# statusline stops showing the stale "⬆ /workflow:update" indicator after update.
rm -f "$HOME/.cache/workflow/workflow-update-check"*.json
```

The SessionStart hook (`workflow-check-update.js`) writes to the detected runtime's cache directory, so preferred/env-derived paths and default paths must all be cleared to prevent stale update indicators.
</step>
...
```
### Workflow Updated: v1.5.10 → v1.5.15

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](../SOURCES.md#source-9c5c83f6d4e65f04)
```
</step>

<step name="restore_custom_files">
`backup_custom_files` copied user-added files into `workflow-user-files-backup/`
...
```bash
RESTORE_JSON=''
if [ -f "$Workflow_TOOLS" ] && [ -n "$Workflow_DIR" ]; then
  RESTORE_JSON=$(node "$Workflow_TOOLS" restore-custom-files --config-dir "$Workflow_DIR" 2>/dev/null)
fi
...
the just-installed release — a renamed workflow it `@`-references, a `/workflow:`
...
```bash
node "$Workflow_TOOLS" restore-custom-files --config-dir "$Workflow_DIR" --apply
```
...
Restore them later with:
  node <config-dir>/workflow-core/bin/workflow-tools.cjs restore-custom-files \
...
Check for workflow-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run `/workflow:update --reapply` to merge your modifications into the new version.
```
