# Complete authoring library

This library provides **complete templates and supporting agent, command,
reference, workflow, and context guides**. Source attribution is isolated in
...
| Content | Local directory | Files |
|---|---|---:|
| Templates | [Templates](../templates/) | 40 |
| `agents/` | [Specialist definitions](agents/) | 64 |
| `commands/` | [Command definitions](commands/) | 72 |
| Reference methods | [Methods and examples](references/) | 131 |
| Workflows | [Workflows and their step files](workflows/) | 177 |
| Context guides | [Context guides](contexts/) | 3 |
...
| `.ai/library/commands/`, `agents/`, `workflows/` | Complete upstream supporting definitions. Read for methodology; they are not installed dispatchers. |
| `/workflow:*`, `/workflow-*`, upstream `Agent`, `Skill`, `AskUserQuestion` | Source host vocabulary. Resolve the capability through the local mapping below; do not claim these tools or commands are registered. |
| `workflow-tools.cjs`, Workflow hooks, installer, state/schema generators | Upstream runtime examples, not installed or invoked by this import. Pinned implementation links are source evidence. |
| `.planning/config.yaml` | Active local execution settings: commands, routes, concurrency, and checks. |
| `.ai/templates/config.json` | Complete upstream Workflow configuration template. Its settings are not accepted aliases for the local YAML runtime. |
| Upstream W019 registry, milestone locks, Workflow state JSON, optional feature gates | Descriptions of upstream behavior. Local availability must be established from the local runtime and guides. |

Read [local adaptation decisions](../references/template-adaptation.md) and the
...
| Research and prepare; `plan-phase` | [phase-prepare](../commands/phase-prepare.md) | [planner](agents/planner.md), [plan-phase](workflows/plan-phase.md) |
| Check preparation; planner/checker revision loop | [phase-prepare](../commands/phase-prepare.md) | [plan checker](agents/plan-checker.md) |
...
| Independent outcome assessment | [phase-verify](../commands/phase-verify.md) | [verifier](agents/verifier.md), [verify-work](workflows/verify-work.md) |
...
| UI, security, AI/eval, milestone, profile, backlog and other specialty commands | Retained guidance; no automatic local route | [all command definitions](commands/), indexed in [REFERENCES.json](REFERENCES.json) |
...
- The registry's missing source `artifacts.cjs` build output is mapped
...
- The `context-bridge.md` and `commands/misc.md` references in the verifier
...
```text
python tools/import_templates.py --source SOURCE_CHECKOUT
python tools/import_templates.py --source SOURCE_CHECKOUT --check
python -m unittest discover -s tests -p test_template_library.py
```
