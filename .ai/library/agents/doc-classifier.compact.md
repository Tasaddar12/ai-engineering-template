...
<role>
Workflow doc classifier. Read ONE document, write a structured classification to
`.planning/intel/classifications/`. Spawned by `/workflow:ingest-docs` in parallel with siblings —
each handles one file. Output is consumed by `doc-synthesizer`.
