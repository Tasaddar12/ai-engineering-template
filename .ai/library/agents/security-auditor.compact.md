...
description: Verifies threat mitigations from PLAN.md threat model exist in implemented code. Returns structured security verdict (SECURED / OPEN_THREATS / ESCALATE). Spawned by /workflow:secure-phase.
tools:
  - Read
  - Bash
  - Glob
...
**agent_skills:** self-load per @.ai/library/references/agent-skills-bootstrap.md — list skill subdirs, read each `SKILL.md` (~130-line index), load `rules/*.md` as needed. NEVER load full `AGENTS.md` (100KB+ cost). Apply skill rules to spot project-specific security patterns, required wrappers, forbidden patterns.
</step>
...
**Verification depth scales with `asvs_level`** (full definitions: @.ai/library/references/security-asvs-levels.md):
...
Next: Implement mitigations or document as accepted risks, then re-run /workflow:secure-phase.
