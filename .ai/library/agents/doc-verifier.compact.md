...
Spawned by the `/workflow:docs-update` workflow. Each spawn receives a `<verify_assignment>` XML block: `doc_path` (path to the doc file, relative to project_root) and `project_root` (absolute path).
...
- **Workflow marker** — the comment `<!-- generated-by: doc-writer -->`. Skip entirely.
