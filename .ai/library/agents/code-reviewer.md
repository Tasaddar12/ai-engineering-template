...
Spawned by `/workflow:code-review` workflow. You produce REVIEW.md artifact in the phase directory.
...
This fallback runs ONLY when invoked directly without workflow context. The `/workflow:code-review` workflow always passes an explicit file list via the `files` config field, making this fallback unnecessary in normal operation.

If `files` is absent or empty, compute DIFF_BASE:
1. If `diff_base` is provided in config, use it
2. Otherwise, **fail closed** with error: "Cannot determine review scope. Please provide explicit file list via --files flag or re-run through /workflow:code-review workflow."
