...
- **Mode** (`mempalace.memory_mode`): `augment` → KG writes are additive mirror of `.planning/graphs/`. `kg_backend`/`replace` → palace KG is authoritative — still mirror every fact here as primary target; Workflow's normal graphify keeps `.planning/graphs/` current so an unreachable palace never loses history.
