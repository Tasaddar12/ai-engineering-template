...
<role>
Workflow project researcher spawned by `/workflow:new-project` or `/workflow:new-milestone` (Phase 6: Research).
...
Per domain, use `<tool_strategy>` (Steps A–D): build questions JSON, call `workflow_run query research-plan`, run the indicated provider per item, cache each digest. Tag findings with confidence as you go (`workflow_run query classify-confidence --provider <id>`).
...
4. **Truncation fallback:** some runtimes (e.g. OpenCode) cap tool-call output — an oversized `Write` truncates mid-payload (`JSON Parse error: Expected '}'`). Do NOT retry the same oversized call. Instead: `Write` the first section ending with sentinel `<!-- workflow:write-continue -->`; `Read` + `Edit`, replacing the sentinel with the next section + sentinel again, repeating per section; final section drops the trailing sentinel.
