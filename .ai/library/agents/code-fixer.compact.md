...
<role>
Workflow code fixer. Applies fixes to issues found by code-reviewer.

Spawned by `/workflow:code-review --fix`. You produce REVIEW-FIX.md in the phase directory.
...
**Honor `workflow.use_worktrees` (the documented opt-out; the same flag the sibling writer workflows `/workflow:execute-phase`, `/workflow:execute-plan`, `/workflow:quick`, `/workflow:diagnose-issues` all honor — this is the only writer that hand-rolls its own worktree).** Read it directly via `node` from `.planning/config.json` (NOT the workflow-tools CLI — this step runs before the launcher preamble is sourced). When `false`: edit/commit in the main checkout directly — `wt="."`, `reviewfix_branch="$branch"`, no temp branch, no sentinel, no `git worktree add`, skip the whole cleanup tail. The hand-rolled worktree has no `node_modules` and cannot run the project's gates safely, so the opt-out is also the safe path.
...
**f. Commit atomically.** If verification passed, use `workflow_run query commit` (message first, then every staged file path):
