...
description: Researches how to implement a phase before planning. Produces RESEARCH.md consumed by planner. Spawned by /workflow:plan-phase orchestrator.
...
Spawned by `/workflow:plan-phase` (integrated) or `/workflow:plan-phase --research-phase <N>` (standalone).
...
<philosophy>
@.ai/library/references/research-philosophy.md
</philosophy>
...
workflow_run query research-plan --input /tmp/research-plan-input.json
```
...
| `brave` | `workflow_run query websearch "<fetch.query>"` (Brave-backed) or built-in `WebSearch` |
...
```bash
workflow_run query research-store put <key> \
...
```bash
workflow_run query classify-confidence --provider <provider-id>
# for cross-checked findings, add --verified:
workflow_run query classify-confidence --provider <provider-id> --verified
```
...
@.ai/library/references/research-verification-protocol.md
...
```bash
workflow_run query package-legitimacy check --ecosystem <npm|pypi|crates> <pkg1> <pkg2> ...
```
...
- **Phase gate:** Full suite green before `/workflow:verify-work`
...
@.ai/library/references/thinking-models-research.md
...
```bash
INIT=$(workflow_run query init.phase-op "${PHASE}")
...
```bash
workflow_run graphify query "<capability-keyword>" --budget 1500
```
...
For each domain, use the `<tool_strategy>` seam (Steps A–D): build questions JSON, call `workflow_run query research-plan`, run the indicated provider per item, then cache each digest. Document findings with confidence levels as you go (use `workflow_run query classify-confidence --provider <id>` to obtain the tier).
...
```bash
workflow_run query commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
...
## Quick Claim-Disposition Pass (`/workflow:explore`)
