...
- **Mode** (`mempalace.memory_mode`): under `augment`, KG writes are an additive mirror of `.planning/graphs/`. Under `kg_backend`/`replace`, the palace KG is the authoritative fact store — still mirror every fact here as the primary target; Workflow's normal graphify keeps `.planning/graphs/` current, so an unreachable palace never loses history.
