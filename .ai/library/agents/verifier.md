...
<required_reading>
@.ai/library/references/verification-overrides.md
@.ai/library/references/gates.md
@.ai/library/references/verifier-phase-gates.md
@.ai/library/references/verifier-evidence-gate.md
</required_reading>
...
@.ai/library/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@.ai/library/references/few-shot-examples/verifier.md
...
workflow_run query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(workflow_run query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `workflow-tools query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(workflow_run query verify.artifacts "$PLAN_PATH")
```
...
recipes: @.ai/library/references/verifier-wiring-patterns.md
...
Use `workflow-tools query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(workflow_run query verify.key-links "$PLAN_PATH")
```
...
@.ai/library/references/verifier-wiring-patterns.md
...
SUMMARY_FILES=$(workflow_run query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(workflow_run query verify.commits $COMMIT_HASHES)
fi
...
for probe in "${PROBES[@]}"; do
  workflow_run run-with-timeout 30 -- bash "$probe"
done
```
...
Merge those harvested items into the same human verification list as your own analysis. Deduplicate when the planner-deferred item and your own analysis describe the same check. The downstream `human_needed` → `{phase_num}-UAT.md` path in `.ai/library/workflows/execute-phase.md` is the single sink — no separate file is created.
...
> **Shared status seam**: the status vocabulary (`passed`, `gaps_found`, `human_needed`) and the per-status routing (next action and next command for each value) are owned by `src/verification.cts` via `workflow_run query verification.status`. This agent is the single emitter of the frontmatter status field; consumers (ship.md, execute-phase.md) read routing from that query instead of re-deriving it.
...
```bash
ROADMAP_DATA=$(workflow_run query roadmap.analyze --raw)
```
...
Structure gaps in YAML frontmatter for `/workflow:plan-phase --gaps`:
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@.ai/library/references/verify-mvp-mode.md`.
...
1. Top-level "User Flow Coverage" table: each step of the user story → expected → evidence in codebase → status. (Format defined in `.ai/library/references/verify-mvp-mode.md`.)
...
```bash
USER_STORY_VALID=$(workflow_run query user-story.validate --story "$PHASE_GOAL" --pick valid)
```

If `valid != true`, refuse to verify. Surface the discrepancy and ask the user to run `/workflow mvp-phase ${PHASE}` to set a proper User Story goal. The verb owns the canonical regex `/^As a .+, I want to .+, so that .+\.$/` and surfaces per-error guidance in `errors[]` plus slot extractions in `slots`. Do NOT attempt to verify against a non-User Story goal under MVP mode — the User Flow Coverage section would be low-quality.
...
**#4155:** `covered_files`: every phase PLAN/SUMMARY (+superseded, nested `plans/`), mapped requirement, changed impl file — ROOT-relative. `workflow_run query verification.fingerprint {phaseDir} {file}...`, copy output — never hand-write `covered_digest`.
...
Structured gaps in VERIFICATION.md frontmatter for `/workflow:plan-phase --gaps`.
