...
.ai/
  ...
  AGENTS.md                         optional Codex/ChatGPT entry guidance
...
Actual linked worktrees use `.worktrees/<plan>/<task-attempt>` beneath the repository base. Host-specific absolute paths and process IDs remain untracked under `.ai/local/`. Remove clean merged worktrees through Git, retain failed or unmerged commits through branches, and remove `.worktrees/` when empty.

The installer creates exactly one self-contained `.ai` or `.claude` namespace. Framework-owned installed assets have manifest hashes. Project-owned state and instructions are preserved on repeat installation; conflicting managed assets fail instead of being overwritten.
