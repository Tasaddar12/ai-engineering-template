...
1. Plan IDs are project-unique. Task, command, review, and evidence IDs are unique within their plan; every reference to one is plan-qualified. All references exist, use compatible schema versions, and resolve within project roots. Full Git OIDs are observed, not fabricated.
...
4. Approved graph digest matches the lifecycle-neutral structural projection of task records. Scope, dependencies, acceptance, contracts, and logical references affect the digest; status, attempts, and physical current/completed/archived locations do not. A structural graph change invalidates isolation approval and rechecks affected candidates.
...
12. Archive manifests retain every referenced required evidence object. Manifest paths resolve relative to their immutable snapshot root and recorded hashes; old records and reviews are never rewritten to current locations. Host-local mappings are excluded from tracked portable records.
