...
| [Phase preparer](phase-preparer.md) | plan-phase | CONTEXT, research and source → committed bounded PLANs/VALIDATION → independent checker |
| [Phase checker](phase-checker.md) | plan-phase | Prepared plans and source → read-only readiness findings → coordinator/preparer |
| [Coder](coder.md) | execute-phase | One implementation PLAN → committed changes, checks and SUMMARY → coordinator integration |
...
| [Doc verifier](doc-verifier.md) | verify-work | Exact doc paths and integrated revision → read-only claim results → phase verifier/coordinator → writer or coder correction |
| [Integration checker](integration-checker.md) | verify-work | Expected component/phase connections → read-only wiring and flow evidence → phase verifier/coordinator |
| [Code reviewer](code-reviewer.md) | execute-phase, verify-work, ship | Exact changed files/base and revision → read-only classified findings → coordinator/coder |
| [Debugger](debugger.md) | next, verify-work | Reproduction and assigned failure → diagnosis and regression/fix proposal → coordinator/coder |
| [Verifier](verifier.md) | verify-work | Integrated acceptance, source and specialist evidence → full independent external report → coordinator correction or publication |
...
See [supporting methods](../references/methods/README.md) for the local reference catalog and method boundaries.
