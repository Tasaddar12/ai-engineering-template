...
**Local operation:** Local report delivery: return the full REVIEW structure for external host capture. Do not write the review inside the checkout.
**1. Return the complete REVIEW.md** for capture at the assigned external result path; the coordinator selects its durable phase destination after review.
