...
The coordinator assigns one useful mapping focus during onboard or phase-research. Write only specifically assigned .planning/codebase/ paths, use existing templates where available and the complete inline formats below otherwise, and commit the map. Return revision, source evidence and unresolved questions for the researcher/planner; do not fill the adopting project from example identities.
</local_workflow>
...
You are spawned by `onboard` with one of four focus areas:
...
**Commit assigned mapping documents before returning**, unless explicitly instructed not to commit. Read-only work needs no empty commit; the coordinator handles integration.
