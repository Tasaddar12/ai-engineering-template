...
<required_reading>
@.ai/gsd/references/domain-probes.md
@.ai/gsd/references/gate-prompts.md
@.ai/gsd/references/universal-anti-patterns.md
</required_reading>
...
| When | Read |
|---|---|
| `--power` in $ARGUMENTS | `.ai/gsd/workflows/discuss-phase/modes/power.md` (then exit standard flow) |
| `--all` in $ARGUMENTS | `.ai/gsd/workflows/discuss-phase/modes/all.md` overlay |
| `--auto` in $ARGUMENTS | `.ai/gsd/workflows/discuss-phase/modes/auto.md` + `.ai/gsd/workflows/discuss-phase/modes/chain.md` (auto-advance) |
| `--chain` in $ARGUMENTS | `.ai/gsd/workflows/discuss-phase/modes/default.md` + `.ai/gsd/workflows/discuss-phase/modes/chain.md` |
| `--text` in $ARGUMENTS or `workflow.text_mode: true` | `.ai/gsd/workflows/discuss-phase/modes/text.md` overlay |
| `--batch` in $ARGUMENTS | `.ai/gsd/workflows/discuss-phase/modes/batch.md` overlay |
| `--analyze` in $ARGUMENTS | `.ai/gsd/workflows/discuss-phase/modes/analyze.md` overlay |
| ADVISOR_MODE = true (USER-PROFILE.md exists) | `.ai/gsd/workflows/discuss-phase/modes/advisor.md` |
| no flags above | `.ai/gsd/workflows/discuss-phase/modes/default.md` |
| in `write_context` step | `.ai/gsd/workflows/discuss-phase/templates/context.md` |
| in `git_commit` step | `.ai/gsd/workflows/discuss-phase/templates/discussion-log.md` |
| writing checkpoints | `.ai/gsd/workflows/discuss-phase/templates/checkpoint.json` |
...
**Text mode** (`--text` or `workflow.text_mode: true`): follow `.ai/gsd/workflows/discuss-phase/modes/text.md` — do not use AskUserQuestion at all.
</answer_validation>
...
- If `--power` in $ARGUMENTS: `Read(.ai/gsd/workflows/discuss-phase/modes/power.md)` and execute it end-to-end. Do NOT continue with the steps below.
- Otherwise, continue. Per-flag overlay reads happen at their relevant steps:
  - `--all` → Read `.ai/gsd/workflows/discuss-phase/modes/all.md` before `present_gray_areas`.
  - `--auto` → Read `.ai/gsd/workflows/discuss-phase/modes/auto.md` before `check_existing` (it overrides several steps).
  - `--chain` → Read `.ai/gsd/workflows/discuss-phase/modes/chain.md` before `auto_advance`.
  - `--text` (or `workflow.text_mode: true`) → Read `.ai/gsd/workflows/discuss-phase/modes/text.md` before any AskUserQuestion call.
  - `--batch` → Read `.ai/gsd/workflows/discuss-phase/modes/batch.md` before `discuss_areas`.
  - `--analyze` → Read `.ai/gsd/workflows/discuss-phase/modes/analyze.md` before `discuss_areas`.
  - `ADVISOR_MODE = true` → Read `.ai/gsd/workflows/discuss-phase/modes/advisor.md` before `analyze_phase` (it changes the discussion flow and adds an `advisor_research` substep).
  - No flags → Read `.ai/gsd/workflows/discuss-phase/modes/default.md` before `discuss_areas`.
...
Read `@.ai/gsd/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
- **Advisor mode (ADVISOR_MODE = true):** follow `.ai/gsd/workflows/discuss-phase/modes/advisor.md` — research-backed comparison tables, table-first selection.
- **--auto:** follow `.ai/gsd/workflows/discuss-phase/modes/auto.md` — Claude picks recommended option for every question; no AskUserQuestion. Single-pass cap enforced.
- **Default (no flags):** follow `.ai/gsd/workflows/discuss-phase/modes/default.md` — 4 single-question turns per area, then check whether to continue.

Overlays (combine with the active mode):
- `--text` → `.ai/gsd/workflows/discuss-phase/modes/text.md` (replace AskUserQuestion with plain-text numbered lists)
- `--batch` → `.ai/gsd/workflows/discuss-phase/modes/batch.md` (group 2–5 questions per turn)
- `--analyze` → `.ai/gsd/workflows/discuss-phase/modes/analyze.md` (trade-off table before each question)
...
- **Incremental checkpoint** — after each area completes, write `${phase_dir}/${padded_phase}-DISCUSS-CHECKPOINT.json`. Read `.ai/gsd/workflows/discuss-phase/templates/checkpoint.json` for the schema. The checkpoint is structured state, not the canonical CONTEXT.md (`write_context` produces the canonical output). On session resume, the parent's `check_existing` step detects the checkpoint and offers to resume.
...
```
Read(.ai/gsd/workflows/discuss-phase/templates/context.md)
```
...
```
Read(.ai/gsd/workflows/discuss-phase/templates/discussion-log.md)
```
...
Auto-advance behavior is defined in `.ai/gsd/workflows/discuss-phase/modes/chain.md`.
