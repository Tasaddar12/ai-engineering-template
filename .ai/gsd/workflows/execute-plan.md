...
@.ai/gsd/references/git-integration.md
</required_reading>
...
- **Pattern A:** read @.ai/gsd/references/dispatch-isolation-gate.md and run its
...
exactly as specified in the canonical `.ai/gsd/references/tdd.md` reference — the
...
See .ai/gsd/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using the template at `.ai/templates/user-setup.md` (or its `.ai/templates/user-setup.compact.md` variant — resolve per `.ai/gsd/references/compact-content-gate.md` §"Streams 1b and 4"). Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
