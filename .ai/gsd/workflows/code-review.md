...
# execute-phase.md, plan scopes feat(N-MM):/test(N-MM): from .ai/gsd/references/tdd.md,
...
1. **Expand tilde paths:** SUMMARY.md `key-files` entries may record a `~/...`-prefixed path (e.g. `.ai/gsd/workflows/verify-work.md`). Bash only tilde-expands a literal `~` written in source text, never one arriving as the value of an already-expanded variable, so every later `[ -f "$file" ]` check must see a real, expanded path or it misclassifies the file as deleted.
```bash
EXPANDED_FILES=()
...
If `section_manifest` is `null` or `"structural-pre-pass"` is in its `included` list: read and execute `.ai/gsd/workflows/code-review/steps/structural-pre-pass.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
This step is itself opt-in at the capability layer (see `.ai/gsd/references/loop-hook-dispatch.md`
...
If `section_manifest` is `null` or `"dispatch-fix"` is in its `included` list: read and execute `.ai/gsd/workflows/code-review/steps/dispatch-fix.md`. Otherwise skip — do not read the file; proceed to `present_results`.
<!-- /gsd:section -->
