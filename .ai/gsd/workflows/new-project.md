...
If `section_manifest` is `null` or `"auto-mode-detection"` is in its `included` list: read and execute `.ai/gsd/workflows/new-project/steps/auto-mode-detection.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
**Compact Content Gate.** Read and follow `.ai/gsd/references/compact-content-gate.md` now — it states the `workflow.compact_content` check and the resolution rule this spine defers to. When it directs a Read, read `.ai/gsd/workflows/new-project/detail/elaboration.md` in full before continuing past this point; its content elaborates on two sections below (Step 2b's prior spike/sketch detection, and Step 6's researcher/synthesizer prompts).
...
If `section_manifest` is `null` or `"codebase-map-offer"` is in its `included` list: read and execute `.ai/gsd/workflows/new-project/steps/codebase-map-offer.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->

**If "Skip mapping" OR `needs_codebase_map` is false:** Continue to Step 3.

<!-- gsd:section id="auto-mode-config" when="flag:--auto" -->
If `section_manifest` is `null` or `"auto-mode-config"` is in its `included` list: read and execute `.ai/gsd/workflows/new-project/steps/auto-mode-config.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
Exact detection commands and the surfaced-findings banner: `.ai/gsd/workflows/new-project/detail/elaboration.md` § 1.
...
Exact agent prompts (all four researcher dimensions): `.ai/gsd/workflows/new-project/detail/elaboration.md` § 2.
