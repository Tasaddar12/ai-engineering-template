...
If user passed `--mode new` but `.planning/` already exists: display warning and require explicit confirm via `AskUserQuestion` (approve-revise-abort from `.ai/gsd/references/gate-prompts.md`) before overwriting.
...
**Display discovered set** and request approval (see `.ai/gsd/references/gate-prompts.md` — `yes-no-pick` pattern works; or `approve-revise-abort`):
...
- `<required_reading>` — `.ai/gsd/agents/gsd-doc-classifier.md` (the agent definition itself)
...
Agent({
  ...
  prompt: "
    ...
    <required_reading>
    - .ai/gsd/agents/gsd-doc-synthesizer.md
    - .ai/gsd/references/doc-conflict-engine.md
    </required_reading>
  "
})
```
...
Apply the safety semantics from `.ai/gsd/references/doc-conflict-engine.md`. Operation noun: `ingest`.
...
Do NOT:
- Violate the shared conflict-engine contract in `.ai/gsd/references/doc-conflict-engine.md` (no markdown tables, no new severity labels, no bypass of the BLOCKER gate)
