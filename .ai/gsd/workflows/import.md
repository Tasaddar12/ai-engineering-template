...
Run conflict checks against the loaded project context. The report format, severity semantics, and safety-gate behavior are defined by `.ai/gsd/references/doc-conflict-engine.md` — read it and apply it here. Operation noun: `import`.
...
Render the full Conflict Detection Report using the format in `.ai/gsd/references/doc-conflict-engine.md`.
...
Ask via AskUserQuestion using the approve-revise-abort pattern (see `.ai/gsd/references/gate-prompts.md`):
