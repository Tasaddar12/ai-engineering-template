...
**Compact Content Gate.** Read and follow `.ai/gsd/references/compact-content-gate.md` now — it states the `workflow.compact_content` check and the resolution rule this spine defers to. When it directs a Read, read `.ai/gsd/workflows/complete-milestone/detail/elaboration.md` in full before continuing past this point; its content elaborates on the audit-acknowledge branch and the handle_branches step below.
...
Exact per-category bash (including the `@file:` large-payload handling, the `todos` 5-per-scan cap, and the phase-scoped `--archived-milestone` handling) and the exact STATE.md table shape: `.ai/gsd/workflows/complete-milestone/detail/elaboration.md` § 1.
</step>
...
Exact bash for each of the four options (squash, history-preserving merge, delete, keep): `.ai/gsd/workflows/complete-milestone/detail/elaboration.md` § 2.

</step>

<!-- gsd:section id="git-tag" when="state:git-create-tag" -->
If `section_manifest` is `null` or `"git-tag"` is in its `included` list: read and execute `.ai/gsd/workflows/complete-milestone/steps/git-tag.md`. Otherwise skip — do not read the file; proceed to `git_commit_milestone`.
<!-- /gsd:section -->
