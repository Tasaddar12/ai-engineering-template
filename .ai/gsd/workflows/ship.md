...
6. **Capability ship gates (generic dispatch).**
   ...
   **For each active entry where `kind == "gate"`** (process in array order), following `.ai/gsd/references/loop-hook-dispatch.md`. Entries of any other `kind` are not gates and are not enforced here. Every gate is visited exactly once by this loop — the named branches below are specializations *within* it, never a separate pass, so no gate is evaluated twice.
