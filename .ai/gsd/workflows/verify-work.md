...
**Compact Content Gate.** Read and follow `.ai/gsd/references/compact-content-gate.md` now — it states the `workflow.compact_content` check and the resolution rule this spine defers to. When it directs a Read, read `.ai/gsd/workflows/verify-work/detail/elaboration.md` in full before continuing past this point; its content elaborates on the resume/reconcile steps and the full gap-closure sub-flow below.
...
**Contribution dispatch:** inject every `kind == "contribution"` fragment per @.ai/gsd/references/loop-hook-dispatch.md (skip when none), before the steps and gates below.
...
If `section_manifest` is `null` or `"automated-ui-verification"` is in its `included` list: read and execute `.ai/gsd/workflows/verify-work/steps/automated-ui-verification.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
If `section_manifest` is `null` or `"mvp-uat-framing"` is in its `included` list: read and execute `.ai/gsd/workflows/verify-work/steps/mvp-uat-framing.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
Exact YAML shape and the announcement line: `.ai/gsd/workflows/verify-work/detail/elaboration.md` § 1.
</step>
...
Exact resume-announcement wording: `.ai/gsd/workflows/verify-work/detail/elaboration.md` § 2.
</step>
...
**Generic step dispatch:** dispatch every `kind == "step"` hook from `VERIFY_POST_HOOKS_JSON` per @.ai/gsd/references/loop-hook-dispatch.md (skip silently when none). Each step is advisory and best-effort — honor `onError` and continue. The secure-phase handling below is an additional specialization of one such hook, not a replacement for the generic dispatch.
...
Read and follow `.ai/gsd/workflows/transition.md`.
...
Exact Agent() prompt fields: `.ai/gsd/workflows/verify-work/detail/elaboration.md` § 2.
</step>
...
`.ai/gsd/references/planner-revision.md`, which you load in revision mode.
...
Exact Agent() prompt fields (revision_context, required_reading): `.ai/gsd/workflows/verify-work/detail/elaboration.md` § 3.
</step>
