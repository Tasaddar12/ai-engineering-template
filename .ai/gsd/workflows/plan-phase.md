...
@.ai/gsd/references/revision-loop.md
@.ai/gsd/references/gate-prompts.md
@.ai/gsd/references/agent-contracts.md
@.ai/gsd/references/gates.md
</required_reading>
...
Read and follow `.ai/gsd/references/compact-content-gate.md` now — it states the `workflow.compact_content` check and the resolution rule this spine defers to. When it directs a Read, read `.ai/gsd/workflows/plan-phase/detail/elaboration.md` in full before continuing past this point; its content elaborates on several steps below.
...
When the tdd capability's `workflow.tdd_mode` is active (resolved via the plan:pre render-hooks), the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `.ai/gsd/references/tdd.md`. The TDD guidance is injected via the tdd capability's contribution hook at §5.6; no inline config-get is needed.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `.ai/gsd/references/skeleton-template.md` — the planner reads it when producing SKELETON.md (lazy; not loaded on non-skeleton runs).
...
If `section_manifest` is `null` or `"reviews-prerequisite"` is in its `included` list: read and execute `.ai/gsd/workflows/plan-phase/steps/reviews-prerequisite.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
If `section_manifest` is `null` or `"prd-express-gate"` is in its `included` list: read and execute `.ai/gsd/workflows/plan-phase/steps/prd-express-gate.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->

<!-- gsd:section id="adr-ingest-express-path" when="flag:--ingest" -->
If `section_manifest` is `null` or `"adr-ingest-express-path"` is in its `included` list: read and execute `.ai/gsd/workflows/plan-phase/steps/adr-ingest-express-path.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
If `section_manifest` is `null` or `"research-only-modifiers"` is in its `included` list: read and execute `.ai/gsd/workflows/plan-phase/steps/research-only-modifiers.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
If `section_manifest` is `null` or `"research-only-early-exit"` is in its `included` list: read and execute `.ai/gsd/workflows/plan-phase/steps/research-only-early-exit.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
**Contribution dispatch (#3606):** inject every `kind == "contribution"` fragment from `PLAN_PRE_HOOKS_JSON` per @.ai/gsd/references/loop-hook-dispatch.md, in array order, into the role each entry's `into` names — planner-targeted ones land in the prompt block below, orchestrator-targeted ones in your working context. The security specialization below is one such contribution, not a replacement for the generic dispatch.
...
**Read and run** the gate + edge probe in `.ai/gsd/references/specless-probe-fallback.md`
...
Read+execute `.ai/gsd/workflows/plan-phase/steps/stall-detection-helpers.md` (defines
...
- explicitly document a deferral/rejection rationale in the relevant PLAN.md, using the Review Dispositions Ledger in `.ai/gsd/references/planner-reviews.md`.
...
Rules + worked examples: @.ai/gsd/references/planner-failing-direction.md
</failing_direction_contract>
<!-- gsd:protected:end -->
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `.ai/gsd/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
**WALKING_SKELETON:** ${WALKING_SKELETON} (when true, the first deliverable must be a Walking Skeleton — Read the template at `.ai/gsd/references/skeleton-template.md` and produce SKELETON.md alongside PLAN.md.)
**Granularity:** {granularity}

${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `.ai/gsd/references/planner-mvp-mode.md` now and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
...
else-branch below, per §A/§B/§C of `.ai/gsd/references/specless-probe-fallback.md`
...
If `section_manifest` is `null` or `"chunked-planning-mode"` is in its `included` list: read and execute `.ai/gsd/workflows/plan-phase/steps/chunked-planning-mode.md`. Otherwise skip — do not read the file.
<!-- /gsd:section -->
...
`.ai/gsd/references/planner-revision.md`.
...
`.ai/gsd/references/revision-loop.md`, with this workflow's bindings:
...
**Step and contribution dispatch:** dispatch every `kind == "step"` hook and inject every `kind == "contribution"` fragment per @.ai/gsd/references/loop-hook-dispatch.md (skip each kind silently when none), before gate evaluation below.
...
Read `.ai/gsd/workflows/plan-phase/steps/windows-troubleshooting.md` if plan-phase freezes on Windows during agent spawning (stdio deadlocks with MCP servers, anthropics/claude-code#28126) — it covers force-kill, orphaned-node cleanup, stale task-dir cleanup, reducing the MCP server count, and the `--skip-research` fallback.
</windows_troubleshooting>
