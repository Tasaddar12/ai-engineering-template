...
- **No planner-side prompt changes.** Folding into `stale: true` reuses the existing rule, which means no new content in `.ai/gsd/agents/gsd-planner.md` (which is already at the `< 48K` decomposition limit per `DEFECT.AGENT-FILE-SIZE-CAP-BREACH`).
