...
The field names are the plan-checker's schema (`.ai/gsd/agents/gsd-plan-checker.md` → `<issue_structure>`):
...
3. **Resolve** — present the conflict and its alternatives to the user and ask which to take
   (pattern: `.ai/gsd/references/gate-prompts.md`): adopt a named alternative / override the
...
2. Use gate prompt (pattern: yes-no from `.ai/gsd/references/gate-prompts.md`):
