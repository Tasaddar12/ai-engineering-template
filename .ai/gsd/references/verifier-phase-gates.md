...
> Loaded eagerly by `.ai/gsd/agents/gsd-verifier.md` (`<required_reading>`). Carries the three
...
Full protocol and rationale: `.ai/gsd/references/honest-verifier.md`.

## Lazy references

- **Per-stack verification patterns:** before Step 4 (artifact verification) on an unfamiliar stack, Read `.ai/gsd/references/verification-patterns.md` — the grep catalog for React/Next.js components, API routes, database schema, and the universal stub patterns. Read it lazily (only the sections for the stack under verification); it is too large to load wholesale on every run.
