...
The verifier (Step 8) harvests every `<verify><human-check>` block at end-of-phase and consolidates them into the existing `human_needed` → `{phase_num}-UAT.md` path in `.ai/gsd/workflows/execute-phase.md`. The user reviews everything in one batch instead of paying a cold-start cost per item.
...
This mode is not purely a planner concern. The **tracer feedback gate** — the executor's early integration checkpoint after a `type="tracer"` task, in `.ai/gsd/workflows/execute-plan.md` and `.ai/gsd/agents/gsd-executor.md` — synthesizes a `checkpoint:human-verify` at runtime that no planner ever emitted, so planner-side suppression cannot reach it.
