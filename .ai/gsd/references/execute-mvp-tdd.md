...
The existing end-of-phase TDD review (in `.ai/gsd/workflows/execute-phase.md`'s `tdd_review_checkpoint` step) is normally **advisory** — it surfaces gate violations but does not block phase completion.
...
- It does not enforce REFACTOR commits. REFACTOR remains optional (per `.ai/gsd/references/tdd.md`).
...
This gate is additive to `.ai/gsd/references/tdd.md`. Tasks not under TDD mode continue to use the existing advisory TDD discipline (RED/GREEN/REFACTOR commits with end-of-phase review checkpoint). Only the runtime gate and the blocking escalation are new.
