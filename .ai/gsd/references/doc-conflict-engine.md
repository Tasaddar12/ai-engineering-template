...
Render the full report, then prompt for approval via the `approve-revise-abort` or `yes-no` pattern from `.ai/gsd/references/gate-prompts.md`. Respect text mode (see the workflow's own text-mode handling). If the user aborts, exit cleanly with a cancellation message.
