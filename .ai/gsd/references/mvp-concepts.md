...
|---|---|---|
| `.ai/gsd/references/planner-mvp-mode.md` | **Rules.** Vertical-slice planning rules, slice ordering, Walking Skeleton constraints. | `gsd-planner` agent when `MVP_MODE=true` |
| `.ai/gsd/references/skeleton-template.md` | **Template.** Shape of `SKELETON.md` for new-project Phase 1 under `--mvp`. | `gsd-planner` agent when the Walking Skeleton gate fires |
| `.ai/gsd/references/user-story-template.md` | **Template.** Format and slot definitions for `As a / I want to / So that`. | `gsd-mvp-phase` workflow during interactive prompting; `gsd-planner` when emitting the `## Phase Goal` header |
| `.ai/gsd/references/spidr-splitting.md` | **Splitting discipline.** Five-axis decomposition (Spike, Paths, Interfaces, Data, Rules) for stories too large for one phase. | `gsd-mvp-phase` workflow when the user story exceeds size threshold |
| `.ai/gsd/references/execute-mvp-tdd.md` | **Gate.** TDD runtime gate semantics: when it fires, what it checks, halt-and-report protocol, end-of-phase blocking escalation, Behavior-Adding Task definition. | `gsd-executor` agent when `TDD_MODE=true` (#4011) |
| `.ai/gsd/references/verify-mvp-mode.md` | **UAT framing.** Three-section UAT structure (user-flow → technical → coverage), anti-patterns, `User Flow Coverage` section in VERIFICATION.md. | `gsd-verifier` agent when the phase under verification has `mode: mvp` |

## Concept-to-file map

If you're looking for the canonical statement of a concept, this is where to find it:

- **MVP Mode resolution chain** — `.ai/gsd/workflows/plan-phase.md` Step 1 (CLI flag → roadmap → config → false). Mirrored in `execute-phase.md` and `verify-work.md`.
...
- **Behavior-Adding Task predicate** — `.ai/gsd/references/execute-mvp-tdd.md` (the canonical three-check definition). Applied at runtime by `gsd-executor`.
- **Walking Skeleton gate condition** — `.ai/gsd/workflows/plan-phase.md` (Phase 1 + new project + `--mvp` + no prior summaries → emit `SKELETON.md`).
- **MVP+TDD Gate** (RED→GREEN enforcement) — `.ai/gsd/references/execute-mvp-tdd.md`.
- **MVP-mode UAT framing** (user-flow first, technical deferred) — `.ai/gsd/references/verify-mvp-mode.md`.
- **Per-phase mode authoring** — `.ai/gsd/workflows/mvp-phase.md` (writes `**Mode:** mvp` to ROADMAP.md after collecting the user story).
- **Project-wide mode prompt at init** — `.ai/gsd/workflows/new-project.md` (Vertical MVP vs Horizontal Layers question).
