...
**Show expected, ask if reality matches** — same philosophy as standard verify-work (from `.ai/gsd/workflows/verify-work.md`). The MVP-mode change is WHAT gets shown:
...
If the phase has `mode: mvp` but the goal is NOT in user-story format, the verifier surfaces this as a discrepancy and asks the user to run `/gsd mvp-phase` to reformat the goal — same pattern as the planner agent under MVP_MODE (per `.ai/gsd/references/planner-mvp-mode.md`).
