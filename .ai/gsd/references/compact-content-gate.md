...
- **`COMPACT_CONTENT` is `"false"` (default):** Read every part under this workflow's own `detail/` directory (a sibling of this spine, e.g. `.ai/gsd/workflows/<name>/detail/*.md`) now, in full, before continuing past this point. Their content elaborates on the spine you are reading — treat everything they say as part of this document from here on.
...
For a `.ai/gsd/workflows/<name>/{modes,steps,templates}/*.md` fragment or a `.ai/templates/**`
...
`.ai/gsd/agents/<name>.compact.md`, same directory, same stem, `.compact.md` suffix — registered and
