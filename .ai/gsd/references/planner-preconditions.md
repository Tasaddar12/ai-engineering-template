...
> Progressive-disclosure reference for `.ai/gsd/agents/gsd-planner.md`. The planner agent
...
> Contract). The slim pointer in `.ai/gsd/agents/gsd-planner.md` → `<task_breakdown>`
...
- `.ai/gsd/agents/gsd-executor.md` → `<execution_flow>` → precondition check step — the
