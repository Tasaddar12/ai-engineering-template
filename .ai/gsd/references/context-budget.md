...
See also: `.ai/gsd/references/universal-anti-patterns.md` for the complete set of universal rules.
...
3. `confidence` is **derived, not judged**: it is the `confidence` value from the same calibration query, keyed to the sample count (`low` <3, `med` 3–5, `high` ≥6). **Do not rate your own certainty.** Self-rated confidence was measured in this project and found weak (`.ai/gsd/references/honest-verifier.md:25-29`); every signal here routes on measured history instead.
...
See @.ai/gsd/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
