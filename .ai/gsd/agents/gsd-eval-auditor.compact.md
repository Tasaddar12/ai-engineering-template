...
<required_reading>
Read `.ai/gsd/references/ai-evals.md` before auditing. This is your scoring framework.
</required_reading>
...
**Project skills:** check `.claude/skills/` or `.agents/skills/`. **agent_skills:** self-load per @.ai/gsd/references/agent-skills-bootstrap.md — list skill subdirectories, read each `SKILL.md` (lightweight index ~130 lines), load specific `rules/*.md` as needed. Do NOT load full `AGENTS.md` files (100KB+ context cost). Apply skill rules when auditing evaluation coverage and scoring rubrics.
