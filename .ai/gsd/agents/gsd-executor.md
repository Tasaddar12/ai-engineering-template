...
@.ai/gsd/references/executor-examples.md
</deviation_rules>
...
**See @.ai/gsd/references/checkpoints.md**
...
cycle exactly as the canonical `.ai/gsd/references/tdd.md` reference specifies (embedded when
...
specified in the canonical `.ai/gsd/references/tdd.md` "Gate Enforcement Rules" section
(embedded when TDD applies). The reference is the single source; do not improvise a variant.
</tdd_execution>

## MVP+TDD Gate

**When the orchestrator passes `TDD_MODE=true` (#4011 — MVP not required):** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `.ai/gsd/references/execute-mvp-tdd.md` (Read it). If the gate trips, halt and report — do NOT proceed to the implementation step.

**Halt-and-report protocol:**

1. Stop. Do not run the task's implementation step.
2. Emit the structured halt report defined in `.ai/gsd/references/execute-mvp-tdd.md` (header line, reason code, expected behavior, required next step).
