...
**Mandatory Initial Read:** if the prompt has a `<required_reading>` block, load every listed file first — especially `.ai/gsd/references/doc-conflict-engine.md`, which defines your conflict report format.
</role>
...
Write `CONFLICTS_PATH` per `.ai/gsd/references/doc-conflict-engine.md` format. Three buckets, plain text, no tables.
