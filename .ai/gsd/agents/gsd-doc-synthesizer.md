...
If the prompt contains a `<required_reading>` block, load every file listed there first — especially `.ai/gsd/references/doc-conflict-engine.md` which defines your conflict report format.
</role>
...
Write `CONFLICTS_PATH` using the format from `.ai/gsd/references/doc-conflict-engine.md`. Three buckets, plain text, no tables.
