...
FEATURE-005 owns each budgeted `.ai/runs/<plan-id-lower>/recovery-NNN/` attempt. Its mutable `protocol.yaml` distinguishes `ready`, `planning`, `applying`, `rejected`, `rolled_back`, `applied` and `uncertain` phases. Each safe hook entry gets a fresh immutable `quiescence-NNN.yaml`, written after workers stop and after blocking/index changes; its generation equals STATE at hook entry. The subject carries `recovery: {attempt, run_dir, protocol, quiescence}`. Changing receipts stay coordinator-side; do not rebuild an immutable agent assignment merely to change its receipt reference.

The receipt binds `plan`, `run`, `attempt`, `subject`, `status: quiescent`, `stopped_features`, `generation`, `baseline`, `protocol` and `outcomes`. Its `subjects` record subject/journal/journal_sha256/session_id/stage; `invocations` record invocation/invocation_sha256/result/result_sha256/request_sha256/output_sha256/status/session_id; `deliveries` record subject/receipt/sha256/push/pr/status. Verify saved outputs, request/result/session identities and the complete invocation and delivery inventory. A missing journal delivery copy never proves that a durable delivery intent has no effects. Pending or uncertain started effects prevent structural recovery; an authority-pending receipt whose effects are all `not_started` is distinguishable.

Use the public `recovery_snapshot(root, plan_id)` helper for exact baseline comparison. It hashes all plan-owned tasks/features, including unreferenced records, their substantive contracts/bodies and referenced context contents; task/feature lifecycle changes count, while routine plan lifecycle/coordinator evidence does not. The protocol's `stopped_evidence` is SHA256 of canonical JSON for `{subjects, invocations, deliveries}` using `sort_keys=True`, `ensure_ascii=True` and default separators. Do not approximate either representation in the hook.

Use `recovery_checkpoint(root, subject, phase, *, dispatch_dir=None)` to reserve every unique attempt-local dispatch root before creating it or invoking an agent. Persist the assignment/session/directory map and reuse it on safe resume. Checkpoint `applying` immediately before `planning.apply_revision`, and `applied` only after success. `rejected` or `rolled_back` require an unchanged baseline. Safe preflight/rejection/verified rollback retries reuse the same budgeted attempt after checking every known invocation/output and delivery effect. Unknown invocation evidence, a changed baseline or interrupted `applying` stays blocked. Model claims and blocked artifact status cannot establish quiescence or authorize replacing active work.
...
## Approved effective remote and current plan references

Independent decomposition approved the exact read-only `git remote get-url --all <named remote>` form alongside `git remote get-url --push --all <named remote>` within TASK-055 / FEATURE-005's existing constraint and seed ownership. Named-remote reconciliation observes the fetch destination; therefore bind both effective destinations and require compatible canonical repository identities. Separately bind explicit PR base repository and fork head ownership. Reject additional operands/options and URL/path remote-name arguments, and protect sensitive output before durable logging. No network action or graph change is authorized by this clarification.

TASK-059 / FEATURE-007's already approved plan-template ownership also replaces frozen creation-time task/feature body placeholders with guidance to the current explicit front-matter references and their decomposition evidence. Preserve the authored desired change, scope and acceptance text. Installed plans must not claim an empty feature list in their body after decomposition has populated the authoritative references. Verify this through CLI/acceptance coverage and synchronize the unchanged installed seed after review. All plans and plan-specific contracts remain under `.ai/` in a PLAN document; no plan-specific content belongs in product documentation.
