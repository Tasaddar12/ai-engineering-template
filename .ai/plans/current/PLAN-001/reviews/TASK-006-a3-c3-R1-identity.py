...
for ref in manifest['validation_refs']:
    ...
    assert 'jsonschema: 4.26.0' in log
    origins = ast.literal_eval(next(line.removeprefix('Origins: ') for line in log.splitlines() if line.startswith('Origins: ')))
    expected_origin = '/mnt/d/Codex Projects/ai-engineering-template/.worktrees/TASK-006-a3/src/commands.py' if 'linux' in ref['path'] else str(CANDIDATE / 'src/commands.py').replace('\\', '/')
    assert origins == [('commands', expected_origin)] or [(name, value.replace('\\', '/')) for name, value in origins] == [('commands', expected_origin)]
