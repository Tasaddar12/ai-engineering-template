...
- `exit_zero` requires normal terminal process evidence and exit code zero.
  `unittest_nonzero_count` additionally parses the final standard unittest summary from verified
  stdout/stderr and requires a positive observed count. TASK-006 remains responsible only for
  recording process facts; TASK-018 performs the success-rule interpretation.
...
- Each command receipt contains project/plan/run/operation/task/worktree/suite identity, requested and
  observed before/after revisions, request command sequence, configured definition and full argv,
  observed command evidence/log refs, verified byte counts, test count, status, and sanitized error.
...
The exact declared leaf is `-m unittest discover -s tests/unit/validation/ -p test_*.py`. Its ten
...
evidence, continued execution after a runner exception, closed catalogue wiring, and receipt storage
failure. Temporary processes, Git repositories, logs, and receipts are local and removed by test
cleanup.
...
- Unittest count proof intentionally depends on complete unredacted logs and the final standard
  `Ran N test(s) in ...` summary. A configured third-party harness needs `exit_zero` or a future
  explicitly owned success rule; prose or a successful process status cannot invent a count.
...
- No concrete prerequisite, schema, scope, or configuration gap was found. There are no skipped
  ...
  interface edits. Fresh independent Astra/xhigh R1 should first probe omitted/substituted/reordered
  suites, configured argv preservation, zero-test parsing, tampered/missing logs and receipts, and
  before/after revision drift. Distinct R2 must bind the same exact current-base candidate.
