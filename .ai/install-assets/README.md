...
- `claude-config.txt` supplies initial runtime routes for a Claude install.
  ...
  registrations and Codex skill discovery entries are generated by the installer;
  personal host directories are never packaging inputs.

The complete workflow is packaged under `.codex` or `.claude`; role methods use
`roles`, procedures use `workflows`, and full skill bodies use `skills`. Only
Codex gets additional `.agents/skills` discovery wrappers. There is no separate
`.ai` directory in a fresh installed project. Upstream download URLs continue to
