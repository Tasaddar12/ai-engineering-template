...
<!-- Executor should use these directly — verify against the assigned dependency revision before use. -->
...
If this plan creates types/interfaces that later plans depend on, define them before dependent implementation, with a real `depends_on` edge between plans. For example:
...
<task type="auto">
  ...
  <verify>Project typecheck validates exported types and intended consumer signatures</verify>
