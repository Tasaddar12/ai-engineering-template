...
An override records an actual human decision to change an outcome; it does not
turn an unmet requirement into a pass. CONTEXT owns the decision. The report may
reference it for traceability. The verifier never authorizes changed scope.

## When a decision is needed

An intentional alternative may satisfy the goal differently: session-based auth
instead of OAuth PKCE, for example. Establish whether the approved contract
already permits that choice. If it does, verify the implemented path. If it does
not, report the discrepancy and the affected acceptance ID to the coordinator.
