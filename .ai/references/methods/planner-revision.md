...
Inspect the scoped diff, stage only assigned PLAN and SUMMARY paths with `git add --`,
then commit with a descriptive revision message. Return the hash to the coordinator.
...
independent preparation review; it does not count as a failed revision iteration.
