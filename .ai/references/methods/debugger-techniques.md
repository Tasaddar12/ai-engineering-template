...
Full technique bodies for [the debugger role](../../agents/debugger.md). The role
...
Bisection changes checkout state. A worker must ask the coordinator to assign a
separate diagnostic checkout; never bisect its assigned implementation branch.
In that approved disposable diagnostic checkout:
...
Check code says:  hooksDir = path.join(configDir, 'hooks')
                  configDir = ~/.example-app
                  → checks ~/.example-app/hooks/

Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = ~/.example-app/runtime
                  → writes to ~/.example-app/runtime/hooks/
