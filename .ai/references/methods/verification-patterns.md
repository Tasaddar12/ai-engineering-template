> Adapted from the pinned source listed in [third-party notices](../../THIRD-PARTY-NOTICES.md).
> Read the relevant stack sections locally. Shell commands below are examples,
> not proof of behavior or permission to execute them. Inspect their effects,
> use an authorized environment, and honor test deferrals. Line counts, matching
> strings and imports are discovery signals; verify actual behavior and wiring.
> Do not mark required UAT passed from source inspection or a generic example.
...
**[Local checkpoints](checkpoints.md)** → `<automation_reference>` section
