...
**Input:** Two plans in wave 1 -- Plan 01 modifies `src/workflow/execute.ts`, Plan 02 also modifies `src/workflow/execute.ts`. No `depends_on` declared.
...
>     description: "Plans 01 and 02 both modify src/workflow/execute.ts in wave 1 with no stated dependency relationship despite a producer/consumer ordering requirement. If one consumes the other's output, that dependency must be explicit; shared ownership already prevents concurrent dispatch in the local runtime."
>     fix_hint: "Either move Plan 02 to wave 2 with depends_on: ['01'] or consolidate the file changes into a single plan"
> ```

**Why this is good:** The checker identified a potential dependency problem -- two plans modifying the same file without stating whether one requires the other's output. It stated the property that must hold, cited dependency_correctness, named both plans and the conflicting file, and offered two example routes -- neither of which binds, since either makes the property true.
