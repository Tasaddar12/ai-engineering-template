...
Use these discovery recipes with the local verifier. Read the matching source
and callers after each search; grep output alone is not proof of runtime behavior.
The Bash snippets require Bash and adaptation to the actual project language.
...
> These tables describe artifact inspection, not a replacement for behavioral evidence.
