...
> Reference file for coder agent. Loaded on-demand via `@` reference.
> Read this local file when detailed examples help the assigned task.
...
- NO → Out of scope (record in assigned SUMMARY Remaining)

### Recording deferred work

Use the assigned SUMMARY Remaining section. Include the finding, path, evidence,
impact and proposed next action; the coordinator owns phase CONTEXT Deferred.
There is no separate deferred-item scanner or shared ledger for workers to write.

```markdown
## Remaining

- Retry budget is hardcoded at 3 in `src/retry.ts`.
  Evidence: `fetchWithRetry` ignores the configured budget.
  Impact: existing behavior outside this component's scope.
  Next action: coordinator evaluates a separately authorized correction.
```

Checkpoint examples below illustrate the information to return. Follow the
[local checkpoint contract](checkpoints.md) for supported task forms and authority.
