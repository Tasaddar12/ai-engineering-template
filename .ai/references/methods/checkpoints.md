...
Adapted supporting method; source attribution is in [third-party notices](../../THIRD-PARTY-NOTICES.md).
Read this file locally; no external workflow or SDK is required.

## Local execution boundary

The XML below illustrates how to describe a decision or human observation. It is
...
[phase-uat](../../commands/verify-work.md). A worker encountering a new blocker
...
4. Preserve existing authorization; do not repeat already-resolved questions.
5. Stop only work that depends on a missing decision or prerequisite. Continue
   independent authorized work when it is safe to do so.
...
notes; do not rely on an unimplemented mode flag to suppress or approve checkpoints.
...
When a plan uses a thin end-to-end slice, validate that slice before expanding it.
Use supported `type="auto"` tasks to express this ordering. A failed check blocks
dependent expansion. If the next step requires a new human decision, return the
facts to the coordinator; a successful automated check cannot stand in for that
human decision. No special tracer task type or synthesized runtime checkpoint is
...
When a coordinator encounters a real decision or human-only observation:

1. **Stop immediately** - do not proceed to next task
2. **Display checkpoint clearly** using the format below
3. **Wait for user response** - do not hallucinate completion
4. **Verify if possible** - check files, run tests, whatever is specified
5. **Resume execution** - record the actual answer and prepare the authorized continuation; do not blindly replay an interrupted worker
...
**The rule:** If authorized work has an available CLI/API, the agent performs it. Never ask human to perform automatable work.
...
# Bound the readiness check; failure is not readiness.
ready=false
for attempt in {1..30}; do
  if node -e "fetch('http://localhost:3000').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"; then
    ready=true
    break
  fi
  sleep 1
...
**Port conflicts:** Identify the process and its owner. Stop only a process owned by this assignment, or use an available alternate port and update the verification URL.

**Server lifecycle:** The coordinator may keep an owned server available through user review. Workers must stop their background processes before returning; do not leave background writers running or terminate another task's process.

## CLI availability

Check the actual project's manifest and installed commands before using these
examples. Prefer existing tools. A missing CLI is not authorization to download
and execute a package or silently install global dependencies. Use an available
equivalent within the assignment, or report the specific missing capability to
the coordinator. Product-specific commands may require current official vendor
documentation; none is required just to read this workflow method.
...
> **Cross-platform note:** Use a health-check client available on this host; inspect the actual response and do not assume a particular shell or HTTP client is installed.
...
For an assigned `type: tdd` plan, review the named failing assertion, RED/GREEN
commit sequence, passing behavior and any refactor result using [TDD](tdd.md).
This is part of independent verification, not an automatically inserted task.
Missing required evidence remains a gap; a human review acknowledgement cannot
fabricate a test result. Record the exact tested revision and any user-requested
validation deferral.
</type>
...
**The golden rule:** Perform authorized, automatable work with available tools; request only the human input actually needed.
