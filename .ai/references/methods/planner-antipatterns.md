...
**Why bad:** When deployment is authorized and configured, an available CLI can automate it. Prepare the concrete result and use the supported deployment procedure; do not infer deployment authorization merely from a verification task.
...
> Enforced at plan-write time by `verify.plan-structure` (the `validate_plan` step). .
...
**Review rule:** flag a contradiction when an action requires emitting the same quoted literal that its acceptance check forbids. If the pattern is ambiguous, report the uncertainty. No automatic plan-creation gate for this rule is supplied by the local runtime.
...
When the literal MUST appear in the plan body verbatim — e.g. the plan documents the test file that exercises the gate itself, or the literal is part of the verification command's own grep regex — add a marker on its own line to explain the exception to the reviewer (no runtime parser consumes this marker):
...
> Surfaced at plan-write time by `verify.plan-structure` (the `validate_plan` step), WARN-level. .
...
If region-scoping is genuinely impractical and the file split is intentional, explain the exception to the reviewer with a marker naming the pattern:

```
<!-- planner-region-allow: await .*refresh -->
```

One marker per pattern. The marker is review context, not an automatic runtime exemption. Prefer region-scoping over suppression.

## CLI Output Format Anchor Mismatch
...
## Error-Suppressing Fallbacks in Verify Gates
...
## External Review and Draft Publication

Keep the first committed slice reviewable in a draft PR under repository delivery
rules. Draft publication is progress tracking, not a claim that internal review is
complete. Resolve accepted internal/external findings before final readiness.
If a project has a genuine publication-time requirement, check it at the relevant
publication event and cite the actual evidence. Do not delay the mandated first
draft push merely to optimize an external review bot's workload.
