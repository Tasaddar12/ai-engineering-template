...
One coordinator is the sole writer per project. An OS-backed exclusive lock in the Git common directory protects its entire ownership tenure; a heartbeat is diagnostic, not permission to steal a live lock. Release on process death is OS-managed. No lock-file timestamp-based takeover. Initial supported execution is a single machine on a local filesystem; multi-host/shared-filesystem coordinators are rejected.
...
4. Write immutable event and updated projections into the control worktree using temp files and replace. Record manifest hashes and old/new generation.
