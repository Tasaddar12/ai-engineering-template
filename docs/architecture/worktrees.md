...
Before cleanup verify actual Git path, managed identity, no live lease, clean tracked/untracked files, and retained accepted/failed commits under policy. Use `git worktree remove` without force. Unknown content remains untouched. Branch deletion and remote cleanup are separate policy-controlled operations.
