...
Use a `src` package, Python 3.11+, `argparse` initially, dataclasses for typed domain values, and Protocols for injected ports. JSON Schema 2020-12 is the serialized contract. The initial runtime may use `jsonschema`; schema validation is offline against a local registry. No Pydantic dependency is required initially.
