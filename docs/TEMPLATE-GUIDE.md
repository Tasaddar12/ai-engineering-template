...
All 40 GSD-Core template files are available under `.ai/templates/` with their
original filenames and complete teaching content. Read the selected template in
...
Source revision: `c0b2a05d2f310adc0a1f35fd71fbc9f28f4e4977` of
[GSD-Core](https://github.com/open-gsd/gsd-core/tree/c0b2a05d2f310adc0a1f35fd71fbc9f28f4e4977).
The [support route map](../.ai/gsd/README.md) resolves referenced methods and
...
|---|---|---|
| [README.md](../.ai/templates/README.md) | Upstream canonical artifact registry | Template maintainers → agents locating upstream fact owners |
...
| [config.json](../.ai/templates/config.json) | Upstream `.planning/config.json` settings | Upstream setup → upstream tools; Python runner uses YAML separately |
...
The complete upstream registry describes upstream artifacts and upstream health
...
maps. Do not assume an uninstalled upstream health command validates them.
...
| [summary-standard.md](../.ai/templates/summary-standard.md) | Upstream standard summary variant | Upstream summary-selection flow → result readers |
| [summary-complex.md](../.ai/templates/summary-complex.md) | Upstream complex summary variant | Upstream summary-selection flow → result readers needing richer handoff |
| [summary-minimal.md](../.ai/templates/summary-minimal.md) | Upstream minimal summary variant | Upstream summary-selection flow → bounded result readers |
| [summary.compact.md](../.ai/templates/summary.compact.md) | Complete upstream compact variant | Its referenced upstream flow → result readers |
...
Every summary variant is retained because upstream supplies it. The Python
adapter currently instructs component workers to use the full `summary.md`
skeleton plus its local coverage/check fields. Retaining a compact upstream file
...
| [user-setup.compact.md](../.ai/templates/user-setup.compact.md) | Complete upstream compact setup variant | Its upstream flow → user and continuation |
...
| [copilot-instructions.md](../.ai/templates/copilot-instructions.md) | Copilot-specific GSD entry-point instructions | Host installation/adaptation → compatible Copilot session |
...
## Local templates without upstream equivalents
...
Both follow the teaching design of the upstream templates: complete output
...
| SUMMARY | Acceptance/documentation coverage and actual check evidence alongside full upstream summary content |
...
| UAT | Runtime source/case/history receipts alongside upstream session sections |

Use one YAML frontmatter block per artifact. Keep the upstream `requirements`,
...
serialize even if upstream `coupling_justified` explains a same-wave coupling.

**Configuration boundary:** `.planning/config.yaml` configures Python execution.
The full upstream JSON template remains available for its separate upstream
