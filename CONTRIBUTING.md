...
Phase one is architecture and planning, with a minimal executable package. Install with `python -m pip install -e ".[dev]"`; validate with `python scripts/validate_foundation.py`. Planned runtime tests will be added per task, not represented as passing today.
