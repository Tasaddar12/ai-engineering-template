"""Deny-by-default command, file, task, and cleanup policy composition."""
...
class ConstraintPolicy:
    ...
    def check_action(self, action: str) -> None:
        effect = self.config.get("external_actions", {}).get(action, "forbid")
        if effect == "allow":
            return
    ...
    @staticmethod
    def _context_match(rule: dict[str, Any], field: str, value: str | None) -> bool:
        selected = rule.get(field)
        if selected is None:
            return True
    ...
    def check_command(
        ...
        *,
        ...
        selected_commands: Iterable[str] | None = None,
    ) -> None:
        tokens = command_tokens(argv)
        _git_guard(tokens)
        permissions = self.config.get("roles", {}).get(role, {})
        if not permissions or permissions.get("run_commands") is not True:
            raise PolicyError(f"Role {role!r} may not run commands")
        if command_name is not None:
            allowed_names = permissions.get("commands", [])
            if command_name not in allowed_names:
                raise PolicyError(f"Role {role!r} may not run named command {command_name!r}")
            if selected_commands is not None and command_name not in set(selected_commands):
                raise PolicyError(
        ...
        if tokens[0] == "git" and len(tokens) > 1 and role != "orchestrator":
            if tokens[1] in _GIT_AGENT_FORBIDDEN:
                raise PolicyError("Agents may not mutate branches, refs, remotes, or Git metadata")
        ...
        for rule in rules:
            ...
            if rule.get("roles") and role not in rule["roles"]:
                continue
            if not self._context_match(rule, "workflows", workflow):
                continue
            if not self._context_match(rule, "tasks", task):
                continue
    ...
    def named_command(
        ...
        *,
        ...
        named = self.config.get("commands", {}).get("named", {})
        command = named.get(name)
        if not isinstance(command, dict):
            raise PolicyError(f"Unknown named command: {name!r}")
        for field, value in (("roles", role), ("workflows", workflow), ("tasks", task)):
            permitted = command.get(field)
            if permitted is not None and (value is None or value not in permitted):
                raise PolicyError(
                    f"Named command {name!r} is not permitted for {field[:-1]} {value!r}"
                )
        argv = command.get("argv")
        timeout = command.get("timeout")
        if not isinstance(argv, list) or not isinstance(timeout, (int, float)):
            raise PolicyError(f"Named command {name!r} is malformed")
        self.check_command(
            argv,
            role,
            workflow=workflow,
    ...
    def check_cleanup(self, kind: str, targets: Iterable[str | Path]) -> None:
        """Authorize only the two explicit, exact cleanup classes."""

        values = list(targets)
        if not values or any(not str(value) for value in values):
            raise PolicyError("Cleanup requires explicit nonempty targets")
        cleanup = self.config.get("cleanup", {})
        if kind == "merged_worktree_retirement":
            effect = cleanup.get(kind, "forbid")
            if effect == "allow" or (effect == "approval" and "branch_retirement" in self.grants):
                return
            raise PolicyError("Merged worktree retirement is not authorized")
        if kind == "obsolete_content_deletion":
            effect = cleanup.get(kind, "forbid")
            if effect == "approval" and "obsolete_content_deletion" in self.grants:
                return
            raise PolicyError("Obsolete-content deletion requires current scoped authority")
        raise PolicyError(f"Unknown cleanup class: {kind!r}")
