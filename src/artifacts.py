...
STATUSES = {
    "plans": {"draft", "ready", "in-progress", "completed", "archived", "superseded"},
    "tasks": {"backlog", "ready", "in-progress", "completed", "superseded", "archived"},
    "features": {"draft", "ready", "in-progress", "review", "completed", "superseded"},
    "bugs": {"open", "in-progress", "review", "completed", "archived", "superseded"},
...
ACTIVE_PHASES = {
    "plans": {"draft", "ready", "in-progress"},
    "tasks": {"backlog", "ready", "in-progress"},
    "features": {"draft", "ready", "in-progress", "review"},
    "bugs": {"open", "in-progress", "review"},
}
...
def _folder(kind: str, status: str) -> str:
    ...
    if kind in {"reviews", "handoffs", "decisions"}:
        return kind
    if kind == "plans" and status in {"draft", "ready", "in-progress"}:
        return "plans/active"
...
class ArtifactStore:
    ...
    def set_hard_block(
        ...
        *,
        ...
        """Attach a hard block without changing or relocating the active phase."""
        artifact = self.find(identifier)
        kind = str(artifact.metadata.get("kind"))
        if artifact.status not in ACTIVE_PHASES.get(kind, set()):
            raise FrameworkError("Hard blocks require an active artifact phase")
        artifact.metadata["hard_block"] = {
    ...
    def clear_hard_block(self, identifier: str) -> Artifact:
        artifact = self.find(identifier)
        if "hard_block" not in artifact.metadata:
            raise FrameworkError(f"Artifact has no hard block: {identifier}")
        del artifact.metadata["hard_block"]
        self.save(artifact)
        return artifact
    ...
    def _validate(kind: str, data: dict[str, Any]) -> None:
        ...
        block = data.get("hard_block")
        if block is not None:
            if kind not in ACTIVE_PHASES or data.get("status") not in ACTIVE_PHASES[kind]:
                raise FrameworkError("Hard blocks require an active artifact phase")
            if not isinstance(block, dict) or set(block) != {"phase", *HARD_BLOCK_FIELDS}:
                raise FrameworkError(
                    "Hard block metadata requires phase, reason, evidence, remedies, "
                    "affected_work, next_action and resume_condition"
                )
            if block.get("phase") != data.get("status"):
                raise FrameworkError("Hard block metadata must preserve the current phase")
            for key in ("reason", "next_action", "resume_condition"):
                if not isinstance(block.get(key), str) or not block[key].strip():
                    raise FrameworkError(f"Hard block {key} must be a nonempty string")
            for key in ("evidence", "remedies", "affected_work"):
                ...
                if (
                    ...
                    raise FrameworkError(f"Hard block {key} requires unique nonempty strings")
