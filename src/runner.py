"""The sole subprocess boundary: policy, bounded evidence, redaction, and deadlines.

Provider confinement is constructed by the platform containment adapters before the
resulting argv reaches this runner. Process groups/jobs remain a second termination
boundary, not a claim of filesystem confinement.
"""
...
class CommandRunner:
    def __init__(
        ...
        self.max_input = execution.get("max_input_bytes", 2_000_000)
        if not isinstance(self.max_input, int) or not 1 <= self.max_input <= 20_000_000:
            raise FrameworkError("execution.max_input_bytes must be 1..20000000")
        self.max_timeout = execution.get("max_timeout", 86400)
        if not isinstance(self.max_timeout, int) or not 1 <= self.max_timeout <= 86400:
            raise FrameworkError("execution.max_timeout must be 1..86400")
        self.secrets = sorted(
    ...
    def run(
        ...
        *,
        ...
        if not isinstance(timeout, (float, int)) or not 0 < timeout <= self.max_timeout:
            raise FrameworkError(
                f"Command timeout must be positive and at most {self.max_timeout} seconds"
            )
        if input_text is not None and len(input_text.encode("utf-8")) > self.max_input:
