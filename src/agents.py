...
def dispatch(
    ...
    *,
    ...
    needs_authority = role in {"implementation", "bugfix"} and definition["permissions"][
        "modify_files"
    ]
    ...
    def prepare() -> AgentResult | None:
        if previous is not None:
            if previous != binding:
                raise FrameworkError(
                    "Existing invocation binding changed; use a new run directory"
                )
            if not result_path.exists():
                raise FrameworkError(
                    "Uncertain invocation: intent exists without verified completion; "
        ...
        if any(
            path.exists()
            for path in (request_path, output, result_path, run_dir / "response.yaml")
        ):
