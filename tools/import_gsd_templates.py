...
def generate(files: dict[str, bytes]) -> dict[str, bytes]:
    ...
    pattern = re.compile(
        r"(?<![\w./-])(?:" + "|".join(re.escape(key) for key in sorted(replacements, key=len, reverse=True)) + r")(?![\w./-])"
    )
    generated = {}
    entries = []
    for source, destination in mapping.items():
        ...
        if source.endswith(".md"):
            ...
            file_pattern = pattern if file_replacements == replacements else re.compile(
                r"(?<![\w./-])(?:" + "|".join(re.escape(key) for key in sorted(file_replacements, key=len, reverse=True)) + r")(?![\w./-])"
            )
            counts = {}
