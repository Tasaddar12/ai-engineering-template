...
|---|---|---|
| `PROJECT.md` | Purpose, users, success and boundaries | Human intent, recorded during onboarding |
| `REQUIREMENTS.md` | Identified desired outcomes and traceability | Orchestrator |
| `ROADMAP.md` | Phase goals, order, plan checklists, milestones and progress | Runtime `phase.*`, `roadmap.*` and `milestone.*` verbs |
| `STATE.md` | Position, decisions, blockers and session continuity | Runtime `state.*` verbs; counters derived from ROADMAP.md |
